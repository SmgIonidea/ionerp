...
/**
 * Description          :   Model logic for Criterion 3(3.1) (Diploma TIER 2) - Correlation between the courses and the PO and PSO.
 * Created              :   05-06-2017
...
class T2dip_c3_cos_model extends CI_Model {
    ...
    public function course_po_mapping($curriculum) {
        $query = $this->db->query('call nba_t2dip_c3_1_3_CoursePOMapping("' . $curriculum . '")');
        mysqli_next_result($this->db->conn_id);
        if ($query->num_rows() > 0) {
            $data['columns_list'] = $query->list_fields();
            $data['row_list'] = $query->result_array();
...
/*
 * End of file t2dip_c3_cos_model.php
 * Location: .nba_sar/dip/tier2/criterion_3/t2dip_c3_cos_model.php 
 */
?>
