...
class import_mte_data_model extends CI_Model {
...
//					  AND c.crclm_id = "'.$curriculum_id.'"';
	...
	*/	
    public function course_list($crclm_id, $term_id) {
        ...
        if ($this->ion_auth->in_group('admin') || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner')) {
                     ...
                     $crs_list = 'SELECT map_ci.oid, map_ci.crs_id,
                                ...
                                ao.status as a_status
                                FROM course_clo_owner as map_ci
                                LEFT JOIN course as crs ON crs.crs_id = map_ci.crs_id
                                LEFT JOIN users as usr ON usr.id = map_ci.clo_owner_id
                                LEFT JOIN course_type as crs_type ON crs_type.crs_type_id = crs.crs_type_id
                                LEFT JOIN assessment_occasions as ao ON ao.crs_id = map_ci.crs_id
                                WHERE crs.crclm_id = "'. $crclm_id .'"
                                AND crs.crclm_term_id = "'. $term_id .'"
                                AND crs.status=1
                                AND ao.mte_flag = 1
                                AND crs.state_id >= 4 GROUP BY ao.crs_id';
			...
			$crs_list_return['crs_list_data'] = $crs_list_data;
                     ...
                     $crs_list = 'SELECT map_ci.oid, map_ci.crs_id,
                                crs.crs_title,crs_code, crs.crs_id, crs.crs_mode,crs.see_marks, crs.total_marks, crs.contact_hours, crs.see_duration, crs.lect_credits,
                                crs.tutorial_credits, crs.practical_credits, crs.self_study_credits, crs.total_credits, crs.cie_marks,
                                crs_type.crs_type_name,ao.status  as a_status,
                                usr.username,usr.title,usr.first_name,usr.last_name,usr.id
                                FROM course_clo_owner as map_ci
                                LEFT JOIN course as crs ON crs.crs_id = map_ci.crs_id
                                LEFT JOIN users as usr ON usr.id = map_ci.clo_owner_id
	...
	*/
	...
	//	if($section_id == 0){
					$mte_data= $this->db->query('SELECT DISTINCT ao.ao_id,  q.qp_rollout ,ao.qpd_id, ao.ao_type_id, ao.ao_name, ao.ao_description, aom.ao_method_name, mtd.mt_details_name,
					...
					WHERE ao.crs_id = "' . $crs_id . '"
						...
						AND q.qpd_id = ao.qpd_id
						AND q.cia_model_qp = 0 
						AND mtd.mt_details_id = ao.ao_type_id
						AND ao.mte_flag = 1   group by ao.ao_id ORDER BY ao.ao_name');

		$data['mte_result']  = $mte_data->result_array();
	...
	/* 	}else{
			$cia = 'SELECT DISTINCT ao.ao_id, ao.qpd_id, ao.ao_type_id, ao.ao_name, ao.ao_description, aom.ao_method_name, mtd.mt_details_name,
					...
					WHERE ao.crs_id = "' . $crs_id . '"
						...
						AND q.qpd_id = ao.qpd_id
						AND q.cia_model_qp = 0 
						AND ao.mte_flag = 0 ORDER BY ao.ao_name';
	...
	*/
	...
	public function check_mte_flag(){
	
		$query=$this->db->query('select mte_flag from organisation');
		$re =  $query->result_array();
		return $re[0]['mte_flag'];
	}
	
	/* Function is used to fetch the MTE details from MTE tables.
	* @param - course id.
	* @returns- a array of values of the course details.
	*/
    public function get_mte_details($crs_id,$section_id) {
		...
		$cia = 'SELECT DISTINCT ao.ao_id, ao.qpd_id, q.qp_rollout , ao.crclm_id,ao.term_id,ao.crs_id, ao.ao_name, ao.ao_description, 
                                        ao.ao_method_id, aom.ao_method_name, mtd.mt_details_name,
						ao.pi_code, ao.weightage, ao.max_marks, ao.avg_cia_marks
					FROM assessment_occasions AS ao, ao_method AS aom, master_type_details AS mtd,qp_definition as q
					WHERE ao.crs_id = "' . $crs_id . '"     				
					AND q.qpd_id = ao.qpd_id
					...
					AND q.cia_model_qp = 0 
					AND mtd.mt_details_id = ao.ao_type_id
					 group by ao_id  ORDER BY ao.ao_name';
        $result = $this->db->query($cia);
        $cia_result = $result->result_array();
        $data['cia_result'] = $cia_result;
    ...
    public function get_organisation_type($crclm_id,$term_id,$crs_id){
            ...
            if($org_data_res[0]['org_type'] == 'TIER-II'){
                    $target_approve_query = 'SELECT mte_target_percentage FROM attainment_level_course WHERE crs_id = "'.$crs_id.'" ';					
					...
					//|| $target_data['mte_target_percentage'] == 0  
                        if($target_data['mte_target_percentage'] == NULL ){
                            $counter =0;
                            break;
                        }else{
                            $counter = 1;
                    ...
                    $threshold_query = 'SELECT IF(mte_course_minthreshhold is NULL,0,mte_course_minthreshhold) as mte_course_minthreshhold FROM course WHERE crs_id = "'.$crs_id.'" ';
                    $threshold_data = $this->db->query($threshold_query);
                    $threshold_data_res = $threshold_data->row_array();
                    if($threshold_data_res['mte_course_minthreshhold']== 0){
                       $data['target_or_threshold_size'] = 0; 
                    }else{
                       $data['target_or_threshold_size'] = $threshold_data_res['mte_course_minthreshhold'];
                    }
                    $data['org_type'] = 'org1';
...
public function check_question_paper_created($ao_id){
  ...
  public function pdf_report_meta_data($ao_id){
      $get_the_meta_data_query = 'SELECT occ.crclm_id, occ.term_id, occ.crs_id, occ.section_id, occ.ao_description, cr.crclm_name,term.term_name, crs.crs_title FROM assessment_occasions as occ'
