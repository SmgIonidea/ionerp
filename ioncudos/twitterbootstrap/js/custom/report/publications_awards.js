	...
	function select_pgm_list() {
		...
		$.ajax({type: "POST",
			url: base_url+'report/publications_awards/select_pgm_list',
			data: post_data,
	...
	function select_crclm_list() {
		...
		$.ajax({type: "POST",
			url: base_url+'report/publications_awards/select_crclm_list',
			data: post_data,
			success: function(msg) {
				$('#crclm_id').html(msg);
	...
	function select_termlist() {
		...
		$.ajax({type: "POST",
			url: base_url+'report/publications_awards/select_termlist',
			data: post_data,
			success: function(msg) {
				$('#term').html(msg);
...
/* 	function select_courselist() {
		...
		var course_list_path = base_url + 'report/publications_awards/select_course';
		var crclm_id = $('#crclm_id').val();
	...
	//$('#loading').hide();
		...
		//	var course  = $('#course').val();
			...
			if(idClicked == 'update_award_publc'){ buttonvalue = 'update'} else if( idClicked == 'save_award_publc'){ buttonvalue = 'save' }
			   ...
			   $.ajax({type:"POST",
						url:base_url+'report/publications_awards/save_update_publication_awards',
						data:values,
						dataType:'json',
						success:function(data){//$('#loading').hide();
						$('#update_award_publc').hide();$('#save_award_publc').show();GetSelectedValue();
						reset_publication_awards();	if(data == 1){success_modal(data);}if(data == 0){fail_modal(data);}}}); 
			}else{//$('#loading').hide();
			} 
			}else{
			$('#loading').hide();	
	...
	function GetSelectedValue() {
		var publications_awards_path = base_url + 'report/publications_awards/show_publications_awards';
	...
	*/	
		...
		$('#example').dataTable(
				{	"sSort": true,
					...
					"aoColumns": [
						...
						{"sTitle": "Participants", "mData": "participants"},
	...
	var publication_awards = $('#publication_awards').val();
	 var post_data ={'publication_awards':publication_awards}
		   $.ajax({type:"POST",
					url:base_url+'report/publications_awards/delete_publc_award',
					data:post_data,					
					...
					var post_data = {
 					...
 					var uploader = document.getElementById('uploaded_file');	 
						upclick({
							...
							multiple: true,
							action: base_url+'report/publications_awards/upload',
							oncomplete:
	...
	function fetch_file_data(per_table_id){
					...
					$.ajax({type: 'POST',
					url: base_url+'report/publications_awards/fetch_files',
					data: post_data,
					success: function(data) {
	...
	$('#delete_uploaded_files').on('click',function(){
			...
			var per_table_id = $('#uload_id').val();
					...
					post_data =  {'per_table_id':per_table_id}
					$.ajax({type: 'POST',
					url: base_url+'report/publications_awards/delete_uploaded_files',
					data: post_data,
					success: function(data) {
