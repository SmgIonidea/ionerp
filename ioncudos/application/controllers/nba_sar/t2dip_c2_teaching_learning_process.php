...
/**
 * Description          :   Controller logic for Criterion 2 (Diploma TIER 2) - Teaching Learning Process.
 * Created              :   04-06-2016
...
class T2dip_c2_teaching_learning_process extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->model('nba_sar/dip/tier2/criterion_2/t2dip_c2_teaching_process_model');
    ...
    public function get_qp($id = '', $nba_sar_id = '', $nba_report_id = '', $other_info = array(), $is_export = 0, $param = '') {
        ...
        if ($is_export && @$qpd_id) {
            $qp_details = $this->t2dip_c2_teaching_process_model->get_qpd_details(@$qpd_id);
        ...
        if ($curriculum_id) {
            $this->data['course_list_data'] = $this->t2dip_c2_teaching_process_model->list_curriculum_courses($crclm_id);

            if ($course_id) {
                $this->data['qp_list'] = $this->t2dip_c2_teaching_process_model->list_course_qp($crclm_id, $course_id);
            }
        }

        return $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_2_qp_vw', $this->data, true);
    }
    ...
    public function display_curriculum_courses() {
        ...
        $data ['course_list'] = $this->t2dip_c2_teaching_process_model->list_curriculum_courses($crclm_id);
    ...
    public function display_course_qp() {
        ...
        $data ['qp_list'] = $this->t2dip_c2_teaching_process_model->list_course_qp($crclm_id, $crs_id);
    ...
    public function generate_model_qp_modal_tee() {
        ...
        $result = $this->t2dip_c2_teaching_process_model->qp_details($crclm_id, $term_id, $crs_id, $qp_type, $qpd_id);
        $model_qp_data['meta_data'] = $result['qp_meta_data'];
        $table = '';
        $qp_mapping_entity = $this->t2dip_c2_teaching_process_model->qp_mapping_entity();
        ...
        echo $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_2_qp_display_vw', $model_qp_data);
    }
    ...
    public function get_qp_course_outcome($id = '', $nba_sar_id = '', $nba_report_id = '', $other_info = array(), $is_export = 0, $param = '') {
        ...
        if (@$qpd_id) {
            $this->data['course_outcome'] = $this->t2dip_c2_teaching_process_model->getCOLevelMarksDistribution($course_id, $qpd_id);
        }

        return $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_2_qp_course_outcome_vw', $this->data, true);
    }
    ...
    public function get_qp_bloom_level($id = '', $nba_sar_id = '', $nba_report_id = '', $other_info = array(), $is_export = 0, $param = '') {
        ...
        if (@$qpd_id) {
            $this->data['bloom_level'] = $this->t2dip_c2_teaching_process_model->getBloomsLevelMarksDistribution_custom($course_id, $qpd_id);
        }

        return $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_2_qp_bloom_level_vw', $this->data, true);
    }
    ...
    public function course_co_planned_coverage() {
        $crs_id = $this->input->post('crs_id');
        $qpd_id = $this->input->post('qpd_id');
        $course['co_outcome'] = $this->t2dip_c2_teaching_process_model->getCOLevelMarksDistribution($crs_id, $qpd_id);
    ...
    public function course_bloom_level_marks_distribution() {
        $crs_id = $this->input->post('crs_id');
        $qpd_id = $this->input->post('qpd_id');
        $course['blooms_level_marks'] = $this->t2dip_c2_teaching_process_model->getBloomsLevelMarksDistribution_custom($crs_id, $qpd_id);
    ...
    /**
     * Function to display Quality of student projects - section 2.2.4
     * @parameters  : 
     * @return      : view
     */
    public function get_co_po_mapping($id = '', $nba_sar_id = '', $nba_report_id = '', $other_info = array(), $is_export = 0, $param = '') {
        ...
        $this->data['is_export'] = $is_export;
        return $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_4_student_quality_projects_vw', $this->data, true);
    }

    /**
     * Function to Get the Term List - section - 2.2.4,3.2.1
     * @parameters  :
     * @return      :
     */
    public function get_term_list() {
        $crclm_id = $this->input->post('curriculum');
        $crclm_list = $this->t2dip_c2_teaching_process_model->get_term_list($crclm_id);
    ...
    public function get_course_list() {
        $crclm_id = $this->input->post('crclm_id');
        $term_id = $this->input->post('term_id');
        $course_list = $this->t2dip_c2_teaching_process_model->get_course_list($crclm_id, $term_id);
    ...
    /**
     * Function to display Quality of student projects tabel - section 2.2.4
     * @parameters  : 
     * @return      : table
     */
    public function get_quality_student_project($crclm_id = NULL, $course_id = NULL) {
        ...
        if ($crclm_id == NULL) {
            ...
            $co_po_map = $this->t2dip_c2_teaching_process_model->get_quality_student_project($crclm_id, $course_id);
     ...
     * @return      :
     */
    public function get_company_industry_internship($id = '', $nba_sar_id = '', $nba_report_id = '', $other_info = array(), $is_export = 0, $param = '') {
        extract($other_info);
        $pgm_id = $this->input->post('pgm_id');
        $data ['curriculum_list'] = $this->nba_sar_list_model->list_curriculum($pgm_id);
        $filters = $this->nba_sar_list_model->nba_filters_list($id, $nba_report_id);
        ...
        if (!empty($filters['filter_list'])) {
            ...
            $content = $this->display_industry_internship($curriculum_id);
        }
        $this->data['is_export'] = $is_export;
        $this->data['crclm_id'] = @$crclm_id = $filter_list_data['curriculum_list'];
        $this->data['filter_list'] = @$crclm_id;
        $this->data['curriculum_list'] = $this->nba_sar_list_model->list_curriculum($pgm_id);
        $this->data['companies'] = $this->t2dip_c2_teaching_process_model->fetch_program_companies_visited($pgm_id);
        $this->data['industry_internship'] = empty($content) ? '' : $content;
        return $this->load->view('nba_sar/dip/tier2/criterion_2/t2dip_c2_2_5_company_industry_intership_vw', $this->data, true);
    }
    ...
    public function display_industry_internship($curriculum = NULL) {
        ...
        if ($this->input->post('curriculum')) {
            ...
            $industry_internship = $this->t2dip_c2_teaching_process_model->fetch_industry_internship($curriculum);
...
/* * End of file t2ug_c2_teachingLearningProcess.php
 * Location: .nba_sar/t2ug_c2_teachingLearningProcess.php 
 */
?>