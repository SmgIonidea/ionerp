...
class Publications_awards_model extends CI_Model {
	...
	/**
	 * Function to fetch level of presentation
	 * @parameters:
	 * @return:
	 */
    ...
    /*
     * Function is to fetch curriculum name and term name.
     * @parameters  : curriculum id, term id
     * returns:     : curriculum name and term name in a array.
     */

    public function fetch_data($crclm_id, $term_id) {
        ...
        $term_query = 'SELECT term_name
                       FROM crclm_terms
                       WHERE crclm_term_id="' . $term_id . '"';
        $term_data = $this->db->query($term_query);
        $term_name = $term_data->result_array();
        $term_name = $term_name[0]['term_name'];
