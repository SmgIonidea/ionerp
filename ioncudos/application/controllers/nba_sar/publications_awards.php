...
class Publications_awards extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('nba_sar/modules/publications_awards/publications_awards_model');
    }
    ...
    public function index() {
        if (!$this->ion_auth->logged_in()) {
			...
			$data['level_of_presentation'] = $this->publications_awards_model->fetch_level_of_presentation();
            $data['title'] = "Publications & Awards List Page";
            $this->load->view('nba_sar/modules/publications_awards/publications_awards_vw', $data);
        }
    }
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            if (!empty($exist)) {
                ...
                foreach ($exist as $data) {
                    ...
                    $abstract = "Abstract : " . htmlspecialchars($data['abstract']);
                    $list[] = array(
                        ...
                        'edit' => '<center><a role = "button" id="' . $data['publ_award_id'] . '"  
								 data-title = "' . $data['title'] . '" data-participants ="' . $data['participants'] . '" data-position ="' . $data['position'] . '"   data-abstract ="' . htmlspecialchars($data['abstract']) . '" data-venue ="' . htmlspecialchars($data['venue']) . '"  data-date="' . $date . '" class=" edit_award cursor_pointer"><i class="icon-pencil icon-black"> </i></a></center>',
                        'delete' => '<center><a role = "button" id="' . $data['publ_award_id'] . '" class="delete_award cursor_pointer"><i class="icon-remove icon-black"> </i></a></center>'
                    );
                    $i++;
                }
            } else {
    ...
    public function save_update_publication_awards() {
        ...
        $data['level_of_presentation'] = $this->input->post('level_of_presentation');
        $date_award = $this->input->post('award_publc_date');
        $data['venue'] = $this->input->post('award_venue');
        $abstract_description = $this->input->post('patent_abstracts');
        ...
        if ($save_update_btn == 'save') {
            $result = $this->publications_awards_model->save_award_publc($data);
            if ($result) {
                $user_id = $this->ion_auth->user()->row()->id;
                $details = $this->publications_awards_model->fetch_data($data['crclm_id'], $data['crclm_term_id']);
                $subject = "Added events details.";
                $body = "Dear Sir / Madam, <br/><br/><br/> This is an automated email from IonCUDOS Software.<br/><br/> <b>Publications & Awards in inter-institute events by students list : " . $data['title'] . "</b> event details has been added by <b>"
                        . $this->ion_auth->user($user_id)->row()->title . $this->ion_auth->user($user_id)->row()->first_name . " " . $this->ion_auth->user($user_id)->row()->last_name . "</b> for the term <b>" . $details['term_name'] . " - " . $details['curriculum_name'] . "</b>. " . "<br/><br/><br/>Warm Regards,<br/>IonCUDOS Admin";
                $this->ion_auth->send_nba_email($subject, $body);
            }
        } else if ($save_update_btn == 'update') {
            $result = $this->publications_awards_model->update_award_publc($data);
            if ($result) {
                $user_id = $this->ion_auth->user()->row()->id;
                $details = $this->publications_awards_model->fetch_data($data['crclm_id'], $data['crclm_term_id']);
                $subject = "Updated events details.";
                $body = "Dear Sir / Madam, <br/><br/><br/> This is an automated email from IonCUDOS Software.<br/><br/> <b>Publications & Awards in inter-institute events by students list : " . $data['title'] . "</b> event details has been updated by <b>"
                        . $this->ion_auth->user($user_id)->row()->title . $this->ion_auth->user($user_id)->row()->first_name . " " . $this->ion_auth->user($user_id)->row()->last_name . "</b> for the term <b>" . $details['term_name'] . " - " . $details['curriculum_name'] . "</b>. " . "<br/><br/><br/>Warm Regards,<br/>IonCUDOS Admin";
                $this->ion_auth->send_nba_email($subject, $body);
            }
        }
    ...
    public function fetch_files() {
        ...
        $output = $this->load->view('nba_sar/modules/publications_awards/uploded_files_data_vw', $data);
        echo $output;
    }
    ...
    public function save_res_guid_desc() {
        ...
        $result = $this->publications_awards_model->save_res_guid_desc($save_form_data);
