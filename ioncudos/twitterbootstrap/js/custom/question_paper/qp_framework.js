	...
	//Form validation rules are defined and checked before it is submitted to controller 
	...
	$('.Delete_framework').live('click',function(){
		var type_val = $(this).attr('value');
		var post_data = {
			'type_val': type_val
		};
		$.ajax({type: "POST",
			url: base_url+'question_paper/manage_qp_framework/check_qp_framework_used',
			data: post_data,
			datatype: "JSON",
			success: function(msg)
			{
				if( msg != 1) {
					...
					$('#myModalCannotdelete').modal('show')
				}
			}
		});
	});
	...
	*/
	...
	$('#section_generate').on('click', function(){
		...
		for( i = 1; i <= section_val; i++){
			...
			sections  = '<tr class="info" id="section_data_'+i+'">';
			sections += '<td><center>';
			...
			sections += '</center></td>';
			sections += '<td><center>';
			sections += '<input type="text"  name ="marks_'+i+'" id="marks_'+i+'" abbr="'+i+'" maxlength=3 class="input-mini required qp_unit_marks" onlyNumbers/>';
	...
	$('#qp_section_data').on('change','.qp_unit_marks',function(){
		ini_grand_total = 0;
	...
	for(n = 0 ; n < array_size; n++ ){
		...
		var table_head = '<tr id="tb_head_'+k+'" class="tb_sub_head"><th><center>Sl No.</center></th><th><center>Question No</center></th><th><center>Question Marks</center></th></tr>';
		...
		for(m = 1 ; m <= question_array[n]; m++ ){
			...
			var main_question_section  ='<tr id="sub_section_'+k+'" class="sub_section" ><td><center><input type="text" name ="main_question_num_'+k+'_'+m+'" id="main_question_num_'+k+'_'+m+'" abbr="'+m+'" class="input-medium required main_questions"  value="Q No '+t+'" readonly/></center></td>';
				main_question_section += '<td>';
				main_question_section += '<center>'+m+'</center>';
				main_question_section += '</td>';
				main_question_section += '<td><center>';
				main_question_section += '<input type="text" name ="que_marks_'+k+'_'+m+'" id="que_marks_'+k+'_'+m+'" abbr="'+m+'" class="input-mini required question_marks" value ="'+parseInt(mark_distribution)+'"/>';
	...
	$('#program_type').on('change', function(){
		...
		if(pgm_type_val){
			...
			$('#qp_title').val(pgm_type+' '+tee_lang +' Framework');
		}else{
			$('#qp_title').val('');
		}
	});
	...
	*/
	...
	$('#qp_data_save').on('click',function(){
		...
		if( form_valid && (section_duplicate == 0)) {
			...
			if(section_marks_total != grand_total){
				$("#grand_total_marks").val(section_marks_total);
				$("#grand_total_marks").css({"border-color": "red"});
