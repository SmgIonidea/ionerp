...
class Companies_visited_model extends CI_Model {
    ...
    public function insert_company_data($company_name, $sector_type_id, $collaboration_date, $company_description, $pgm_id, $crclm_id) {
        $company_data = array(
            'pgm_id' => $pgm_id,
            'crclm_id' => $crclm_id,
    ...
    public function fetch_companies_details($pgm_id, $crclm_id) {
        $companies_details_query = 'SELECT * ,m.mt_details_name,
                                    (SELECT COUNT(DISTINCT date) 
                                    FROM company_details AS cd
                                    WHERE cd.company_id=cv.company_id) as num_visits,
                                    (SELECT SUM(stud_intake) 
                                    FROM company_details AS cd
                                    WHERE cd.company_id=cv.company_id) as total_stud_intake
    ...
    public function update_company_data($company_id, $company_name, $sector_type_id, $collaboration_date, $company_description, $pgm_id, $crclm_id) {
    ...
    public function check_insert_data($company_name, $company_type_id, $collaboration_date, $pgm_id, $crclm_id) {
        $company_name = $this->db->escape_str($company_name);
        $check_insert_data_query = 'SELECT * '
                . '                 FROM companies_visited '
                . '                 WHERE company_name="' . $company_name . '" '
                . '                     AND collaboration_date="' . $collaboration_date . '" '
                . '                     AND sector_type_id="' . $company_type_id . '"'
                . '                     AND pgm_id="' . $pgm_id . '"'
                . '                     AND crclm_id="' . $crclm_id . '"';
    ...
    public function check_update_data($company_id, $company_name, $company_type_id, $collaboration_date, $pgm_id, $crclm_id) {
        $company_name = $this->db->escape_str($company_name);
        $check_insert_data_query = 'SELECT * '
                . '                 FROM companies_visited '
                . '                 WHERE company_name ="' . $company_name . '" '
 ...
 * Location: .report/curriculum_student_info/companies_visited_model.php 
 */
?>