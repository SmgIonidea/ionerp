...
/**
 * Description          :   Model logic for Criterion 4 (Pharmacy TIER 2)- Student Performance
 * Created              :   19-06-2017
...
class T2pharm_c4_student_performance_model extends CI_Model {
    ...
    public function fetch_student_performance($pgm_id, $year) {
        $result = $this->db->query('CALL nba_t2pharm_c4_Cummulative_shifts(' . $pgm_id . ', ' . $year . ')');
    ...
    public function fetch_enrolment_ratio($crclm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_1_EnrolmentRatio(' . $result['pgm_id'] . ',' . $result['start_year'] . ')');
        mysqli_next_result($this->db->conn_id);
    ...
    /**
     * Function to fetch student performance for report 4 listing students without backlog - section 4.2
     * @paramenters : pgm id and year
     * @return      : Array
     */
    public function fetch_student_without_backlogs($pgm_id, $year) {
        $result = $this->db->query('CALL nba_t2pharm_c4_Without_backlogs(' . $pgm_id . ', ' . $year . ')');
        mysqli_next_result($this->db->conn_id);
        $result = $result->result_array();
        return $result;
    }
    ...
    public function fetch_students_graduated($pgm_id, $year) {
        $result = $this->db->query('CALL nba_t2pharm_c4_Graduated_students(' . $pgm_id . ', ' . $year . ')');
        mysqli_next_result($this->db->conn_id);
        $result = $result->result_array();
        return $result;
    }
    ...
    public function fetch_success_rate($crclm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_2_1_SRWithoutBacklog(' . $result['pgm_id'] . ',' . $result['start_year'] . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_success_rate_in_stipulated($crclm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_2_2_SRInPeriod(' . $result['pgm_id'] . ',' . $result['start_year'] . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function to fetch the academic performance in in the given year - section 4.3 to 4.6
     * @paramenters : curriculum id
     * @return      : array
     */
    public function fetch_academic_performance($crclm_id, $year) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_3_AcademicPerformance(' . $result['pgm_id'] . ',' . $result['start_year'] . ',' . $year . '' . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function to fetch the Placement and Higher Studies details - section 4.7
     * @paramenters : curriculum id
     * @return      : array
     */
    public function fetch_placement_higher_studies($crclm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_7_Placement(' . $result['pgm_id'] . ',' . $result['start_year'] . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_professional_societies($dept_id) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_8_1_Professional_Societies(' . $dept_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_publications($dept_id) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_8_2_Publication(' . $dept_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_inter_institute_events($dept_id) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c4_8_3_inter_institute_events(' . $dept_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
...
/*
 * End of file t2pharm_c4_student_performance_model.php 
 * Location: .nba_sar/pharm/tier2/criterion_4/t2pharm_c4_student_performance_model.php
 */
?>