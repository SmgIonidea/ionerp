 ...
 */
  ...
  class Po_level_assessment_data extends CI_Controller {
  	...
  	public function __construct() {
  		...
  		$this->load->model('assessment_attainment/po_attainment/po_attainment_model');
	...
	*/
    public function index() {
    	if (!$this->ion_auth->logged_in()) {
			...
			$data['po_attainment_type'] = $this->po_attainment_model->getPOAttainmentType();
			$data['crclmlist'] = $this->po_attainment_model->crlcm_drop_down_fill();
	...
	*/
	public function select_term() {
		if (!$this->ion_auth->logged_in()) {
			...
			$term_data = $this->po_attainment_model->term_drop_down_fill($crclm_id);
			$list = '';
	...
	public function POThreshholdAttainment(){
		if (!$this->ion_auth->logged_in()) {
			...
			$graph_data = $this->po_attainment_model->getPOThreshholdAttainment($crclm_id, $term, $course=NULL, $qpid=NULL, $student_usn=NULL, $type_data,$po_attainment_type,$core_crs_id);			
			if($graph_data) {
	...
	public function CoursePOAttainment(){
		if (!$this->ion_auth->logged_in()) {
			...
			//var_dump($po);exit;
			$graph_data = $this->po_attainment_model->getCoursePOAttainment($crclm,$term, $po_id,$qp_type,$po_attainment_type,$core_crs_id);
		//	var_dump($graph_data);			
			if($graph_data) {
				echo json_encode($graph_data);
			} else {
	...
	public function CourseAttainment(){
		if (!$this->ion_auth->logged_in()) {
			...
			$term = implode(",", $this->input->post('term'));		
			$type_data = $this->input->post('type_data');			
			$graph_data = $this->po_attainment_model->getCourseAttainment($crclm_id, $term, $type_data);
	...
	*/
	public function select_survey() {
		if (!$this->ion_auth->logged_in()) {
			...
			$crclm_id = $this->input->post('crclm_id');
			$survey_data = $this->po_attainment_model->survey_drop_down_fill($crclm_id);
	...
	public function getDirectIndirectCOAttaintmentData() {
		if (!$this->ion_auth->logged_in()) {
			...
			$graph_data = $this->po_attainment_model->getDirectIndirectCOAttaintmentData($crclm_id,$qpd_id,$usn,$qpd_type,$direct_attainment,$indirect_attainment,$survey_id);
	...
	public function getDirectIndirectPOAttaintmentData() { 
		if (!$this->ion_auth->logged_in()) {
			...
			$graph_data = $this->po_attainment_model->getDirectIndirectPOAttaintmentData($crclm_id,$term_id,$qpd_id,$usn,$qpd_type,	
	...
	public function add_more_rows(){
		...
		$survey_data = $this->po_attainment_model->getSurveyNames();
		++$counter;
		$add_row = "";
		$add_row .="<tr>";
