...
/**
 * Description          :	Model logic for Criterion 2 (Pharmacy TIER 2) - Teaching Learning Process.
 * Created              :	17-06-2017
...
class T2pharm_c2_teaching_process_model extends CI_Model {
    ...
    public function qp_details($crclm_id, $term_id, $crs_id, $qp_type, $qpd_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2pharm_c2_2_2_Question_Paper(' . $qpd_id . ')');
    ...
    public function get_quality_student_project($crclm_id = NULL, $course_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c2_2_3_COPOMapping(' . $crclm_id . ', ' . $course_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_program_companies_visited($pgm_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c2_2_4_CompaniesList(' . $pgm_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_industry_internship($curriculum = NULL) {
        $call_proce = $this->db->query('CALL nba_t2pharm_c2_2_5_IndustryInternship(' . $curriculum . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
...
/*
 * End of file t2pharm_c2_teaching_process_model.php
 * Location: .nba_sar/pharm/tier2/criterion_2/t2pharm_c2_teaching_process_model.php 
 */
?>