...
class Tier1_course_clo_attainment_model extends CI_Model {
    /*
	...
	public function mte_dropdown_list($crclm_id, $term_id, $course_id){
        ...
        $tee_wgt = $wgt_data->total_tee_weightage;
				...
				$section_status_query = 'SELECT COUNT(sclo_oall_id) as status FROM tier1_section_clo_overall_cia_attainment WHERE  section_id IS NULL AND crs_id = "'.$course_id.'" ';
				$status_data = $this->db->query($section_status_query);
				$status_res = $status_data->row_array();
				   
				$select_query = 'SELECT tier1.clo_id, tier1.threshold_section_direct_attainment as attainment,  ROUND((tier1.threshold_section_direct_attainment)*' . $mte_wgt . '/100,2) as threshold_clo_da_attainment_wgt , tier1.average_section_direct_attainment as avg_attainment, tier1.section_id, tier1.crs_id, '
								. ' mtd.mt_details_name as section_name, mtd.mt_details_id, cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold  '
								. ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
								. ' LEFT JOIN master_type_details as mtd ON mtd.mt_details_id = tier1.section_id'
								. ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
     ...
     * Function to show section wise attainment drilldown
     */
    public function attainment_drill_down_mte($sec_id, $clo_id){
        $clo_data_query = 'SELECT clo_code, clo_statement, crs_id FROM clo WHERE clo_id = "'.$clo_id.'" ';
        $clo_data = $this->db->query($clo_data_query);
        $clo_result = $clo_data->row_array();
        ...
        $attainment_table_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id, '
                                ...
                                . ' LEFT JOIN course as crs ON crs.crs_id = att.crs_id '
                                . ' WHERE att.clo_id = "'.$clo_id.'" '
                                . ' AND att.crs_id = "'.$clo_result['crs_id'].'" AND att.assess_type = 3 AND ao.mte_flag = 1 ';
        $attainment_data = $this->db->query($attainment_table_query);
        $attainment_result = $attainment_data->result_array();
    ...
    public function tee_cia_finalized_co_data($crclm_id, $term_id, $course_id){
        ...
        $check_tee_query = 'SELECT COUNT(assess_type) as type FROM tier1_section_clo_ao_attainment WHERE crs_id = "'.$course_id.'" AND assess_type = 5 ';
        $check_tee_data = $this->db->query($check_tee_query);
        $check_tee_res = $check_tee_data->row_array();
       // if((empty($all_section_res)) && $check_tee_res['type'] > 0)
		{
            $crs_all_co_attainment_query = 'SELECT crs_id, clo_id,cast(ROUND(SUM(co_threshold_average)) as decimal(10,2)) as total_co_avg, 
                                            cast(ROUND(SUM(ao_da_average)) as decimal(10,2)) as ao_average,
                                            clo_code, clo_statement, CAST(cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , CAST(tee_clo_minthreshhold as decimal(10,2)) tee_clo_minthreshhold  
												FROM(
													SELECT tier1.crs_id, tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment)*'.$cia_wgt.'/100)as decimal(10,2)) as co_threshold_average,
													cast(ROUND(AVG(tier1.average_section_direct_attainment)*'.$cia_wgt.'/100)as decimal(10,2)) ao_da_average, 
													...
													FROM tier1_section_clo_overall_cia_attainment as tier1
													LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id
													WHERE tier1.crs_id = "'.$course_id.'"
													AND tier1.crclm_term_id = "'.$term_id.'"
													AND tier1.crclm_id = "'.$crclm_id.'"
													group by tier1.clo_id
													UNION ALL
													SELECT att.crs_id, att.clo_id, cast(ROUND(AVG(att.threshold_ao_direct_attainment)*'.$tee_wgt.'/100)as decimal(10,2)) as co_threshold_average,
													cast(ROUND(AVG(att.average_ao_direct_attainment)*'.$tee_wgt.'/100)as decimal(10,2)) as ao_da_average,
													cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold 
													FROM tier1_section_clo_ao_attainment as att
    ...
    public function finalized_po_attainment($crclm_id, $term_id, $course_id){
        $po_attainment_query = 'SELECT pod.po_da_id, pod.crclm_id, pod.crclm_term_id, pod.crs_id, pod.po_id, '
                                . ' pod.threshold_po_direct_attainment, pod.average_po_direct_attainment,pod.average_da ,pod.hml_weighted_average_da , pod.hml_weighted_multiply_maplevel_da,'
                                . ' pod.created_by, pod.created_date, pod.modified_by, pod.modified_date, '
                                . ' p.po_reference, p.po_statement  '
                                . ' FROM tier1_po_direct_attainment as pod '
	...
	public function finalized_po_attainment_mapping_data( $crclm_id  , $course_id){
			$result = $this->db->query("call POAttainment_COPOMapping(". $crclm_id .",".$course_id.")");
			return $result->result_array();	
	}
    ...
    public function all_section_attainemnt($crclm_id, $term_id, $course_id){
        ...
        if(empty($all_section_res)){
            ...
            $crs_all_co_attainment_query = 'SELECT tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) as co_average,'
                                            . ' cast(ROUND(AVG(tier1.average_section_direct_attainment))as decimal(10,2)) as average_attainment,'
                                            . ' cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold  '
                                            . ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
                                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
                                            . ' WHERE tier1.crs_id = "'.$course_id.'" '
                                            . ' AND tier1.crclm_term_id = "'.$term_id.'" '
                                            . ' AND tier1.crclm_id = "'.$crclm_id.'" GROUP BY tier1.clo_id';
            $crs_all_section_data = $this->db->query($crs_all_co_attainment_query);
            $crs_all_section_res = $crs_all_section_data->result_array();
    ...
    public function all_mte_coure_attainemnt($crclm_id, $term_id, $course_id){
        $check_all_mte_attain = '  SELECT * FROM course_clo_owner c JOIN course as crs on crs.crs_id = c.crs_id and crs.mte_flag =1 and c.crs_id = 1815 AND c.mte_finalize_flag !=1 ';
        $all_mte_data = $this->db->query($check_all_mte_attain);
        $all_mte_res = $all_mte_data->result_array();
        if(empty($all_mte_res)){
            ...
            $crs_all_co_attainment_query = 'SELECT tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) as co_average,'
                                            ...
                                            . ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
                                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
											. ' LEFT JOIN course as c on c.crs_id = tier1.crs_id and c.mte_flag =1'
                                            . ' WHERE tier1.crs_id = "'.$course_id.'" '
                                            . ' AND tier1.crclm_term_id = "'.$term_id.'" '
                                            . ' AND tier1.crclm_id = "'.$crclm_id.'" GROUP BY tier1.clo_id';
            $crs_all_mte_data = $this->db->query($crs_all_co_attainment_query);
            $crs_all_mte_res = $crs_all_mte_data->result_array();
            $data['all_mte_co_avg_attainment'] = $crs_all_mte_res;
            $data['all_mte_co_attain_check'] = 'false';
        }else{
            $data['all_mte_co_avg_attainment'] = 'false';
            $data['all_mte_co_attain_check'] = $all_section_res;
        }
        return $data;
     ...
     */
    public function clo_all_section_attainment ($crs_id, $clo_id){
        $crs_all_co_attainment_query = 'SELECT tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) as co_average, c.total_cia_weightage ,'
											. ' cast((cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) * c.total_cia_weightage)/100 as decimal(10,2)) as after_wt_cia,'
                                            . ' cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold '
                                            . ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
                                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
											. ' JOIN course as c on c.crs_id = cl.crs_id'
                                            . ' WHERE tier1.crs_id = "'.$crs_id.'" '
                                            . ' AND tier1.clo_id = "'.$clo_id.'" ';
            $crs_all_section_data = $this->db->query($crs_all_co_attainment_query);
            $crs_all_section_res = $crs_all_section_data->result_array();
	...
	public function student_drop_down_fill_tee($crclm_id ,$crs_id){
    ...
    public function tee_all_attainemnt($crclm_id, $term_id, $course_id){
        $tee_attainment_query = 'SELECT tier1.clo_id, tier1.threshold_ao_direct_attainment ,tier1.average_ao_direct_attainment, '
                                . '  cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold  '
                                . ' FROM tier1_section_clo_ao_attainment as tier1 '
                                . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
                                . ' WHERE tier1.crclm_id = "'.$crclm_id.'" '
                                . ' AND tier1.crclm_term_id = "'.$term_id.'" '
                                . ' AND tier1.crs_id = "'.$course_id.'" '
                                . ' AND tier1.assess_type = 5 ';
    ...
    public function clo_tee_attainemnt($crs_id,$clo_id){
        $tee_attainment_query = 'SELECT tier1.clo_id, tier1.threshold_ao_direct_attainment , cast((tier1.threshold_ao_direct_attainment * c.total_tee_weightage) / 100 as decimal(10,2)) as after_wt_tee, c.total_tee_weightage,'
                                . '  cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold   '
                                . ' FROM tier1_section_clo_ao_attainment as tier1 '
                                . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
								. ' JOIN course as c on c.crs_id = cl.crs_id '
                                . ' WHERE tier1.crs_id = "'.$crs_id.'" and tier1.clo_id = "'.$clo_id.'" '
                                . ' AND tier1.assess_type = 5 ';
        $tee_attainment_data = $this->db->query($tee_attainment_query);
        $tee_attainment_res = $tee_attainment_data->result_array();
        return $tee_attainment_res;
     ...
     */
    public function finalize_cia_tee_attainment_data($crclm_id, $term_id, $course_id,$clo_ids,$tee_cia_attain,$avg_ao_attain,$threshold_ao_attain, $co_threshold , $tee_threshold){
        
		$count_query = $this->db->query('select * from tier1_crs_clo_overall_attainment where crs_id = "'. $course_id.'" and crclm_term_id = "'. $term_id.'" and crclm_id = "'. $crclm_id.'"');
		$count_data = $count_query->result_array();
		// Delete data from the course before inserting.
        if(!empty($count_data)){$delete_query = 'DELETE FROM tier1_crs_clo_overall_attainment 
		...
		$tee_threshold_array = explode(',',$tee_threshold);
       	  
        $size = count($threshold_ao_attain_array);
        for($j=0;$j<$size;$j++){
            $insert_finalize_data = array(
                ...
                'threshold_clo_da_attainment'=>$tee_cia_attain_array[$j],
                'threshold_clo_ia_attainment'=>0,
                'average_clo_da_attainment'=>$avg_ao_attain_array[$j],
                'average_clo_ia_attainment'=>0,
                'threshold_clo_overall_attainment'=>$tee_cia_attain_array[$j],
                'average_clo_overall_attainment'=>$avg_ao_attain_array[$j],
                'created_by'=>$this->ion_auth->user()->row()->id,
                'created_date'=>date('y-m-d'),
                'modified_by'=>$this->ion_auth->user()->row()->id,
                'modified_date'=>date('y-m-d'),
            );
            $this->db->insert('tier1_crs_clo_overall_attainment',$insert_finalize_data);
        }
        
        $po_delete_query = 'DELETE FROM tier1_po_direct_attainment 
								WHERE crclm_id = "'.$crclm_id.'" 
								AND crclm_term_id = "'.$term_id.'" 
								AND crs_id = "'.$course_id.'" ';
        $po_data_delete_success = $this->db->query($po_delete_query);
        
        $procedure_call = $this->db->query('CALL Save_Tier1_POAttainment('.$crclm_id.','.$term_id.','.$course_id.' , NULL)');
    ...
    public function finalize_attainment_data($crclm_id, $term_id, $course_id){
        $finalize_query = 'SELECT att.crs_clo_oall_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.clo_id, att.crs_da_weightage, '
                        . ' att.crs_ia_weightage, att.threshold_clo_da_attainment, att.threshold_clo_ia_attainment, '
                        . ' att.average_clo_da_attainment, att.average_clo_ia_attainment, att.threshold_clo_overall_attainment, '
                        . ' att.average_clo_overall_attainment, att.created_by, att.created_date, att.modified_by, att.modified_date, '
                        . ' cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.tee_clo_minthreshhold  '
                        . ' FROM tier1_crs_clo_overall_attainment as att'
                        . ' LEFT JOIN clo as cl ON cl.clo_id = att.clo_id '
                        . ' WHERE att.crclm_id = "'.$crclm_id.'" '
                        . ' AND att.crclm_term_id = "'.$term_id.'" '
                        . ' AND att.crs_id = "'.$course_id.'" ';
        ...
        if(isset($occasion_type)) {
            $type_string = ($occasion_type == '3') ? $this->lang->line('entity_cie') : 
                            (($occasion_type == '5') ? $this->lang->line('entity_tee') : 
                               ( $this->lang->line('entity_cie').', '.$this->lang->line('entity_tee') ) ) ;
	...
	public function multiple_section_attainemnt($crclm_id, $term_id, $course_id , $type_id , $type_not_selected){				
		   ...
		   $tee_attainment_query = 'SELECT tier1.clo_id, tier1.threshold_ao_direct_attainment as co_threshold_average ,tier1.average_ao_direct_attainment as ao_da_average, '
                                . '  cl.clo_code, cl.clo_statement, CAST(cl.tee_clo_minthreshhold as decimal(10,2)) as tee_clo_minthreshhold , cl.mte_clo_minthreshhold,cl.cia_clo_minthreshhold  '
                                . ' FROM tier1_section_clo_ao_attainment as tier1 '
                                . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
                                . ' WHERE tier1.crclm_id = "'.$crclm_id.'" '
                                . ' AND tier1.crclm_term_id = "'.$term_id.'" '
            ...
            $mte_attainment_query = 'SELECT tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) as co_threshold_average,'
                                            . ' cast(ROUND(AVG(tier1.average_section_direct_attainment))as decimal(10,2)) as ao_da_average,'
                                            . ' cl.clo_code, cl.clo_statement, CAST(cl.mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold , cl.cia_clo_minthreshhold , cl.tee_clo_minthreshhold  '
                                            . ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
                                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
											. ' LEFT JOIN course as c on c.crs_id = tier1.crs_id '
                                            . ' WHERE tier1.crs_id = "'.$course_id.'" '
                                            . ' AND tier1.crclm_term_id = "'.$term_id.'" '
											. ' AND tier1.section_id IS NULL'
											. ' AND c.mte_flag =1'
            ...
            $cia_attainment_query = 'SELECT tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment))as decimal(10,2)) as co_threshold_average,'
                                            . ' cast(ROUND(AVG(tier1.average_section_direct_attainment))as decimal(10,2)) as ao_da_average,'
                                            . ' cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , cl.mte_clo_minthreshhold, cl.tee_clo_minthreshhold '
                                            . ' FROM tier1_section_clo_overall_cia_attainment as tier1 '
                                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
                                            . ' WHERE tier1.crs_id = "'.$course_id.'" '
                                            . ' AND tier1.crclm_term_id = "'.$term_id.'" '
                                            . ' AND tier1.crclm_id = "'.$crclm_id.'"'
											. ' AND tier1.section_id IS NOT NULL GROUP BY tier1.clo_id';											
	$res_wgt = $this->db->query("SELECT total_cia_weightage, total_mte_weightage , total_tee_weightage FROM course c where crs_id='" . $course_id . "'");
        $wgt_data = $res_wgt->row();
        $cia_wgt = $wgt_data->total_cia_weightage;
        $mte_wgt = $wgt_data->total_mte_weightage;
		$tee_wgt = $wgt_data->total_tee_weightage;
			...
			$tee_finalize_status_query = $this->db->query('SELECT * FROM tier1_section_clo_ao_attainment t where crs_id = "'. $course_id .'" and assess_type = 5');
			...
			if(!empty($tee_finalize_status_re)){$tee_finalize_status =  1;}else{$tee_finalize_status = 0;}	
			 ...
			 $cia_query = 'SELECT tier1.crs_id, tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment)*'.$cia_wgt.'/100)as decimal(10,2)) as co_threshold_average,
													cast(ROUND(AVG(tier1.average_section_direct_attainment)*'.$cia_wgt.'/100)as decimal(10,2)) ao_da_average, 
													cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold ,
													CAST(cl.mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold ,
													cl.tee_clo_minthreshhold 
													FROM tier1_section_clo_overall_cia_attainment as tier1
													LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id
													LEFT JOIN assessment_occasions as ao ON ao.crs_id = tier1.crs_id
													WHERE tier1.crs_id = "'.$course_id.'"
													AND tier1.crclm_term_id = "'.$term_id.'"
													AND tier1.crclm_id = "'.$crclm_id.'"
													AND ao.mte_flag = 0
													AND tier1.section_id  IS NOT NULL
													group by tier1.clo_id
													';	
			$mte_query = 'SELECT tier1.crs_id, tier1.clo_id, cast(ROUND(AVG(tier1.threshold_section_direct_attainment)*'.$mte_wgt.'/100)as decimal(10,2)) as co_threshold_average,
													cast(ROUND(AVG(tier1.average_section_direct_attainment)*'.$mte_wgt.'/100)as decimal(10,2)) ao_da_average, 
													cl.clo_code, cl.clo_statement, CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold ,
													CAST(cl.mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold ,
													cl.tee_clo_minthreshhold 
													FROM tier1_section_clo_overall_cia_attainment as tier1
													...
													group by tier1.clo_id
													';
			$tee_query = 'SELECT att.crs_id, att.clo_id, cast(ROUND(AVG(att.threshold_ao_direct_attainment)*'.$tee_wgt.'/100)as decimal(10,2)) as co_threshold_average,
													cast(ROUND(AVG(att.average_ao_direct_attainment)*'.$tee_wgt.'/100)as decimal(10,2)) as ao_da_average,
													cl.clo_code, cl.clo_statement,CAST(cl.cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , 
													CAST(cl.mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold ,													
													cl.tee_clo_minthreshhold 
			...
			if($type_not_selected == 0){
				...
				if($cia_finalize_status == 0 && $crs_cia_flag == 1 ){ $Finalize_status .= '<br/>'.$this->lang->line('entity_cie')." is not Finalized  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/tier1_section_clo_attainment>Click here to Finalize course .</a>";}
				if($mte_finalize_status == 0 && $crs_mte_flag == 1){ $Finalize_status .= '<br/>'.$this->lang->line('entity_mte')." is not Finalized  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/tier1_mte_clo_attainment>Click here to Finalize course .</a>";}
				if($tee_finalize_status == 0 && $crs_tee_flag == 1){ $Finalize_status .= '<br/>'.$this->lang->line('entity_tee')." marks not uploaded  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/import_assessment_data>Click here to Upload Marks .</a>";}
		 			
				$crs_all_co_attainment_query = 'SELECT crs_id, clo_id,cast(ROUND(SUM(co_threshold_average)) as decimal(10,2)) as co_threshold_average, 
                                            cast(ROUND(SUM(ao_da_average)) as decimal(10,2)) as ao_da_average,
                                            clo_code, clo_statement, CAST(cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , 
											CAST(mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold , 
											CAST(tee_clo_minthreshhold as decimal(10,2)) tee_clo_minthreshhold  
												FROM('. $cia_query.'
				...
				$data['error_msg'] = $Finalize_status;	
					...
					if(count($type_id) == 1){				
						for($i=0;$i<count($type_id);$i++){						
							switch($type_id[$i]){
								case 3: $executing_query = $cia_attainment_query;
										if($cia_finalize_status == 0){ $Finalize_status .= '<br/>'.$this->lang->line('entity_cie')." is not Finalized  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/tier1_section_clo_attainment>Click here to Finalize course .</a>";} 
								break;
								case 6: $executing_query = $mte_attainment_query;
										if($mte_finalize_status == 0){  $Finalize_status .= '<br/>'.$this->lang->line('entity_mte')." is not Finalized  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/tier1_mte_clo_attainment>Click here to Finalize course .</a>";}
								break;
								case 5: $executing_query = $tee_attainment_query;
										if($tee_finalize_status == 0){ $Finalize_status .= '<br/>'. $this->lang->line('entity_tee')." marks not uploaded  for this course."."<br/><a class='cursor_pointer' href=". base_url()."assessment_attainment/import_assessment_data>Click here to Upload Marks .</a>";}
								break;													
							}
						}
						...
						$multipe_type_attainment = 'SELECT crs_id, clo_id,cast(ROUND(SUM(co_threshold_average)) as decimal(10,2)) as co_threshold_average, 
						cast(ROUND(SUM(ao_da_average)) as decimal(10,2)) as ao_da_average,
						clo_code, clo_statement, CAST(cia_clo_minthreshhold as decimal(10,2)) as cia_clo_minthreshhold , 
						CAST(mte_clo_minthreshhold as decimal(10,2)) as mte_clo_minthreshhold , 
						CAST(tee_clo_minthreshhold as decimal(10,2)) tee_clo_minthreshhold 
											FROM('. $query1.' UNION ALL '. $query2.')s
									GROUP BY clo_id';
