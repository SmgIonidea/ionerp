...
/**
 * Description      :	Model logic for Criterion 8 (TIER 1) - First Year Academics.
 * Created          :   14-6-2016
...
class T1ug_c8_first_year_academic_model extends CI_Model {

    /**
     * Function to fetch First year student faculty ratio - section 8.1
     * @parameters  :
     * @return      :
     */
    public function fysfr_grid($crclm_id, $pgm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t1ug_c8_1_Student_Faculty_Ratio_IYear(' . $result['start_year'] . ', ' . $pgm_id . ')');
    ...
    public function get_facutly_teaching_fycc($crclm_id, $pgm_id, $dept_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t1ug_c8_2_Faculty_Qualification_IYear(' . $result['start_year'] . ', ' . $pgm_id . ', ' . $dept_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function to fetch academic performance - section 8.3
     * @parameters  :
     * @return      :
     */
    public function get_academic_performance($crclm_id, $pgm_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t1ug_c4_3_AcademicPerformance(' . $result['pgm_id'] . ',' . $result['start_year'] . ',1' . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function to get the curriculum course
     * @parameters  : curriculum id 
     * @return      : array
     */
    ...
    /**
     * Function to fetch the Course Wise PO Attainment and PO Indirect Attainment - 8.5.1
     * @parameters  : curriculumn id
     * @return      : array
     */
    public function for_all_po_mapping($crclm_id) {
        ...
        $proc_query = $this->db->query('CALL nba_t1ug_c8_5_1_POAttainment(' . $crclm_id . ',"' . $term_id_val . '")');
        mysqli_next_result($this->db->conn_id);
...
/*
 * End of file t1ug_c8_first_year_academic_model.php
 * Location: .nba_sar/ug/tier1/criterion_8/t1ug_c8_first_year_academic_model.php 
 */
?>