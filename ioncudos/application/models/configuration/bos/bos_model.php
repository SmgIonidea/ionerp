...
class bos_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
         ...
         * @author Saraswathi
         * */
        if (isset($additional_data['bos_dept_id'])) {
            $this->db->insert('bos', array(
                'bos_user_id' => $id,
                'bos_dept_id' => $additional_data['bos_dept_id'],
     ...
     *
     * Get groups list as array
     *
     * @return bool
     * @author Taha M Asadullah
     * */
    public function has_permission($group_list, $permission_name) {
        ...
        SELECT permission_id
        FROM permission
        WHERE permission_function =  '$permission_name')"
        );
        return ($query->num_rows() > 0) ? TRUE : FALSE;
    }

    public function user_number($username, $dept_id) {

        $query = (($dept_id > 0) ? ($this->db->query("SELECT id FROM users WHERE username LIKE '%$username%'AND user_dept_id='$dept_id'")) : ($this->db->query("SELECT id FROM users WHERE username LIKE '%$username%'")));
    ...
    /*
      ...
      return $this->db->select('id,name,description')->get('groups')->result_array();
      }
     */

    /* Function is to fetch user groups list from groups table.
     * @param - 
     * @returns- an array of values of user groups.
     */
    ...
    /* Function is to fetch designations list from designation table.
     * @param - 
     * @returns- an array of values of designations.
     */

    function designation_list() {

        return $this->db->select('designation_id, designation_name')->where('upper(designation_name) !=' , 'STUDENT')->get('user_designation')->result_array();
    }

    /* Function is to fetch departments list from department table.
     * @param - 
     * @returns- an array of values of departments.
     */
    ...
    /* Function is to fetch general department id from department table.
     * @param - 
     * @returns- an general department id value.
     */

    function default_department() {
        $name = 'General Department';
        return $this->db
                        ->select('dept_id')
                        ->where('dept_name', $name)
                        ->get('department')->row_array();
    }

    /* Function is to fetch department id of BOS user from BOS table.
     * @param - BOS user id
     * @returns- an BOS department id value.
     */

    function edit_default_department($user_id, $bos_id) {

        return $this->db
                        ->select('bos_dept_id')
                        ->where('bos_id', $bos_id)
                        ->where('bos_user_id', $user_id)
                        ->get('bos')->row_array();
    }

    /* Function is to fetch department name from department table.
     * @param - department id
     * @returns- an department name value.
     */

    public function get_dept_name_by_id($dept_id) {
        $this->db->select('dept_name');
        return $this->db->get_where('department', array('dept_id' => $dept_id))->result_array();
    }

    public function bulk_upload($file_name) {
        $file_path = $_SERVER['DOCUMENT_ROOT'] . 'ion-auth8/uploads/' . $file_name;

        $query = "LOAD DATA LOCAL INFILE '$file_path' INTO TABLE users FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\\\\' LINES TERMINATED BY '\\r\\n' IGNORE 1 LINES";
        echo $query;
        //exit();
        $this->db->query($query);
    }

    /* Function is to fetch BOS user ids from BOS table.
     * @param - 
     * @returns- an array of values of BOS user ids.
     */

    public function bos_user_list() {
        return $this->db
                        ->select('bos_user_id')
                        ->get('bos')
                        ->result_array();
    ...
    //BOS table update function
    /* Function is to update BOS user details onto BOS table.
     * @param -BOS department id, user id. 
     * @returns- a boolean value.
     */
    public function update_bos($bos_dept_id, $uid, $bos_id) {

        $this->db->where('bos_id', $bos_id)
                ->where('bos_user_id', $uid)
                ->update('bos', array('bos_dept_id' => $bos_dept_id,));
        //'modified_by' => $this->ion_auth->user()->row()->id,
        //'modify_date' => date('Y-m-d') 
    }

    // BOS Add existing user function.
    /* Function is to insert new BOS user onto BOS table.
     * @param -BOS user id, BOS department id. 
     * @returns- a boolean value.
     */
    public function insert_existing_bos_user($bos_user_id, $bos_dept_id) {
        $data = array('bos_user_id' => $bos_user_id,
            'bos_dept_id' => $bos_dept_id,
            'create_by' => $this->ion_auth->user()->row()->id,
            'create_date' => date('Y-m-d'));

        $group_data = array('user_id' => $bos_user_id,
            'group_id' => 15);

        //Function to count the BOS user id with the same BOS department id entry onto BOS
        $res = $this->db
                ->where('bos_user_id', $bos_user_id)
                ->where('bos_dept_id', $bos_dept_id)
                ->count_all_results('bos');

        //Function to count the BOS user id with the same group id entry onto users_groups		
        $group_res = $this->db
                ->where('user_id', $bos_user_id)
                ->where('group_id', 15)
        ...
        //condition to avoid duplicate entry onto BOS table   
        if ($res == 0) {
            $this->db->insert('bos', $data);
            // email
            $dept_name = $this->db->select('dept_name')
                    ->where('dept_id', $bos_dept_id)
                    ->get('department')
                    ->result_array();
            $links = base_url();
            $entity_id = '19';
            $additional_data['dept_name'] = $dept_name[0]['dept_name'];
            $this->ion_auth->send_email($bos_user_id, $cc = NULL, $links, $entity_id, $state = 3, $crclm_id = NULL, $additional_data);
            //echo "Inserted successfully";
            return "0";
        } else {
            //return msg to ajax call
            //echo "cannot successfully";
            return "1";
        }
    }

    /* Function is to fetch BOS users list from user table.
     * @param - 
     * @returns- an array of values of BOS user details.
     */

    public function list_bos_users() {
        return $this->db
                        ->select('id')
                        ...
                        ->join('bos', 'users.id=bos.bos_user_id')
                        ->join('user_designation', 'user_designation.designation_id= users.designation_id')
                        ->join('department', 'department.dept_id= bos.bos_dept_id')
                        ->get('users')
                        ->result();
    }

    /* Function is to update BOS user status field onto BOS table.
     * @param - BOS user id, status value
     * @returns- a boolean value.
     */

    function bos_status($bos_id, $active) {
        $bos_stat = 'UPDATE users SET active = "' . $active . '" WHERE id = "' . $bos_id . '" ';
        $res = $this->db->query($bos_stat);
        $row = $this->db->query($bos_stat);
        $row1 = $row->result_array();
        $ret['rows'] = $row1;
        return $ret;
    }

    public function delete_bos($id, $bos_dept_id) {
        $query1 = $this->db->query('DELETE FROM bos WHERE bos_user_id = "' . $id . '" and bos_dept_id = "' . $bos_dept_id . '" ');
        $query2 = $this->db->query('DELETE FROM map_user_dept_role WHERE user_id = "' . $id . '" AND dept_id = "' . $bos_dept_id . '" AND role_id = 15');
        return true;
    }

    public function check_delete_bos($id) {
        $query = 'SELECT * FROM peo_po_approver WHERE approver_id = ' . $id;
