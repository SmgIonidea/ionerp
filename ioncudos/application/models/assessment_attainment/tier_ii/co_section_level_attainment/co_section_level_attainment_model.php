...
class Co_section_level_attainment_model extends CI_Model {
    /*
...
////AND q.qp_rollout > 1  and AND a.qpd_id IS NOT NULL and AND q.qpd_id = a.qpd_id
	...
	public function find_occsions_finalized_or_not($course, $qpd_id, $type ,$section_id , $occasion_not_selected){
    ...
    public function get_clo_attainment($course_id, $ao_id, $type , $section_id ,$occasion_not_selected) {
	  ...
	  if ($type == 3) {
            if ($occasion_not_selected == 0) { 
					  ...
					  ROUND(AVG(threshold_direct_attainment),2) as threshold_direct_attainment , ROUND(AVG(threshold_direct_attainment)*'" . $cia_wgt . "'/100,2) as threshold_clo_da_attainment_wgt ,
					  ...
					  ROUND(AVG(t.attainment_level),2)as attainment_level FROM tier_ii_section_clo_ao_attainment t,clo c
						WHERE c.clo_id = t.clo_id AND t.crs_id= '". $course_id ."' AND t.assess_type= '". $type ."' and section_id = '". $section_id ."' group by c.clo_code");
			  ...
			  if( threshold_direct_attainment  IS NULL ||  threshold_direct_attainment  = 0 ,'', CONCAT( ROUND(AVG(threshold_direct_attainment),2) ,'%')  ) as threshold_direct_attainment_display ,
				...
				ROUND(AVG(threshold_direct_attainment)*'". $cia_wgt . "'/100,2) as threshold_clo_da_attainment_wgt ,
			  if( average_direct_attainment  IS NULL ||  average_direct_attainment  = 0 ,'', CONCAT( ROUND(AVG(average_direct_attainment),2) ,'%')  ) as average_direct_attainment_display , 
			  ROUND(AVG(average_direct_attainment),2) as average_direct_attainment   ,
			  (t.attainment_level) as attainment_level FROM tier_ii_section_clo_ao_attainment t,clo c
				WHERE c.clo_id = t.clo_id AND t.ao_id  IN(".$occasion_selected.") AND t.crs_id= '". $course_id ."' AND t.assess_type= '3' and section_id = '". $section_id ."' group by c.clo_code");
            }
        } else if ($type == 5) {
	...
	function fetch_status($course, $ao_id, $type ,$section_id ,$occasion_not_selected){
	if( $occasion_not_selected != 0){
		$occasion = implode("," , $ao_id);
		$occasion_selected = str_replace("'", "", $occasion);
		$query = $this->db->query("SELECT a.ao_id , a.qpd_id , a.ao_description,q.qp_rollout ,
									...
									FROM assessment_occasions as a
									left join qp_definition as q on q.qpd_id = a.qpd_id where a.crs_id = ". $course ."  and ao_id IN(". $occasion_selected.") and section_id = ". $section_id ." ");
		}else{
		$query =  $this->db->query("SELECT a.ao_id , a.qpd_id , a.ao_description,q.qp_rollout , 
												CASE WHEN  (a.qpd_id IS NULL) THEN concat(' >> Question Paper is not defined for these Occasions : <b>',  group_concat(a.ao_description SEPARATOR  '  ,  ') ,'</b>', '  ')
												WHEN  (q.qp_rollout = 1) THEN concat('','  <br/> >> Assessment data (student marks) are not imported/uploaded for these Occasions : <b>', group_concat(a.ao_description SEPARATOR  ' , ') ,'</b>')
    ...
    public function section_dropdown_list_finalize($crclm_id, $term_id, $course_id ,$section_id){
        ...
        if(!empty($section_result))
		{
            ...
            $select_query = 'SELECT tier2.clo_id, tier2.threshold_clo_da_attainment as attainment, ROUND((tier2.threshold_clo_da_attainment)*' . $cia_wgt . '/100,2) as threshold_clo_da_attainment_wgt , tier2.average_clo_da_attainment as avg_attainment, tier2.section_id, tier2.crs_id,tier2.attainment_level, '
                            ...
                            .'  AND tier2.section_id = "'.$section_val['section_id'].'"';
    ...
    public function finalize_clo_attainment($crclm_id,  $term_id, $course_id, $section_id, $type_id, $occasion_id){
        ...
        $attainment_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id, '
                            ...
                            . ' AND att.crclm_term_id = "'.$term_id.'" AND att.crs_id = "'.$course_id.'" AND att.section_id = "'.$section_id.'"   AND section_id IS NOT NULL GROUP BY att.clo_id ';
       $clo_attainment_data = $this->db->query($attainment_query);
       $clo_attainment_res = $clo_attainment_data->result_array();
        
        $data_check = 'SELECT COUNT(clo_oa_id) as data_size FROM tier_ii_section_clo_overall_cia_attainment WHERE crs_id = "'.$course_id.'"  AND section_id = "'.$section_id.'" AND section_id IS NOT NULL ';
		...
		foreach($clo_attainment_res as $clo_final){
            ...
            $delete_query  = 'DELETE FROM tier_ii_section_clo_overall_cia_attainment WHERE crs_id = "'.$course_id.'" AND section_id = "'.$section_id.'" ';
    ...
    public function get_co_mapped_questions($clo_id, $type_data, $ao_id , $section_id ,$occasion_not_selected) {	
		$occasion = implode("," , $ao_id);
		$occasion_selected = str_replace("'", "", $occasion);
        ...
        if ($type_data == 2) {
            ...
            if ($occasion_not_selected == 0) {
                $result = $this->db->query('SELECT qpm.actual_mapped_id, qpm.entity_id, mnq.qp_subq_code, qpm.qp_mq_id, qpm.qp_map_id, mnq.qp_unitd_id, mnq.qp_content, un.qpd_id,  qpd.qpd_type,mm.mt_details_name,mnq.qp_subq_marks,aocc.ao_description,c.clo_code,c.clo_statement   
										...
										AND aocc.section_id = "'. $section_id .'"
