...
/**
 * Description          :   Model logic for NBA SAR Report - Section 3.2 and 3.3 (TIER II) - Attainment of co and po.
 * Created              :   04-01-2017
...
class T2ug_c3_co_attainment_model extends CI_Model {
    ...
    public function course_assement_occasions_list($course_id, $crclm_id = NULL, $term_id = NULL) {
        $query = $this->db->query('CALL nba_t2ug_c3_2_1_AssessmentMethods_CO(' . $crclm_id . ', ' . $term_id . ', ' . $course_id . ')');
        mysqli_next_result($this->db->conn_id);
    ...
    /**
     * Function to fetch course assessment occasion - section 8,3.2.1 - 3
    ...
    public function attainment_qpd_rubrics($crclm_id, $course_id, $term_id, $ao_method_id) {
        $call_proce = $this->db->query('CALL nba_t2ug_c3_2_1_Rubrics(' . $ao_method_id . ')');
        mysqli_next_result($this->db->conn_id);
    ...
    public function course_target_level($crs_ids = null) {
        $course_wise_query = $this->db->query('call nba_t2ug_c3_2_2_COAttainment("' . $crs_ids . '")');
        mysqli_next_result($this->db->conn_id);
    ...
    public function fetch_curriculum_survey_attainment_tools($survey_id) {
        $call_proce = $this->db->query('CALL nba_t2ug_c3_3_1_Survey(' . $survey_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function for_all_po_mapping($crclm_id) {
        $proc_query = $this->db->query('CALL nba_t2ug_c3_3_2_POAttainment(' . $crclm_id . ')');
        mysqli_next_result($this->db->conn_id);
    ...
    public function fetch_curriculum_survey_attainment($survey_id) {
        $call_proce = $this->db->query('CALL nba_t2ug_c3_3_2_SurveyAttainment(' . $survey_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
...
/*
 * End of file t2ug_c3_co_attainment_model.php 
 * Location: .nba_sar/t2ug_c3_co_attainment_model.php
 */
?>