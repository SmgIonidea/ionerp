...
/**
 * Description	:	Controller Logic for PO to PEO Mapped Report Module. 
...
class Po_peo_map_report extends CI_Controller {

    public function __construct() {
	parent::__construct();
	$this->load->model('report/po_peo_map/po_peo_map_report_model');
...
// End of function __construct.
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $crclm_list = $this->po_peo_map_report_model->fetch_crclm_list();
	    ...
	    $data['title'] = $this->lang->line('so') . ' To PEO Mapping Report';
	    $this->load->view('report/po_peo_map/po_peo_map_report_vw', $data);
	}
    }
...
// End of function index.
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $data['title'] = $this->lang->line('so') . ' To PEO Mapping Report';
	    $this->load->view('report/po_peo_map/static_po_peo_map_report_vw', $data);
	}
    }

// End of function static_index.

    /* Function is used to load the table grid of po peo mapped report view.
     * @param-
     * @retuns - the table grid of po peo mapped report details.
     */

    public function fetch_po_peo_mapping_details() {
	$crclm_id = $this->input->post('crclm_id');
	// $data store an array values of mapped POs & PEOs fetched from the po_peo_map table.
	$data = $this->po_peo_map_report_model->fetch_po_peo_mapping_details($crclm_id);
	if (!empty($data['map_details'])) {
	    $data['title'] = $this->lang->line('so') . ' To PEO Mapping Report';
	    $this->load->view('report/po_peo_map/po_peo_map_report_table_vw', $data);
	} else {
...
// End of function fetch_po_peo_mapping_details.
    ...
    /* Function is used to generates a PDF file of the po peo mapped report view.
     * @param-
     * @retuns - the PDF file of po peo mapped report details.
     */
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    pdf_create($report, 'po_peo_map_report', 'P');
	    return;
	}
    }
...
// End of function export_pdf.
    ...
    /* Function is used to fetch justification of po to peo mapped list.
     * @param-
     * @retuns -justification data.
     */
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $dept_name.= $this->po_peo_map_report_model->dept_name_by_crclm_id($curriculum_id);
	    ...
	    $crclm = $this->po_peo_map_report_model->fetch_po_peo_mapping_details($curriculum_id);
	    ...
	    //create file name
	    $term_course_file_name = 'PO_to_PEO_Mapped_Report.doc';
	...
	$justification = $this->po_peo_map_report_model->justification_details($curriculum_id);
	if ($justification != null) {
	    $justification_data = '<table class="table table-hover" style="width:100%; overflow:auto;">
                                    <tr>
                                    <td style="border:0;color:blue" width=800 ><b>Overall Justification:</b>
                                    </td>
                                    </tr>
                                    <tr><td class="table-bordered" style="border-left:1px solid #dddddd" width=800>' . $justification . '</tr></td></table><br/>';
    ...
    /* Function is used to fetch individual justification of po to peo mapped list.
     * @param-
	...
	$individual_map = $this->po_peo_map_report_model->individual_map();

	if ($individual_map[0]['indv_mapping_justify_flag'] != 0) {
	    $justify_map = $this->po_peo_map_report_model->justification_data($curriculum_id);

	    $individual_justification = $this->po_peo_map_report_model->individual_justification_details($curriculum_id);
	    $count = count($individual_justification);

	    if ($justify_map != null) {
		$just_data = '<b style="border:0;color:blue" width=800">Individual Mapping Justification:</b>';
		echo $just_data;
	    }

	    for ($i = 0; $i < $count; $i++) {
		if ($individual_justification[$i]['justification'] != null) {
		    $justification_data = '<br><table class="table table-hover" style="width:100%; overflow:auto;">
				                       <tbody>
						       <tr>
							   <td class="table-bordered" width="110" style="border-left:1px solid #dddddd":>' . $individual_justification[$i]['po_reference'] . ' - ' . $individual_justification[$i]['peo_reference'] . '</td>
							   <td class="table-bordered" width="790" gridspan = "10" colspan="10" style="width: 40px;":>' . $individual_justification[$i]['justification'] . '</td>
						       </tr>
						       </tbody>
				                       </table>';
		    echo $justification_data;
