...
class Awards_to_students_model extends CI_Model {
	...
	*/
	 ...
	 */
    public function publication_awards_details($crclm_id){	
		$loggedin_user_id = $this->ion_auth->user()->row()->id;
		...
		if ($this->ion_auth->in_group('admin') || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner')) {
			...
			$course_query =  ' SELECT 	*
								FROM student_awards
								WHERE crclm_id="'.$crclm_id.'"  and user_id= "' . $loggedin_user_id . '"  
								';
			$course_result = $this->db->query($course_query);
			$course_data = $course_result->result_array();
		}
	...
	*/
	public function save_award_publc($data){
		$result = ($this->db->insert('student_awards', $data));	
	...
	*/
	public function delete_publc_award($publication_awards){
		$result = $this->db->query('delete from student_awards where stud_award_id="'.$publication_awards.'"');
		if($result){return '1';}else{return '0';}
	}
	...
	*/
	public function save_res_guid_desc($save_form_details) {
		...
		//var_dump($save_form_details['actual_date'][$i]);
		//var_dump($date);
