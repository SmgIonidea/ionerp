...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {      
                            ...
                            if (typeof settings.columns.editable[i][3] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input inline-input-val ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none; width:90%" disabled>';
                                
                                //added by himansu for getting assessment id
                                var getAttrValue=false;
                                if (settings.columns.editable[i][2]) {
                                    getAttrValue=settings.columns.editable[i][2];
                                } 
                                
                                if(getAttrValue){
                                    var inputVal=$(this).data(getAttrValue);
                                    input += '<input class="tabledit-input ' + settings.inputClass + '" type="hidden" name="_' + settings.columns.editable[i][1] + '" value="' + inputVal + '" style="display: none; width:90%" disabled>';
                                }
        ...
        var Edit = {
            ...
            submit: function(td) {
                ...
                //console.log('edit submit',settings);
                if(Edit.validate_input(td)){
            ...
            validate_input:function(td){
                return settings.elementValidator(td);
            }
        };
        ...
        var Delete = {
            ...
            submit: function(td) {
                ...
                //$(td).parent('tr').find('input.tabledit-identifier').attr('disabled', false);//commented by himansu
                $(td).parent('tr').find('input.tabledit-input,input.tabledit-identifier').prop('disabled', false);//added by himansu
                ...
                $(td).parent('tr').addClass('tabledit-deleted-row btn-danger');
                // Hide table row.
        ...
        if (settings.editButton) {
            ...
            $table.on('click', 'button.tabledit-save-button', function(event) {
                if (event.handled !== true) {
                    ...
                    if(Edit.submit($(this).parents('tr').find('td.tabledit-edit-mode')))
                        event.handled = true;
                }
            });
        } else {
    ...
    function filter_underscore(val,flag){
        if(flag){
