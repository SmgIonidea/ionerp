...
class Dashboard_course_status_model_new extends CI_Model {
...
;
	...
	//$crclm_id, $term_id,$dept_id,     curriculum_id="'.$crclm_id.'" and term_id="'.$term_id.'" and
	public function fetch_topic($crs_id){
		$query='select count(topic_id)  from topic where  course_id="'.$crs_id.'"';
		$query_data=$this->db->query($query);
		return $query_data->result_array();
		
	}
	
	//$crclm_id, $term_id,$dept_id,   t.curriculum_id="'.$crclm_id.'" and t.term_id="'.$term_id.'" and 
	public function fetch_tlo($topic_id,$crs_id){
				$query='select count(t.tlo_id),t1.topic_title  from tlo as t
				join topic as t1 ON t1.topic_id=t.topic_id
				where t.course_id="'.$crs_id.'"and t.topic_id="'.$topic_id.'"';
		$query_data=$this->db->query($query);
		return $query_data->result_array();
	}
	...
	public function fetch_assessment_attainment_data_temp($crclm_id, $term_id, $dept_id){
		$result = $this->db->query("call get_Assessmentstatus_Dashboard($crclm_id,$term_id)");
	...
	public function fetch_terms_course($crclm_id, $term_id,$dept_id){
	
			$query = $this->db->query('select * from crclm_terms where crclm_id="'.$crclm_id.'"');
			return $query->result_array();
	}
	
	public function fetch_crs($crclm_id, $term_id,$dept_id){
	/* 	$query = $this->db->query('select * from course where crclm_term_id = "'.$term_id.'"');
		return $query->result_array(); */
	...
	$course_state_query = 'SELECT c.crs_id, c.crs_code,c.co_crs_owner, u.username,u.title,u.first_name,u.last_name,cco.crs_id,cco.clo_owner_id,c.crs_title, c.state_id 
								...
								AND   c.status = 1 ';
		$course_state_data = $this->db->query($course_state_query);
        return $course_state = $course_state_data->result_array();		
	}
	
	public function fetch_crs_state($crclm_id, $term_id,$dept_id,$course_state_details){
	$state_result_clo='';$count_clo='';$clo_po_map_result='';$tlo_result='';$state_val = '';
	$course_state_query = 'SELECT c.crs_id, c.crs_code,c.co_crs_owner, u.username,u.title,u.first_name,u.last_name,cco.crs_id,cco.clo_owner_id,c.crs_title, c.state_id 
								FROM course AS c
								join course_clo_owner AS cco ON c.crs_id=cco.crs_id 
								join users AS u ON u.id=cco.clo_owner_id
