...
/**
 * Description          :	Model logic for Criterion 2 (Diploma TIER 2) - Teaching Learning Process.
 * Created              :	05-06-2017
...
class T2dip_c2_teaching_process_model extends CI_Model {
    ...
    public function qp_details($crclm_id, $term_id, $crs_id, $qp_type, $qpd_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t2dip_c2_2_2_Question_Paper(' . $qpd_id . ')');
    ...
    /**
     * Function is to fetch term. - section - 2.2.4,3.2.1
     * @parameter   : curriculum id
     * @return      : Array
     */
    ...
    /**
     * Function is to fetch quality student project details. - section 2.2.4
     * @parameter   : curriculum id and  course id
     * @return      : Array
     */
    public function get_quality_student_project($crclm_id = NULL, $course_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t2dip_c2_2_4_COPOMapping(' . $crclm_id . ', ' . $course_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function is to fetch companies visited details.- section 2.2.5
     * @parameter   : program id
     * @return      : Array
     */
    public function fetch_program_companies_visited($pgm_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t2dip_c2_2_5_CompaniesList(' . $pgm_id . ')');
        mysqli_next_result($this->db->conn_id);
        $data['company_details'] = $call_proce->result_array();
        $data['column_list'] = $call_proce->list_fields();
        return $data;
    }

    /**
     * Function is to fetch industry internship details.- section 2.2.5
     * @parameter   : curriculum id
     * @return      : Array
     */
    public function fetch_industry_internship($curriculum = NULL) {
        $call_proce = $this->db->query('CALL nba_t2dip_c2_2_5_IndustryInternship(' . $curriculum . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
...
/*
 * End of file t1dip_c2_teaching_process_model.php
 * Location: .nba_sar/dip/tier1/criterion_2/t1dip_c2_teaching_process_model.php 
 */
?>