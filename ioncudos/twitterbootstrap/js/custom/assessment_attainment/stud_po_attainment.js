...
function select_term() {
	...
	var term_list_path = base_url + 'assessment_attainment/stud_po_level_assessment_data/select_term';
...
function select_type() {
	...
	if (term) {
		...
		$('#stud_usn').html('<option value="0">Select USN</option>');
	}
}
...
function select_usn() {
	...
	if (type_data) {
		$('#stud_usn').html('<option value=0>Select USN</option><option value="usn01" class="cursor_pointer" title="Akshay Patil">14045180</option><option value="usn02" class="cursor_pointer" title="Arun N">14045181</option><option value="usn03" class="cursor_pointer" title="Basavaraj Hiremath" >14045184</option><option value="usn04" class="cursor_pointer" title="Chetan Gubbi" >14045182</option><option value="usn05" class="cursor_pointer" title="Chaitra Shinde" >14045186</option><option value="14045185" class="cursor_pointer" title="OOI CHI YEN" >14045185</option><option value="14045189" class="cursor_pointer" title="TUNG SHINN WEI" >14045189</option><option value="14045206" class="cursor_pointer" title="K.P.KESHUK A/L PURUSHOTHAMAN" >14045206</option><option value="14045208" class="cursor_pointer" title="NEOH JUN WEE" >14045208</option><option value="14045209" class="cursor_pointer" title="ONG LIAN CHIEK" >14045209</option><option value="14045194" class="cursor_pointer" title="TAN CHANG YIN" >14045194</option><option value="14045199" class="cursor_pointer" title="MUHAMMAD ASYRAF BIN MOHAMED NAZIR" >14045199</option><option value="14045202" class="cursor_pointer" title="CHOONG YUAN PIOU" >14045202</option><option value="14045204" class="cursor_pointer" title="YONG YIK FONG" >14045204</option><option value="14045211" class="cursor_pointer" title="EDMUND HILARY NICHOLAS" >14045211</option><option value="14045183" class="cursor_pointer" title="DHARANIDHARAN A/L RAMESH" >14045183</option>');
	} else {
		$('#stud_usn').html('<option value="0">Select USN</option>');
	}
}
...
function display() {
	...
	var term = $('#term').val();
	var type_data = $('#type_data').val();
	var usn = $('#stud_usn').val();
	var stud_name =  $('#stud_usn').find("option:selected").attr("title"); 
	...
	if (crclm_id != '') {
		$.ajax({
			type : "POST",
			url : base_url + 'assessment_attainment/stud_po_level_assessment_data/POThreshholdAttainment',
			data : post_data,
			dataType : 'JSON',
			success : function (json_graph_data) {
				if (json_graph_data.length > 0) {
					...
					$('#po_attainment_no_data').empty();
					$('#po_attainment_nav').html('<b><div class="row-fluid"><div class="span12 rightJustified" style="padding-right:17px;"><div id="stud_details">Student ID: <font color="#b30000">'+usn+'</font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Student Name: <font color="#b30000">'+stud_name+'</font></div></b><a id="exportToPDF" href="#" class="btn btn-success"><i class="icon-book icon-white"></i> Export </a></div></div><br><div class="navbar"><div class="navbar-inner-custom">Student ' + student_outcome + ' Attainment</div></div>');
 {
						var i = 0;
						$.each(json_graph_data, function () {
							...
							data3.push(value3[i]);						
							po_attainment.push(Math.round(Number(this['Attainment'])));
							i++;
						});
						...
						$('#po_attainment > tbody:first').append('<tr><td><center><b>' + so + ' Reference</b></center></td><td><center><b>Max Marks </b></center></td><td><center><b>Secured Marks</b></center></td><td><center><b>Threshold Attainment%</b></center></td><td><center><b>' + so + ' Attainment% </b></center></td></tr>');
						var m = 0;
						$.each(json_graph_data, function () {
							var poAttainment = 0;
							if (Math.round(this['Attainment']) == 0) {
								...
								poAttainment = "" + Math.round(this['Attainment']) + '%<a title="' + so + ' Attainment Drilldown" href="#" id="2" class="myModalQPdisplay displayDrilldown" abbr="' + this['po_id'] + '"><i class="myTagRemover "></i>-</a>';
							}						
						...
						$('#po_attainment_note').html('<br><div class="bs-docs-example"><b>Note:</b><table class="table table-bordered"><tbody><tr><td>The above bar chart depicts the individual ' + student_outcome + ' Attainment for Term.</td></tr><tr><td>Student ' + so + ' Attainment % = (Secured Marks*100) / Max Marks </td></tr></tbody></table></div><br>');
					}
					var ticks = data3;
					var s1 = data1;
					var s2 = data2;
		...
		$.ajax({
			type : "POST",
			url : base_url + 'assessment_attainment/stud_po_level_assessment_data/CourseAttainment',
			data : post_data,
			dataType : 'JSON',
	...
	$('#add_form').on('click', '#exportToPDF', function () {
		...
		var stud_id = $('#stud_usn :selected').text();
		var stud_name = $('#stud_usn :selected').attr('title');
		var po_attainment_graph = $('#chart1').jqplotToImageStr({});
		var po_attainment_graph_image = $('<img/>').attr('src', po_attainment_graph);
		$('#po_attainment_graph_data').html('<table><tr><td><b>Curriculum :</b></td><td>' + crclm_name + '</td></tr><tr><td><b>Term :</b></td><td>' + term_name + '</td></tr><tr><td><b>Assessment Type :</b></td><td>' + occa_name + '</td></tr><tr><td><b>Student ID & Name :</b></td><td>' + stud_id +' - '+ stud_name + '</td></tr></table><br><b>Student ' + student_outcome + ' Attainment</b>');
...
$('.displayDrilldown').live('click', function () {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'assessment_attainment/stud_po_level_assessment_data/CoursePOAttainment',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
...
function select_survey() {
	...
	empty_indirect_attainment_divs();
	var course_list_path = base_url + 'assessment_attainment/stud_po_level_assessment_data/select_survey';
	var data_val = $('#crclm_id').val();
	if (data_val) {
		var post_data = {
			'crclm_id' : data_val
...
function plot_graphs_comparison_attainment(crclm_id, term_id, direct_attainment_val, indirect_attainment_val, type_data, survey_id_arr, survey_perc_arr, po_attainment_type, core_courses_cbk) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'assessment_attainment/stud_po_level_assessment_data/getDirectIndirectPOAttaintmentData',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
...
$(document).ready(function () {
	...
	$('.add_survery_field').live('click', function (e) {
		...
		if (max_rows_for_survey == counter_val) {
			...
			$.ajax({
				type : "POST",
				url : base_url + 'assessment_attainment/stud_po_level_assessment_data/add_more_rows',
				data : count_val,
				async : false,
