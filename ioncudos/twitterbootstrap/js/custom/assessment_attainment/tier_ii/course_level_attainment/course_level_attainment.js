...
$('#section_finalize_status_tbl').empty();$('#mte_finalize_status_tbl').empty();
...
function select_pgm() {
	...
	empty_all_divs();
	var pgm_list_path = base_url + 'tier_ii/course_clo_attainment/select_pgm';
...
function select_crclm() {
	...
	var crclm_list_path = base_url + 'tier_ii/course_clo_attainment/select_crclm';
...
function select_term() {
	...
	empty_all_divs();
	var term_list_path = base_url + 'tier_ii/course_clo_attainment/select_term';	
	var student_list_path = base_url + 'tier_ii/course_clo_attainment/student_list';
...
function select_course() {
	...
	empty_all_divs();
	$('#section_finalize_status_tbl').empty();	$('#mte_finalize_status_tbl').empty();
	var course_list_path = base_url + 'tier_ii/course_clo_attainment/select_course';
...
$('#course').on('change',function(){
	...
	var post_data =  {'course_id':course_id,'crclm_id':crclm_id,'term_id':term_id};
	 ...
	 $('.toggle_doc_button').attr('id');

	if(course_id){
	$('#export_doc').prop('disabled', false);
		$.ajax({
            type : "POST",
            url : base_url + 'tier_ii/course_clo_attainment/fetch_section_data',
            data : post_data,
            dataType: 'json',
            success : function (msg) {			
							console.log(msg);
							...
							$('#section_finalize_status_tbl').append(msg.cia_redirect);
							$('#mte_finalize_status_tbl').empty();
                            $('#mte_finalize_status_tbl').html(msg.mte_wise_table); 
							$('#mte_finalize_status_tbl').append(msg.mte_redirect);
                            $('#cla_data_table').empty();
                           	$('#cla_data_table').html(msg.cla_data_table_cia);
							$('#cla_data_table_mte').empty();
                           	$('#cla_data_table_mte').html(msg.cla_data_table_mte);
                            $('#select_occa_type').prop('selectedIndex',0);	
							id = $("li.toggle_doc_button.active").attr('id');
							if(id =='tab2'){ 								
									type_val = $('#select_occa_type').val();
									if(type_val == ''){ $('#export_doc').prop('disabled', true);} 
									else{ $('#export_doc').prop('disabled', false); } 
							}else if(id == 'tab1'){$('#export_doc').prop('disabled', false);} 											
                            $('#all_section_attainment').empty();
                            $('#attainment_finalize_button_div').hide();
                            $('#all_section_co_attainment_chart').empty();
                            $('#all_section_attainment').empty();
							$('#occasion_wise_attainment_navbar_all_attainment').hide();
                            $('#all_section_co_attainment_chart').empty();
                            $("#all_section_co_attainment_chart").css("height",0);
							$('#occasion_wise_attainment_doc2').hide();$('#error_msg').html('');
							 $('.targets').hide();$('#ocaasion_type_navbar').empty(); $('#occasion_wise_attainment_navbar').hide();
							$("#note_data_both").hide();
                            $('#note_data').html('<div class="span12"><table border="1" class="table table-bordered" style="width:100%"><tbody><tr><td width= 650 colspan="2"><b>Note:</b> The above bar graph depicts the overall class performance with respect to the Threshold % for individual '+ entity_clo_full_singular +' ('+ entity_clo +'). The Threshold based Attainment % & Average based Attainment % is calculated using the below formula.</td></tr><tr><td width= 250 ><b>For Threshold based Attainment % = ( x / y ) * 100 </b><br/> x = Count of Students &gt;= to Threshold % <br/> y = Total number of Students Attempted .</td><td width= 250 ><b>For Average based Attainment % = ( x / y ) *100 </b> <br/> x = Average Secured marks of Attempted Students <br/> y = Maximum Marks . </td></tr></tbody></table></div>');     

							$('#note_data_mte').html('<div class="span12"><table border="1" class="table table-bordered" style="width:100%"><tbody><tr><td width= 650 colspan="2"><b>Note:</b> The above bar graph depicts the overall class performance with respect to the Threshold % for individual '+ entity_clo_full_singular +' ('+ entity_clo +'). The Threshold based Attainment % & Average based Attainment % is calculated using the below formula.</td></tr><tr><td width= 250 ><b>For Threshold based Attainment % = ( x / y ) * 100 </b><br/> x = Count of Students &gt;= to Threshold % <br/> y = Total number of Students Attempted .</td><td width= 250 ><b>For Average based Attainment % = ( x / y ) *100 </b> <br/> x = Average Secured marks of Attempted Students <br/> y = Maximum Marks . </td></tr></tbody></table></div>');
                            ...
                            if(msg.finalized_data_tbl != 'false'){
								...
								$('#cia_tee_attainment_navbar_cia').show();
                               // $('#cia_tee_attainment_navbar_cia').empty();
                                $('#cia_tee_attainment_navbar_cia').html('<div class="navbar" ><div class="navbar-inner-custom"> Overall '+  entity_clo_full + ' ' + entity_clo + ' Attainments are Finalized </div></div>');

                            }else{
                                $('#display_finalized_attainment_div').hide();
                                $('#display_finalized_attainment_tbl').empty();
                            }
                            console.log(msg.po_attainment_tbl);
                            if(msg.po_attainment_tbl != 'false'){
                                ...
                                $('#display_finalized_po_attainment_div').hide();
                                $('#display_finalized_po_attainment_tbl').empty();
								$('#display_finalized_po_mappingh_attainment_div').hide();
                                $('#display_finalized_po_attainment_mapping_tbl').empty();	
								$('#display_map_level_weightage_div').hide();
                                $('#display_map_level_weightage').empty();
								$('.Finalized_div').hide();
								$('.Finalized_div').empty();
                            }
	 ...
	 //  $('#export_doc').prop('disabled', true);
	  $('.po_all_note').hide();
        ...
        $('#attainment_finalize_button_div').hide();
        $('#all_section_co_attainment_chart').empty();
		$('#occasion_wise_attainment_doc2').empty();
		$('#error_msg').html('');
		$('#section_finalize_status_tbl').empty();
		$('#section_finalize_status_tbl').html("<span style='color:red; left-padding:10px;'> No Data To Display. </span>");	
		$('#mte_finalize_status_tbl').empty();
		$('#mte_finalize_status_tbl').html("<span style='color:red; left-padding:10px;'> No Data To Display. </span>");
		$('#display_finalized_po_attainment_div').hide();
		$('#display_finalized_attainment_div').hide();
		$('#display_finalized_po_mappingh_attainment_div').hide();
		$('#display_map_level_weightage_div').hide();
		$('#cla_data_table_cia').hide();
		$('#cla_data_table_tee').hide();$("#note_data_both").hide();
		 $('.targets').hide();$('#ocaasion_type_navbar').empty(); $('#occasion_wise_attainment_navbar').hide();
  }
...
$(document).on('click','.attainment_drill_down',function(){
   ...
   $.ajax({
            type : "POST",
            url : base_url + 'tier_ii/course_clo_attainment/section_attainment_drill_down',
            data : post_data,
            dataType: 'json',
...
$('.bloom_navabar_header').show();
	...
	$('#display_bloom_data').hide();
	  ...
	  $.ajax({
			type : "POST",
			url : base_url + 'tier_ii/course_clo_attainment/fetch_sections',
			data : post_data,
			success : function (msg) {
					 $('#tier1_section').html(msg);
					 if ($.cookie('remember_course') != null) {
					// set the option to selected that corresponds to what the cookie is set to
					$('#tier1_section option[value="' + $.cookie('remember_section') + '"]').prop('selected', true);  console.log(student_usn);
...
/* 		$.ajax({
			...
			dataType : 'JSON',
			success : function (msg) {
				if(msg.table){$('#display_student_data').html(msg.table);
					 $('#loading').hide();
				}else{ $('#display_student_data').html(''); }
			}
		}); */
		}  else{ 
	$('#loading').hide();}
...
$('#tier1_section').on('change' , function(){
		...
		var post_data = {'type_val':type_val, 'course_id':course_id, 'crclm_id': crclm_id , 'term_id':term_id , 'section_id':section_id};
		  $.ajax({
			type : "POST",
			url : base_url + 'tier_ii/course_clo_attainment/fetch_occasions',
			data : post_data,
			success : function (msg) {
					 $('#occasion').html(msg);
					 $('#student_list').val(''); 
...
$('#occasion , #student_list').on('change' , function(){
	$('#loading').show();
$('#example_bloom').show();
 	$('#diplay_bloom_data_chart').show();
		$("#diplay_bloom_data_chart").css("height",350);
	...
	$('#example_bloom').show();
		...
		$('#example_bloom').dataTable(
				...
				"sEmptyTable" : "Your custom message for empty table",
						...
						{"sTitle": "Threshhold","mData":"percent"}, 			
...
$('#select_occa_type').on('change',function(){
   ...
   if(crclm_id && term_id && crs_id && type_val != null){
	
	if(type_val == ''){ $('#occasion_wise_attainment_div').hide(); $('.targets').hide();$('#ocaasion_type_navbar').empty(); $('#occasion_wise_attainment_navbar').hide(); $('#export_doc').prop('disabled', true);}
	if(type_val){
		$('#export_doc').prop('disabled', false);
		$('#occasion_wise_attainment_div').show(); 
			...
			$('#all_section_attainment').empty();
			$('#note_data_both').empty();
			$('#note_data_both').html('');		
			$('#export_doc').prop('disabled', true);
			$('#occasion_wise_attainment_doc2').hide();
			$('#error_msg').html('');
	}
		
	$('.toggle_doc_button').trigger('change');
	
   $.ajax({
			type : "POST",
			url : base_url + 'tier_ii/course_clo_attainment/show_section_co_attainment_multiple',
			data : post_data,
                        dataType: 'json',
                        success : function (msg) {
						    $('#loading').hide();	
                           		if(msg.all_section_table != 'empty'){
							$('#note_data_both').empty();
							$('#note_data_both').html('<div class="span12 table-bordered note" style="border:1"><b>Note:</b><table border="1" class="table table-bordered"><tbody><tr><td>The above bar graph depicts the overall class performance with respect to the Min Threshold % for individual '+ entity_clo_full_singular +' ('+ entity_clo +') and the Students &gt;= Min Threshold % (Target) for individual '+ entity_clo_full_singular +' ('+ entity_clo +') as in the question paper. The Students &gt;= Min Threshold % is calculated using the below formula.</td></tr><tr><td>For Students &gt;= Min Threshold % = (Count of Students &gt;= to Min Attainment Threshold % / Total number of Students Attempted) x 100</td></tr></tbody></table></div>');	
						//	if(msg.all_section_table == '')
							$('#occasion_wise_attainment_doc2').show();
							$('#error_msg').html(msg.msg);
										$('#cla_data_table_cia_only_div').show();			
										$('#cla_data_table_cia_only').empty();
										$('#cla_data_table_cia_only').html(msg.cla_data_table_cia_only);
										$('#all_section_attainment').html(msg.all_section_table);
										$('#cla_data_table_cia_only_div').show();
										$('#occasion_wise_attainment_navbar_all_attainment_all').show();					
								...
								$('#attainment_level').val(msg.attainment_level);
                                ...
                                // plot attainment graph.
                                            ...
                                            $('#all_section_co_attainment_chart').empty();
                                var plot1 = $.jqplot('all_section_co_attainment_chart', [msg.threshold_array], {
			...
			if(type_not_selected == 0 ){$('#attainment_finalize_button_div').show();}else{$('#attainment_finalize_button_div').hide();}
			}else{
...
$(document).on('click','.cia_tee_drill_down',function(){
  ...
  $.ajax({
			type : "POST",
			url : base_url + 'tier_ii/course_clo_attainment/show_cia_tee_drilldown',
			data : post_data,
                        dataType: 'json',
                        success : function (msg) {
							...
							$('#weightage_display').html('<div class="span6"><center><b>'+ entity_cie+' Weightage : </b>'+ msg.cia_weightage+'</center></div><div class="span6"><center><b>'+ entity_tee +' Weightage : </b> ' + msg.tee_weightage+ '</center></div></div>');
...
$('#tee_cia_attainment_finalize_btn').on('click' , function(){
		$('#attainment_finalize').modal('show');
});
...
$('#finalize_clo_attainment').on('click',function(){
    ...
    var attainment_level =  $('#attainment_level').val();
    ...
    var post_data = {'crclm_id':crclm_id, 'term_id':term_id, 'crs_id':crs_id, 'clo_ids':clo_ids,
                        ...
                        'co_threshold':co_threshold,'attainment_level' : attainment_level,
                    };
   $.ajax({
            type : "POST",
            url : base_url + 'tier_ii/course_clo_attainment/finalize_co_overall_attainment',
            data : post_data,
            dataType: 'json',
            success : function (msg) {  
                        if($.trim(msg) == 'true'){
                   $('#attainment_finalize').modal('hide');
...
$('.cancel_button').on('click', function(){
	window.location.href = base_url + 'tier_ii/po_level_attainment';
});
...
$('#add_form').on('change', '#type_data', function () {
	...
	if (type_data_id == 1) {
		...
		$.ajax({
			type : "POST",
			url : base_url + 'tier_ii/course_clo_attainment/select_occasion',
			data : post_data1,
...
function plot_graphs(course, qpd_id, type, section_id) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/getCourseCOThreshholdAttainment',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
				...
				$('#co_level_nav').html('<div class="row-fluid"><div class="span6"></div><div class="span6 rightJustified" ><a id="exportToPDF" href="#" class="btn btn-success"><i class="icon-book icon-white"></i> Export </a></div></div><br><div class="navbar"><div class="navbar-inner-custom">'+ entity_clo_full_singular +'('+ entity_clo +') Attainment</div></div>');
				...
				$('#co_level_table_id > tbody:first').append('<tr><td><center><b>'+ entity_clo_full_singular +'</b></center></td><td><center><b>Max Marks</b></center></td><td style="white-space:no-wrap;"><center><b>Average of Secured Marks</b></center></td><td><center><b>Threshold %</b></center></td><td><center><b>Attainment %</b></center></td></tr>');
				$.each(json_graph_data, function () {
	...
	$.ajax({
		...
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
				...
				$('#co_level_student_threshold_nav').html('<div class="row-fluid"><div class="span6"></div></div><br><div class="navbar"><div class="navbar-inner-custom">'+ entity_clo_full_singular +'('+ entity_clo +') wise Class Performance % for Students above or equal to % of Threshold (Target)</div></div>');
				...
				$('#co_level_student_threshold_nav_table_id > tbody:first').append('<tr><td><center><b>'+ entity_clo_full_singular +'</b></center></td><td><center><b>% of Threshold</b></center></td><td><center><b>% of Student Threshold</b></center></td><td style="white-space:no-wrap;"><center><b>Students Attainment % </b></center></td></tr>');
				$.each(json_graph_data, function () {
	...
	/*	$.ajax({type: "POST",
	url: base_url + 'tier_ii/course_clo_attainment/getCourseCOAttainment',
	data: post_data,
	dataType: 'JSON',
	success: function(json_graph_data) {
	if (json_graph_data.length > 0) {
...
{label:'Actual Contribution %'},
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/CourseBloomsLevelThresholdAttainment',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/CourseBloomsLevelAttainment',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
			if (json_graph_data.length > 0) {
	...
	/*	var po;
	$.ajax({type: "POST",
	url: base_url + 'tier_ii/course_clo_attainment/CoursePOCOAttainment',
	data: post_data,
	dataType: 'JSON',
	success: function(json_graph_data) {
	$('#CLO_PO_attainment_nav').empty();
...
$('#add_form').on('click', '#exportToPDF', function () {
	...
	$('#course_outcome_attainment_graph_data').html('<table><tr><td><b>Curriculum :</b></td><td>' + crclm_name + '</td></tr><tr><td><b>Term :</b></td><td>' + term_name + '</td></tr><td><b>Course :</b></td><td>' + course_name + '</td></tr></table><br><b>'+ entity_clo_full_singular +'('+ entity_clo +') Attainment</b><br />');
...
function select_survey() {
	...
	empty_indirect_attainment_divs();
	var course_list_path = base_url + 'tier_ii/course_clo_attainment/select_survey';
...
$('#indirect_attainment_form').on('change', '#survey_id', function () {
	...
	if (survey_id != '' || survey_id != 0 ) {
		...
		var survey_link = base_url +'survey/surveys';
				$('#graph_val').html("<br/><center><span style='color:red;'>There are no Surveys found under this Course. Create the survey to view '+ entity_clo_full_singular +' ('+ entity_clo +') Indirect Attainment.<br\> Use this link to create the survey : <a href="+survey_link+"> </span>survey create link</a></center>");
	}
});
function plot_indirect_attainment_graph(survey_id, crs_id) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/get_indirect_attainment_data',
		data : post_data,
		success : function (attainment_data) {
			$('#co_level_indirect_nav').html('<div class="row-fluid"><div class="span6"></div><div class="span6 rightJustified" ><a id="exportToPDF" href="#" class="btn btn-success"><i class="icon-book icon-white"></i> Export </a></div></div><br><div class="navbar"><div class="navbar-inner-custom">'+ entity_clo_full_singular +'('+ entity_clo +') Indirect Attainment</div></div>');
			...
			var survey_link = base_url +'survey/host_survey';
				$('#graph_val').html("<center><span style='color:red;'>The selected survey is still <b>In-Progress</b> status. Change the survey status to <b>Closed</b> to view '+ entity_clo_full_singular +' ('+ entity_clo +') Indirect Attainment.<br\> Use this link to close the hosted survey : <a href="+survey_link+"> </span>survey close link</a></center>");
			}
		} //end of success
	});
}
...
var course_list_path = base_url + 'tier_ii/course_clo_attainment/getDirectIndirectCOAttaintmentData';
...
function show_finalized_co_grid() {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/course_clo_attainment/get_finalized_co_attainment_data',
        data: post_data,
        dataType: 'HTML',
...
$('#finalize_direct_indirect_confrim').on('click', function (e) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/finalize_getDirectIndirectCOAttaintmentData',
		data : post_data,
		dataType : 'JSON',
		success : function (msg) {
			if (msg != 0) {
...
function plot_graphs_comparison_attainment(survey_id, course_id, direct_attainment_val, indirect_attainment_val, type_data, occasion) {
	...
	$.ajax({
		type : "POST",
		url : base_url + 'tier_ii/course_clo_attainment/getDirectIndirectCOAttaintmentData',
		data : post_data,
		dataType : 'JSON',
		success : function (json_graph_data) {
...
$('#indirect_attainment_form').on('click', '#exportToPDF', function () {
	...
	var course_attainment_graph_image = $('<img/>').attr('src', course_attainment_graph);
	$('#course_outcome_indirect_attainment_graph_data').html('<table><tr><td><b>Curriculum :</b></td><td>' + crclm_name + '</td></tr><tr><td><b>Term :</b></td><td>' + term_name + '</td></tr><td><b>Course :</b></td><td>' + course_name + '</td></tr></table><br><b>'+ entity_clo_full_singular +'('+ entity_clo +') Indirect Attainment</b><br />');
...
$('#indirect_direct_attainment_form').on('click', '#exportToPDF', function () {
	...
	var course_attainment_graph_image = $('<img/>').attr('src', course_attainment_graph);
	$('#direct_indirect_attain_data').html('<table><tr><td><b>Curriculum :</b></td><td>' + crclm_name + '</td></tr><tr><td><b>Term :</b></td><td>' + term_name + '</td></tr><td><b>Course :</b></td><td>' + course_name + '</td></tr></table><br><b>'+ entity_clo_full_singular +''+ entity_clo +' Direct and Indirect Attainment Analysis</b><br />');
...
$('form[name="cia_co_attainment_form"]').on('click', '#export_doc', function(e){
	tab_id  = $("ul#myTab li.active").attr('id');
	$( ".navbar-inner-custom" ).wrapInner( "<b></b>" );
    e.preventDefault();	
	if(tab_id == 'tab1'){	
		$('#form_name').val('cia_co_attainment_form');
		filename = entity_cie+'-'+ entity_clo +'Attainment(Section/Devision_wise)';
		$('#file_name').val(filename); 	
		export_data = $('#finalised_data').html(); 
		$('#export_data_to_doc').val(export_data);	
		
    }else if(tab_id == 'tab4'){
	
		$('#form_name').val('cia_co_attainment_form');
		filename = entity_mte+'-'+ entity_clo +'Attainment(Section/Devision_wise)';
		$('#file_name').val(filename); 	
		export_data = $('#finalised_data_mte').html(); 
		$('#export_data_to_doc').val(export_data);
		
	}else if(tab_id = 'tab2' && $('#select_occa_type').val() != ''){

		var imgData_1 = $('#all_section_co_attainment_chart').jqplotToImageStr({});
	
		$('#form_name').val('cia_co_attainment_form');
		if($('#select_occa_type').val() == 'cia_tee'){
		var export_data = $('#occasion_wise_attainment_doc1').clone().html() + $('#all_section_attainment').clone().html() +  $('#finalised_doc_export_div').clone().html();
		}else{
		var export_data = $('#occasion_wise_attainment_doc2').clone().html() + $('#finalised_doc_export_div').clone().html();
		}
		
		filename = 'Finalize_Course-'+ entity_clo +'Attainment';
		$('#export_data_to_doc').val(export_data);
		$('#file_name').val(filename);
