...
$('#loading').show();
		...
		$.ajax({type: "POST",
			url: base_url+'nba_sar/awards_to_students/select_pgm_list',
			data: post_data,
			success: function(msg) {
				...
				if ($.cookie('remember_pgm') != null) {
					...
					select_crclm_list();$('#loading').hide();
				}
			}
		});
	}
	...
	$('#loading').show();
		...
		$.ajax({type: "POST",
			url: base_url+'nba_sar/awards_to_students/select_crclm_list',
			data: post_data,
			success: function(msg) {
				$('#crclm_id').html(msg);
	...
	$('#loading').show();
		...
		$.ajax({type: "POST",
			url: base_url+'nba_sar/awards_to_students/select_termlist',
			data: post_data,
			success: function(msg) {
				$('#term').html(msg);
				if ($.cookie('remember_term') != null) {
					// set the option to selected that corresponds to what the cookie is set to
					$('#term option[value="' + $.cookie('remember_term') + '"]').prop('selected', true);GetSelectedValue();
					$('#loading').hide();
...
/* 	function select_courselist() {
		...
		var course_list_path = base_url + 'nba_sar/awards_to_students/select_course';
		var crclm_id = $('#crclm_id').val();
	...
	$('#award_date_btn').click(function(){
		$(document).ready(function(){
			$('#award_date').datepicker().focus();
		});
	});	
		...
		//	$("#awards_to_students").validate();
		//	var flag = $('#awards_to_students').valid();  
		//$('#loading').show();
		 	$("#awards_to_students").validate({
			rules: {
				  award_date:{
				...
				errorPlacement : function(error, element) {
					if (element.attr('name') == "award_date") {
						error.appendTo('#error_award_date_btn');
					} else {
		...
		//	var course  = $('#course').val();
			...
			if(idClicked == 'update_award_publc'){ buttonvalue = 'update'} else if( idClicked == 'save_award_publc'){ buttonvalue = 'save' }
			   var values = $("#awards_to_students").serialize();
				values = values + '&button_update='+buttonvalue;
				values = values + '&abstracts='+abstract_description;
			...
			//values = values + '&term='+term;
			   ...
			   $.ajax({type:"POST",
						url:base_url+'nba_sar/awards_to_students/save_update_awards_to_students',
						data:values,
						dataType:'json',
						success:function(data){$('#loading').hide();$('#update_award_publc').hide();$('#save_award_publc').show();GetSelectedValue();
						reset_awards_to_students();	if(data == 1){success_modal(data);}if(data == 0){fail_modal(data);}}}); 
			}else{$('#loading').hide();} 
			}else{
					$('#Exist').modal('show');
			}
		});
	function reset_awards_to_students(){
		$("#awards_to_students").trigger('reset');
		var validator = $('#awards_to_students').validate();
		validator.resetForm();	
		$('#update_award_publc').hide();$('#save_award_publc').show();
	}	
		//Function to display the publications and awards table on select of curriculum
	function GetSelectedValue() {
		var publications_awards_path = base_url + 'nba_sar/awards_to_students/show_publications_awards';
	...
	*/	
	 ...
	 //$('#example').empty();
		$('#example').dataTable(
				{	
					"sPaginate": true,	
					"aoColumns": [
						{"sTitle": "Sl No.", "mData": "sl_no" ,"sType": 'numeric',"bSortable" : true, "sSortDataType": "dom-text"},
						{"sTitle": "Award for", "mData": "award_for"},
						{"sTitle": "Participant(s)", "mData": "participants"},
						{"sTitle": "Sponsored By", "mData": "sponsored_by"},
						{"sTitle": "Venue", "mData": "venue"},
						{"sTitle": "Date", "mData": "date"},
						{"sTitle": "Uplaod", "mData": "upload"},
	...
	$('#stud_award_id').val($(this).attr('id'));
	$('#award_recieved_for').val($(this).attr('data-award_for'));
	$('#participants').val($(this).attr('data-participants'));	
	$('#sponsored_by').val($(this).attr('data-sponsored_by'));
	$('#award_venue').val($(this).attr('data-venue'));	
	$('#award_date').val($(this).attr('data-date'));
	tiny_init();	
		tinyMCE.get('award_abstract').setContent($(this).attr('data-abstract'));
	
			$('#update_award_publc').show();$('#save_award_publc').hide();
	}); 
	...
	$('#loading').show();
	var stud_award_id = $('#stud_award_id').val();
	 var post_data ={'stud_award_id':stud_award_id}
		   $.ajax({type:"POST",
					url:base_url+'nba_sar/awards_to_students/delete_publc_award',
					data:post_data,					
					success:function(data){ 
					if(data == 1){ }success_modal_delete();
					$('#loading').hide();GetSelectedValue();
						$('#update_award_publc').hide();$('#save_award_publc').show();
					reset_awards_to_students();}}); 
	});	
					...
					var post_data = {
 					...
 					var uploader = document.getElementById('uploaded_file');	 
						upclick({
							...
							multiple: true,
							action: base_url+'nba_sar/awards_to_students/upload',
							oncomplete:
	...
	function fetch_file_data(per_table_id){
					...
					post_data =  {'per_table_id':per_table_id}
					$.ajax({type: 'POST',
					url: base_url+'nba_sar/awards_to_students/fetch_files',
					data: post_data,
					success: function(data) {
	...
	$('#delete_uploaded_files').on('click',function(){
			...
			var per_table_id = $('#uload_id').val();
					...
					post_data =  {'per_table_id':per_table_id}
					$.ajax({type: 'POST',
					url: base_url+'nba_sar/awards_to_students/delete_uploaded_files',
					data: post_data,
					success: function(data) {
		...
		//check whether file any file exists or not
		 if(form_val.length > 0) {
			...
			$.ajax({
				  type: "POST",
				  url: base_url+'nba_sar/awards_to_students/save_res_guid_desc',
				   data : form_data,
				   contentType : false,
				   cache : false,
				   processData : false,
