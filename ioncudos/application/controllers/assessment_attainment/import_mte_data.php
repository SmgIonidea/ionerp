...
class Import_mte_data extends CI_Controller {
	
	function __construct() {
        // Call the Model constructor
        parent::__construct();
		$this->load->model('assessment_attainment/import_mte_data/import_mte_data_model');
	...
	$mte_lang = $this->lang->line('entity_mte');
        if (!$this->ion_auth->logged_in()) {
            ...
            $dept_list = $this->import_mte_data_model->dept_fill();
            $data['dept_data'] = $dept_list['dept_result'];
            $data['title'] = 'Import '.$mte_lang.' List Page';
            $this->load->view('assessment_attainment/import_mte_data/import_mte_data_vw', $data);
        }
    }
	...
	*/
    public function select_pgm_list() {
		if (!$this->ion_auth->logged_in()) {
			...
			$pgm_data = $this->import_mte_data_model->pgm_fill($dept_id);
	...
	*/
    public function select_crclm_list() {
		if (!$this->ion_auth->logged_in()) {
			...
			$curriculum_data = $this->import_mte_data_model->crclm_fill($pgm_id);
	...
	*/
    public function select_termlist() {
		if (!$this->ion_auth->logged_in()) {
			...
			$term_data = $this->import_mte_data_model->term_fill($crclm_id);
	...
	*/
    public function show_course() {
		if (!$this->ion_auth->logged_in()) {
			...
			$course_data = $this->import_mte_data_model->course_list($crclm_id, $term_id);
			$mte_flag = $this->import_mte_data_model->check_mte_flag();
			...
			$crs_list = array();
			$cia_lang = $this->lang->line('entity_mte');
			...
			foreach ($data as $crs_data) {
				...
				$section  = 0;
				$manage_mte = '<a title = "Add/Modify '.$cia_lang.' Marks" href = "'.base_url().'assessment_attainment/import_mte_data/manage_mte_marks/'.$dept_id.'/'.$prog_id.'/'.$crclm_id.'/'.$term_id.'/'.$crs_data['crs_id'].'/'.$section.'" class = "myTagRemover get_crs_id" id = "' . $crs_data['crs_id'] . '" > Add/Modify '.$cia_lang.' Marks  </a>'; 
				...
				$crs_list[] = array(
									...
									'cia_details' => '<a data-toggle="modal" href="#myModal_cia_details" class="get_cia_details" id="' . $crs_data['crs_id'] . '" data-mte_flag = "'. $mte_flag.'" >'.$this->lang->line('entity_mte').' Occasions </a>',
									'manage_cia' => $manage_mte,
									'publish' => $publish
								);
				$i++;
			}
	...
	/**/
	public function manage_mte_marks($dept_id, $prog_id, $crclm_id, $term_id, $crs_id, $section_id) {
		if (!$this->ion_auth->logged_in()) {
            //redirect them to the login page
            redirect('login', 'refresh');
        } elseif (!($this->ion_auth->is_admin() || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner') || $this->ion_auth->in_group('Course Owner'))) {   
            //redirect them to the home page because they must be an administrator or owner to view this
            redirect('curriculum/peo/blank', 'refresh');
        } else {	$cia_lang = $this->lang->line('entity_mte'); 
			$result = $this->import_mte_data_model->fetch_cia_marks($dept_id, $prog_id, $crclm_id, $term_id, $crs_id, $section_id);
			$data['cia_data'] = $result['mte_result'];
			//$data['mte_data'] = $result['mte_result'];
...
//			$data['section_name'] = $result['section_name']['section_name'];
			...
			$this->load->view('assessment_attainment/import_mte_data/add_mte_marks_vw', $data);
		}
	}
	...
	public function update_cia_marks() {
		if (!$this->ion_auth->logged_in()) {
			...
			$result = $this->import_mte_data_model->update_cia_marks($ao_id, $avg_cia_marks);
    ...
    public function get_cia_details() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $result = $this->import_mte_data_model->get_cia_details($crs_id,$section_id);           
			$output = '<table class="table table-bordered">';
            $cia_data = $result['cia_result'];
            $crs_data = $result['crs_data'];
			$i = 0;
			$output.=" <div class='navbar' style='width:450px'>
							<div class='navbar-inner-custom'>
								".$this->lang->line('entity_cie_full') ."Occasions List
							</div>
						</div>";
            $output.="<tr><td colspan='8'><b>".$this->lang->line('entity_cie_full') ." (". $this->lang->line('entity_mte').") </b><b>   Course : </b>" . $crs_data[0]['crs_title'] . " (" .$crs_data[0]['crs_code']. ")"."</td></tr>";
            ...
            //$output.="<th><b>".$this->lang->line('entity_mte')." weightage in %</b></td></th>";
            $output.="<th><b>".$this->lang->line('entity_mte')." Max Marks</b></td></th>";
            $output.="<th><b>View QP</b></td></th>";
			if(empty($cia_data)) {
				$output.="<tr><td>1. </td>";
				$output.="<td colspan='7'><center><font color='red'>Continuous Internal Assessment (". $this->lang->line('entity_mte') .") Occasions are not defined</td></center></tr>";
				echo $output;
			} else {
				foreach ($cia_data as $items) {
					$i++;
					if ($items['qpd_id'] == NULL) {
						$avg_marks = '<font color="blue"> <p class="pull-right">'.$items['avg_cia_marks'].'</font>';
					} else {
						...
						$avg_marks = '<center><font color="blue"> QP is defined </font> <a class="cursor_pointer view_qp" id='.$items['crs_id'].'/'.$items['qpd_id'].' abbr="'.$pgm_id.'/'.$items['crclm_id'].'/'.$items['term_id'].'/'.$items['crs_id'].'/3/'.$items['qpd_id'].'">View QP</a></center>';
                                                ...
                                                $avg_marks = '<center><font color="blue"> QP is defined </font> <a class="cursor_pointer view_rubrics" id='.$items['crs_id'].'/'.$items['qpd_id'].' data-ao_id = "'.$items['ao_id'].'"'
     ...
     * @return: an object
     */
    public function get_mte_details() {
        if (!$this->ion_auth->logged_in()) {
            //redirect them to the login page
            redirect('login', 'refresh');
        } elseif (!($this->ion_auth->is_admin() || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner') || $this->ion_auth->in_group('Course Owner'))) {   
            ...
            $term_id = $this->input->post('term_id');
            $section_id = $this->input->post('section_id');
            $result = $this->import_mte_data_model->get_mte_details($crs_id,$section_id);           
			$output = '<table class="table table-bordered">';
            $cia_data = $result['cia_result'];
            $crs_data = $result['crs_data'];
			$i = 0;
            $output.="<tr><td colspan='8'><b>".$this->lang->line('entity_mte_full') ." (". $this->lang->line('entity_mte').") </b><b>   Course : </b>" . $crs_data[0]['crs_title'] . " (" .$crs_data[0]['crs_code']. ")"."</td></tr>";
            $output.="<th><b>Sl. No.</b></td></th>";
            $output.="<th><b>AO Description</b></td></th>";
          //  $output.="<th><b>AO Method</b></td></th>";
            $output.="<th><b>Assessment Type</b></td></th>";
    ...
    public function get_organisation_type(){
        ...
        $crs_id = $this->input->post('crs_id');
            ...
            $get_organisation_type = $this->import_mte_data_model->get_organisation_type($crclm_id,$term_id,$crs_id); 
    ...
    public function check_student_uploaded_or_not(){
       ...
       $check_students = $this->import_mte_data_model->check_student_uploaded_or_not($crclm_id);
	   $check_main_question = $this->import_mte_data_model->check_main_question($qpd_id);
	   ...
	   $error_msg .= 'Questions are not defined for this Question Paper. <br/><p>Click here to <a  href="'. base_url()."question_paper/manage_mte_qp".'" class="cursor_pointer" > Define Question(s).</a> </p>'; $flag = 0;
	   }
...
public function pdf_report_rubrics_list_table($ao_method_id,$ao_id){
    $get_rubrics_details = $this->import_mte_data_model->get_saved_rubrics($ao_method_id);
    $check_qp_created = $this->import_mte_data_model->check_question_paper_created($ao_id); // Check Question question paper is created or not
    $meta_data = $this->import_mte_data_model->pdf_report_meta_data($ao_id); // Check Question question paper is created or not
