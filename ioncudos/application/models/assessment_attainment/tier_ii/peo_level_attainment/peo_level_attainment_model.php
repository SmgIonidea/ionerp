...
class Peo_level_attainment_model extends CI_Model {
    /*
 ...
 $loggedin_user_id = $this->ion_auth->user()->row()->id;
	...
	$result = $this->db->query('SELECT pm.po_id, p.peo_id, p.peo_reference, p.peo_statement,po.po_reference, po.po_statement, t.overall_attainment as attainment_perc,t.attainment_level as attainment_level
				    FROM po po, peo p,po_peo_map pm
				    JOIN tier_ii_po_overall_attainment t ON t.po_id = pm.po_id
				    WHERE p.crclm_id= "' . $crclm_id . '" and p.peo_id=pm.peo_id and po.po_id = pm.po_id and pm.map_level >= 1 and pm.peo_id = "' . $peo_id . '" ');
	return $result->result_array();
    }
