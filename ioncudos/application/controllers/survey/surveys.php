...
class Surveys extends CI_Controller {
    ...
    function index() {
        if ($this->input->is_ajax_request()) {
            ...
            if ($flag == 'program') {
                ...
                if (array_key_exists('term_id', $postData)) {
                ...
                if ($programId == '' && $crclm_id == '' && $term_id == '' && $su_for_id == '' && $departmentId == '' && $surveyTypeId == '') {
                    return false;
                } else {
                    $condition = array();
                    ...
                    if ($term_id && $term_id != 0) {
                        ...
                        $condition['su_survey.crclm_term_id'] = $term_id;
                    }
                    ...
                    if (count($surveyList) > 0) {
                        ...
                        foreach ($surveyList as $surKy => $listData) {
                            if ($listData['section_id'] != "") {
                                $val = "<a href='survey/surveys/edit_survey/$listData[survey_id]' id='view_survey_3'><b>$listData[name]</b> [$listData[crs_title] ($listData[crs_code])] </a>";
                                $section_name_list = $listData['section_name'];
                            } else {
                                $section_name_list = ' ALL';
                                $val = "<a href='survey/surveys/edit_survey/$listData[survey_id]' id='view_survey_3'><b>$listData[name]</b></a>";
                            }
                            ...
                            if (($listData['status'] == 0)) {
                                ...
                                $surveyList[$surKy]['end_date'] = $listData['end_date'] . ' | <a class="cursor_pointer edit_survey_end_date" data-survey_id="' . $listData['survey_id'] . '" data-end_date="' . $listData['end_date'] . '" data-survey_status="progress" title="To Modify Survey END Date."><i class="icon-pencil cursor_pointer"></a>';
                            } else {
                                $status = 3;
                                $btn = 'Survey Closed';
                                $surveyList[$surKy]['end_date'] = $listData['end_date'] . ' | <a class="cursor_pointer edit_survey_end_date" data-survey_id="' . $listData['survey_id'] . '" data-survey_status="closed" data-end_date="' . $listData['end_date'] . '" title="To Modify Survey END Date."><i class="icon-pencil cursor_pointer"></a>';
                            }
                            ...
                            //  $surveyList[$surKy]['group_section_id'] = $group_section_id;
                            ...
                            $surveyList[$surKy]['section_name_list'] = $section_name_list;
                        }
                    }
        ...
        $data['programs'] = array('Select Program');
    ...
    function create_survey() {
        ...
        $data['term_list'] = array('Select Term');
        $data['section_list'] = array('Select Section');
...
//        $otherData['su_stakeholder_group']=$surveyData['su_stakeholder_group'];
        ...
        $deptId = $data['template_data']['su_template']['dept_id'];

        $condition = array('dept_id' => $deptId);
        if (array_key_exists('program_id', $data['template_data']['su_template'])) {
            $programId = $data['template_data']['su_template']['pgm_id'];
            $condition['pgm_id'] = $programId;
        }

        if (array_key_exists('su_for', $data['template_data']['su_template'])) {
            $surveyFor = $data['template_data']['su_template']['su_for'];
            $condition['su_for'] = $surveyFor;
        }

        if (array_key_exists('su_type_id', $data['template_data']['su_template'])) {
            $su_type_id = $data['template_data']['su_template']['su_type_id'];
            if ($su_type_id == 15) {
                $su_type_id_val = 9;
            } else if ($su_type_id == 16) {
                $su_type_id_val = 10;
            }

            $condition['su_type_id'] = $su_type_id;
        }

        $template = $this->Survey->listTemplateOptions('template_id', 'name', $condition);
        $data['survey_for'] = $this->Survey->getMasters('survey_for', 'Survey for');
        $data['template_type'] = $this->Survey->getMasters('template_type', 'template type');
        $data['question_choice'] = $this->Survey->getMasters('option_choice', 'question type');
        $data['option_type'] = $this->Survey->getMasters('option_type', 'option type');
        ...
        $drop_down = '<select name="course_name"  id="course_name123" class="input term_name survey_section_list_as_course remove_err">';
        foreach ($surveyData['course_data'] as $course) {
            if ($course['crs_id'] == $data['template_data']['su_survey']['crs_id']) {
                $drop_down .= '<option value="' . $course['crs_id'] . '" selected="selected">' . $course['crs_title'] . '</option>';
            } else {
                $drop_down .= '<option value="' . $course['crs_id'] . '">' . $course['crs_title'] . '</option>';
            }
        }
        $drop_down .= '</select>';
        $data['course_drop_cown'] = $drop_down;
        foreach ($surveyData['course_data'] as $course) {
        ...
        $term_one = array();
        $term_one[''] = 'Select Term';
        foreach ($surveyData['term_data'] as $term) {

            $term_one[$term['crclm_term_id']] = $term['term_name'];
        }
        $data['crclm_term_id'] = $surveyData['su_survey']['crclm_term_id'];
        $data['term'] = $term_one;


        $section_one = array();
        $section_one[''] = 'Select Section';
        $section_one[''] = 'ALL';
        foreach ($surveyData['section_data'] as $section) {

            $section_one[$section['mt_details_id']] = $section['mt_details_name'];
        }
        $data['section_id'] = $surveyData['su_survey']['section_id'];
        $data['section'] = $section_one;
    ...
    function getSurveyForList() {
        if ($this->input->is_ajax_request()) {
            ...
            if ($list != 0) {
                ...
                $opt.="</select> <span  style='color:red' id='errorspan_su_for_qstn_$qstnNo'></span>";
                $opt .= "<input type='hidden' name ='totalqtn_new' id = 'totalqtn_new' value ='" . $qstnNo . "' />";
                echo $opt;
                exit();
            }
        }
    ...
    // function to check the survey uniqueness.
    public function survey_uniqueness_edit() {

        $crclm_id = $this->input->post('crclm_id');
        $survey_name = $this->input->post('survey_name');
        $survey_id_for = $this->input->post('survey_id_for');
        $surveyName = $this->Survey->survey_uniqueness_edit($crclm_id, $survey_name, $survey_id_for);
    ...
    function makeFinalData($formData, $action, $otherData) {
        $finalData = array();
        $temp_op = "";
        $standard_option_feedbk = "";
        if (array_key_exists('template_options', $formData)) {
            $temp_op = $formData['template_options'];
        }
        if (array_key_exists('standard_option_feedbk', $formData)) {
            $standard_option_feedbk = $formData['standard_option_feedbk'];
        }
        //if($formData['template_options']){$temp_op = $formData['template_options'];}
        if ($formData['term_name'] != 0 || $formData['section_name'] != 0) {
            $finalData['su_survey'] = $su_survey = array(
                ...
                'crclm_term_id' => $formData['term_name'],
                'section_id' => $formData['section_name'],
                ...
                'status' => '0',
                'threshold_value' => $temp_op,
                'su_ans_tmplts' => $standard_option_feedbk,
        ...
        if ($action == 'edit') {
            ...
            if (array_key_exists('totalqtn_new', $formData)) {
                $totalQstn = $formData['totalqtn_new'];
            } else {
            ...
            if (array_key_exists('totalqtn_new', $formData)) {
                ...
                $totalQstn = $formData['total_question'];
            }//as i increment question no before it added ini ui i need substract by 1 here.
        }
        $feedBk = 'feedbk_';
        ...
        for ($i = 1; $i <= $totalQstn; $i++) {
            if (array_key_exists('question_' . $i, $formData)) {
                ...
                echo $finalData['su_survey_questions'][$i] = $su_question;
    ...
    function surveyAllAjax($ajaxParam = null) {
        ...
        if ($flag == 'program') {
            ...
            if (!($this->ion_auth->is_admin() || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner'))) {
                $termList = $this->Survey->listTermOptions_co('crclm_term_id', 'term_name', array('crclm_id' => $programId), array('course_instructor_id' => $user_id));
            } else {
                $termList = $this->Survey->listTermOptions('crclm_term_id', 'term_name', array('crclm_id' => $programId));
            }
            $optSt = "<select class='input' id='term_name' name='term_name'>";
            foreach ($termList as $key => $termName) {
                @$opt.="<option value='$key'>$termName</option>";
            }
            $optEnd = "</select>";
            $box = $optSt . $opt . $optEnd;
            echo $box;
            ...
            $user_id = $this->ion_auth->user()->row()->id;
            if (!($this->ion_auth->is_admin() || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner'))) {
                $courseList = $this->Survey->listCourseOptions('crs_id', 'crs_title', array('crclm_term_id' => $programId), array('clo_owner_id' => $user_id));
            } else {
                $courseList = $this->Survey->listCourseOptions('crs_id', 'crs_title', array('crclm_term_id' => $programId));
            }
            ...
            $user_id = $this->ion_auth->user()->row()->id;
            if (!($this->ion_auth->is_admin() || $this->ion_auth->in_group('Chairman') || $this->ion_auth->in_group('Program Owner'))) {
                $sectionList = $this->Survey->listSectionOptions('mt_details_id', 'mt_details_name', array('crs_id' => $programId));
            } else {
                $sectionList = $this->Survey->listSectionOptions('mt_details_id', 'mt_details_name', array('crs_id' => $programId));
            }
            $optSt = "<select class='input' id='section_name' name='course_name'>";

            foreach ($sectionList as $key => $sectionName) {
                @$opt.="<option value='$key'>$sectionName</option>";
            }
            $optEnd = "</select>";
            $box = $optSt . $opt . $optEnd;
            echo $box;
            ...
            //<font color='red'> Select Curriculum.</font>
            $optEnd = "</select><span id='errorspan_curriculum' class='error help-inline'></span>";
            $box = $optSt . $opt . $optEnd;
            echo $box;
            exit();
            ...
            if (array_key_exists('su_type_id', $ajaxParam)) {
                $su_type_id = $ajaxParam['su_type_id'];
                if ($su_type_id == 15) {
                    $su_type_id_val = 9;
                } else if ($su_type_id == 16) {
                    $su_type_id_val = 10;
                ...
                $condition['su_type_id'] = $su_type_id_val;
            }
            ...
            if ($ajaxParam['std_grp'] == 1) {
                $crclm_term_id = $ajaxParam['crclm_term_id'];
                $section_id = $ajaxParam['section_id'];
            }
            ...
            if ($check_survey_status['status'] == 0 && $check_survey_status['usr_count'] >= 0) {
                if ($stud_flag_Id != 1) {
                    ...
                    if ($section_id != '') {
                        $conditions = array('su_student_stakeholder_details.crclm_id' => $crclmId, 'su_student_stakeholder_details.status_active' => 1, 'su_student_stakeholder_details.section_id' => $section_id);
                    } else {
                ...
                $opt.=" <div class='span2'><b>Stakeholder</b></div>";
                if (@$stud_flag_Id) {
                    $opt.=" <div class='span2'><b>Section</b></div>";
                }
                $opt.=" <div class='span2'><b>Email</b></div>";
                $opt.=" <div class='span2'><b>Contact</b></div>";
                ...
                foreach ($stakeholderList as $key => $stakeholderListData) {
                    ...
                    $opt.="<div class='span2'>" . ($stud_flag_Id != 1 ? '' : $stakeholderListData['title']) . "" . $stakeholderListData['first_name'] . " " . $stakeholderListData['last_name'] . "</div>";
                    $section_name = '';
                    if (@$stud_flag_Id) {
                        if (isset($stakeholderListData['section_id'])) {
                            $section_name = $stakeholderListData['section_id'];
                        }
                        $opt.="<div class='span2'>" . $section_name . "</div>";
                    }
                    $opt.="<div class='span2' style='margin-right: 5%;margin-left:0;'>" . $stakeholderListData['email'] . "</div>";
                    $opt.="<div class='span2'>" . $stakeholderListData['contact'] . "</div>";
    ...
    public function fetch_template() {
        ...
        $pgm_id = $this->input->post('program_id');
        $su_for = $this->input->post('su_for');
        ...
        $result = $this->Survey->fetch_template($dept_id, $pgm_id, $su_for, $su_type_id);
        $template_option = array();
        $template_option[''] = 'Select Term';
        foreach ($result as $template) {

            $template_option[$template['template_id']] = $template['name'];
        }
        echo json_encode($template_option);
    }

    public function change_date() {
        $new_date = $this->input->post('new_date');
        $survey_id = $this->input->post('survey_id');
        $result = $this->Survey->update_new_date($survey_id, $new_date);
        if ($result == true) {
            echo '1';
        } else {
            echo 0;
