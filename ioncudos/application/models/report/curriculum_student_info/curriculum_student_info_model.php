...
class Curriculum_student_info_model extends CI_Model {
    ...
    public function update_student_info($program_id, $crclm_id, $stud_intake, $stud_admitted, $stud_migrated_other_pgm, $stud_migrated_this_pgm, $stud_lateral, $stud_division, $rank_from, $rank_to, $stud_graduate_data, $num_year_count, $stud_companies, $stud_higher_studies, $stud_entrepreneur) {
        $update_data = array(
    ...
    /*
     * Function is to get the list of Entrance Exam type.
     * @param   :
     * returns  :list of Entrance Exam.
     */

    public function fetch_entrance_exam() {
        $entrance_exam_type_query = 'SELECT * '
                . '                 FROM master_type_details '
                . '                 WHERE master_type_id=28';
    ...
    public function fetch_entrance_exam_h($progrm_type) {
        if ($progrm_type == 233) {
            $entrance_exam_type_query = 'SELECT * '
                    . '                 FROM master_type_details '
                    . '                 WHERE master_type_id=28 AND mt_details_id BETWEEN 95 AND 99';
            $entrance_exam_type_data = $this->db->query($entrance_exam_type_query);
            $entrance_exam_type_list = $entrance_exam_type_data->result_array();
            return $entrance_exam_type_list;
        }
        if ($progrm_type == 93) {
            $entrance_exam_type_query = 'SELECT * '
                    . '                 FROM master_type_details '
                    . '                 WHERE master_type_id=28 AND mt_details_id BETWEEN 100 AND 103';
            $entrance_exam_type_data = $this->db->query($entrance_exam_type_query);
            $entrance_exam_type_list = $entrance_exam_type_data->result_array();
            return $entrance_exam_type_list;
        }
    ...
    public function insert_student_admitted_data($pgm_id, $crclm_id, $ent_exam, $caste, $gender, $intake, $rank_from, $rank_to) {
    ...
    public function update_student_admitted_data($student_admitted_id, $pgm_id, $crclm_id, $ent_exam, $caste, $gender, $intake, $rank_from, $rank_to) {
        $student_admitted_data = array(
            'entrance_exam_id' => $ent_exam,
            'caste_id' => $caste,
            'gender_id' => $gender,
    ...
    public function insert_higher_studies_data($pgm_id, $crclm_id, $ent_exam, $caste, $gender, $num_stud) {
    ...
    public function update_higher_studies_data($higher_studies_edit, $pgm_id, $crclm_id, $ent_exam, $caste, $gender, $num_stud) {
        $higher_studies_data = array(
            'entrance_exam_id' => $ent_exam,
            'caste_id' => $caste,
            'gender_id' => $gender,
