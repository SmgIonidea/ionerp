...
/*
  --------------------------------------------------------------------------------------------------------------------------------
* Description	: Controller Logic for List of CIA, Provides the facility to Edit and Delete the particular MTE and its Contents.	  
...
class Tier1_mte_clo_attainment_model extends CI_Model {
  ...
  public function dropdown_dept_title() {
    ...
    public function course_drop_down_fill($term) {
       ...
       if($this->ion_auth->in_group('Course Owner') && !$this->ion_auth->in_group('admin') && !$this->ion_auth->in_group('Chairman') && !$this->ion_auth->in_group('Program Owner')){
             $fetch_course_query = $this->db->query('SELECT DISTINCT(c.crs_id),c.crs_title
                                                FROM course c,qp_definition qpd, map_courseto_course_instructor as map
    ...
    public function dropdown_list_finalize($crclm_id, $term_id, $course_id){
        $select_query = $this->db->query('SELECT * FROM course_clo_owner c JOIN course as crs on c.crs_id = crs.crs_id  and mte_flag = 1 where c.crs_id =  "'.$course_id.'" ');
        ...
        if(!empty($result)){
            ...
            $status_query = 'select   count(ao_id) as status  from tier1_section_clo_overall_cia_attainment as t JOIN assessment_occasions as a on a.crs_id = t.crs_id and a.mte_flag = 1 where t.crs_id  = "'.$course_id.'"  and  t.section_id IS NULL';
            $status_data = $this->db->query($status_query);
            $status_res = $status_data->row_array();

            $select_query = 'SELECT tier1.clo_id, tier1.threshold_section_direct_attainment as attainment,  ROUND((tier1.threshold_section_direct_attainment)*' . $mte_wgt . '/100,2) as threshold_clo_da_attainment_wg , tier1.average_section_direct_attainment as avg_attainment, tier1.section_id, tier1.crs_id, '
                            ...
                            . ' LEFT JOIN assessment_occasions as ao on ao.crs_id = tier1.crs_id and ao.mte_flag = 1'
                            . ' LEFT JOIN clo as cl ON cl.clo_id = tier1.clo_id '
                            . ' WHERE tier1.crclm_id = "'.$crclm_id.'"'
                            . ' AND tier1.crclm_term_id = "'.$term_id.'" AND tier1.section_id IS NULL AND ao.mte_flag = 1'
                            .'  AND tier1.crs_id = "'.$course_id.'" group by cl.clo_id';
      ...
      //  }
        ...
        $data['section_name_array'] =  $result;
     ...
     */
    public function fetch_clo_attainment_data($crclm_id,  $term_id, $course_id, $type_id, $occasion_id , $occasion_not_selected){
        if($type_id == 2){
            $cia = 3;
        }
        ...
        $count_occasions = 'SELECT count(ao_id) as count_ao_id FROM assessment_occasions WHERE crs_id = "'.$course_id.'" AND mte_flag = 1 ';
		...
		$error .= " Marks are not  uploaded for  <b>" . $occasion  ." </b>  occasion. \n\n";
        ...
        if($occasion_not_selected != 0){
            $attainment_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id, '
                            . ' att.ao_weightage, ROUND(AVG(att.threshold_ao_direct_attainment),2) as  threshold_ao_direct_attainment, 
							ROUND((att.threshold_ao_direct_attainment)*' . $mte_wgt . '/100,2) as threshold_clo_da_attainment_wgt ,
							ROUND( AVG(att.average_ao_direct_attainment) ,2) as average_ao_direct_attainment, '
                            . ' att.created_by, att.created_date, att.modified_by, att.modified_date,  '
                            ...
                            . ' LEFT JOIN assessment_occasions  as a  ON a.ao_id  = att.ao_id'
                            ...
                            . ' AND att.crclm_term_id = "'.$term_id.'" AND att.crs_id = "'.$course_id.'" AND  a.mte_flag = 1   group by cl.clo_id';
			...
			$clo_overall_attainment_query = 'SELECT oatt.sclo_oall_id, oatt.crclm_id, oatt.crclm_term_id, oatt.crs_id, oatt.section_id, oatt.clo_id, '
									. ' oatt.threshold_section_direct_attainment, ROUND((oatt.threshold_section_direct_attainment)*' . $mte_wgt . '/100,2) as threshold_clo_da_attainment_wgt  ,  oatt.average_section_direct_attainment, '
            ...
            $attainment_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id, '
                            ...
                            . ' AND att.crclm_term_id = "'.$term_id.'" AND att.crs_id = "'.$course_id.'" AND  a.mte_flag = 1 GROUP BY att.clo_id ';
        }
   ...
   $po_attain_query = 'SELECT t.sclo_oall_id, ROUND(AVG(t.threshold_section_direct_attainment),2) as direct_attainment, 
    ...
    public function fetch_drilldown_attainment_data($clo_id){
        ...
        $attainment_table_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id, '
                                ...
                                . ' AND att.crs_id = "'.$clo_result['crs_id'].'" AND att.assess_type = 3 group by att.clo_id ';
    ...
    public function get_co_mapped_questions($clo_id,$type,$ao_id ,$occasion_not_selected) {
        ...
        if ($occasion_not_selected == 0) {
                $result = $this->db->query('SELECT qpm.actual_mapped_id, qpm.entity_id, mnq.qp_subq_code, qpm.qp_mq_id, 
                                            ...
                                            LEFT JOIN assessment_occasions aocc ON  aocc.mte_flag = 1
                ...
                $ao_query = 'SELECT qpd_id  FROM assessment_occasions WHERE ao_id IN('.$occasion_selected.') and mte_flag = 1 ';
    ...
    public function co_finalize($crclm_id,  $term_id, $course_id, $type_id, $occasion_id){
        if($type_id == 2){
            $cia = 3;
        }
        ...
        $count_occasion_data = $this->db->query($count_occasions);
        $count_result = $count_occasion_data->row_array();
        $attainment_query = 'SELECT att.sclo_ao_id, att.assess_type, att.ao_id, att.crclm_id, att.crclm_term_id, att.crs_id, att.section_id, att.clo_id,  att.ao_weightage,  att.threshold_ao_direct_attainment, cast(ROUND(AVG(att.average_ao_direct_attainment))as decimal(10,2)) as average_ao_direct_attainment,cast(ROUND(AVG(att.threshold_ao_direct_attainment)) as decimal(10,2)) as threshold_ao_direct_attainment, att.created_by, att.created_date, att.modified_by, att.modified_date,
			cl.clo_statement, cl.cia_clo_minthreshhold, cl.clo_code
			FROM tier1_section_clo_ao_attainment as att
			LEFT JOIN assessment_occasions  as ao  ON ao.ao_id = att.ao_id and ao.mte_flag = 1
			LEFT JOIN clo as cl ON cl.clo_id = att.clo_id
			WHERE att.assess_type = "'.$cia.'"  AND att.crclm_id = "'.$crclm_id.'" 
			AND att.crclm_term_id = "'.$term_id.'" AND att.crs_id = "'.$course_id.'"  AND ao.mte_flag = 1 AND ao.section_id = 0  GROUP BY att.clo_id ';
       $clo_attainment_data = $this->db->query($attainment_query);
       $clo_attainment_res = $clo_attainment_data->result_array();
        
        $data_check = 'SELECT COUNT(sclo_oall_id) as data_size FROM tier1_section_clo_overall_cia_attainment WHERE crs_id = "'.$course_id.'" AND section_id IS NULL';
        ...
        if($data_res['data_size'] == 0){
            ...
            $delete_query  = 'DELETE FROM tier1_section_clo_overall_cia_attainment WHERE crs_id = "'.$course_id.'" AND section_id IS NULL';
            ...
            $get_course_owner_id = $this->db->query('SELECT clo_owner_id FROM course_clo_owner  WHERE crs_id = "'.$course_id.'" ');            
            $course_owner_id = $get_course_owner_id->row_array();
            
            $update_dashboard = 'UPDATE dashboard SET status = 0 WHERE receiver_id = "'.$course_owner_id['clo_owner_id'].'" AND particular_id = "'.$course_id.'" AND crclm_id = "'.$crclm_id.'" AND entity_id = 4 ';
    ...
    public function fetch_co_details($param) {
        ...
        if($type_all_selected == 0 || $type_all_selected == '' ) {
            $query = $this->db->query('SELECT c.crclm_name AS "crclm_name", t.term_name AS "term_name", crs.crs_code AS "crs_code", crs.crs_title AS "crs_title", GROUP_CONCAT(o.ao_description) AS "occasion"
                                        FROM curriculum c
                                        ...
                                        LEFT JOIN assessment_occasions o ON (o.crclm_id = c.crclm_id AND o.term_id = t.crclm_term_id AND o.crs_id = crs.crs_id AND o.section_id = 0 AND o.mte_flag = 1)
                                        ...
                                        AND a.term_id = '.$term_id.' AND a.crs_id = '.$crs_id.' AND a.section_id = 0 AND a.mte_flag = 1 )');
        } else {
        ...
        $query = $this->db->query('SELECT c.crclm_name AS "crclm_name", t.term_name AS "term_name", crs.crs_code AS "crs_code", crs.crs_title AS "crs_title", o.ao_description AS "occasion"
                                        ...
                                        LEFT JOIN crclm_terms t ON t.crclm_id = c.crclm_id
                                        LEFT JOIN course crs ON (crs.crclm_id = c.crclm_id AND t.crclm_term_id = crs.crclm_term_id)                                       
                                        LEFT JOIN assessment_occasions o ON (o.crclm_id = c.crclm_id AND o.term_id = t.crclm_term_id AND o.crs_id = crs.crs_id AND  o.section_id IS NULL AND o.mte_flag = 1)
                                        WHERE c.crclm_id = '.$crclm_id.'
                                        AND t.crclm_term_id = '.$term_id.'
                                        AND crs.crs_id = '.$crs_id.'
										AND o.mte_flag = 1
