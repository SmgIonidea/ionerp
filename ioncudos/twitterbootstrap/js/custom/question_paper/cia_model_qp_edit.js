...
var id = abbr_array[0].match(/\d+/g); // main question number
...
sub_que_count_val++;
//alert(sub_que_count_val);
sub_que_array.push({index:parseInt(id), value:sub_que_count_val});
console.log(sub_que_array);
var main_que_abbr_val = $(this).attr('main_que_abbr');
var main_que_data = main_que_abbr_val.split('-');
var main_que_number = main_que_data[0].match(/\d+/g);
var main_que_alphbet = main_que_data[1];

//alert(main_que_alphbet)
$('.sub_que_count'+id+':last').val(sub_que_count_val);
sub_que_counter = String.fromCharCode(main_que_alphbet.charCodeAt() + 1);

$(this).attr('main_que_abbr','Q No '+main_que_number+'-'+sub_que_counter);
					...
					success: function(msg)
					{
						...
						// console.log(bloom_level)
								var sub_que = '<tr id="row_'+id+'_'+sub_que_count_val+'" class="row_'+id+'">';
									 ...
									 sub_que += '&nbsp&nbsp;&nbsp;&nbsp;&nbsp; <input type="text" name="ques_nme_'+id+'_'+sub_que_count_val+'" id="ques_nme_'+id+'_'+sub_que_count_val+'" value="Q No '+main_que_number+'-'+sub_que_counter+'" class="input-mini ques_nme_'+id+'" readonly />';
									 sub_que += '<input type="hidden" name="question_name_'+id+'_'+sub_que_count_val+'" id="question_name_'+id+'_'+sub_que_count_val+'" value="Q_No_'+main_que_number+'_'+sub_que_counter+'" class="input-mini question_name_'+id+'" readonly /></td>';
							...
							// for(var i =0; i< entity_val;i++ ){
								if(msg.qp_entity[i].entity_id == 22){
									 ...
									 sub_que += '<button class="btn btn-inverse" type="button" id="delete_subque_'+id+'_'+sub_que_count_val+'" name="delete_subque_'+id+'_'+sub_que_count_val+'" disabled="disabled" ><i class="icon-plus-sign icon-white"></i></button>';
									 sub_que += '<button class="btn btn-danger delete_'+id+' delete_subque error_add" type="button" id="delete_subque_'+id+'_'+sub_que_count_val+'" name="delete_subque_'+id+'_'+sub_que_count_val+'" abbr="QNo_'+id+'_'+sub_que_count_val+'" sub_que_abbr="'+main_que_number+'"><i class="icon-minus-sign icon-white"></i></button>';
									 sub_que += '</div>';
									 sub_que += '</td>';
									 sub_que += '</tr>';
...
//$('#add_subque_'+main_id[0]+'_1').attr('abbr','Q No '+abbr_val+'-'+alpha)
$('#add_subque_'+main_id[0]+'_1').attr('main_que_abbr','Q No '+abbr_val+'-'+alpha)
...
// for( var j = 1; j <= main_que_count; j++){
 ...
 // alert(remove_img[0]+'_'+remove_img[1])
  ...
  // Form validation
	...
	$(document).ready(function(){
		...
		}, "<font class='font_color'>Field must contain only letters, spaces, ' or dashes or dot</font>");
  ...
  console.log($('#modal_footer').html());
$('#modal_footer').on('click','#save_data', function(){
...
if(validation_flag == true){
	...
	$.ajax({
					type: "POST",
					url: base_url+'question_paper/manage_model_qp/cia_update_qp_data/3',
...
$('#qp_table_load').on('change','.co_list_data',function(){
if($(this).val() !=''){
...
$('#main_que_load').on('click','.add_main_que', function(){
var co_data;
var topic_data;
var bloom_level;
var entity_val;
						...
						pi_code = msg.pi_list.length;
						 ...
						 //console.log(msg.bloom_data)
								var main_que = '<tr id="row_'+total_counter+'_1" abbr="main_que_'+total_counter+'" class="row_'+total_counter+'">';
									 ...
									 main_que += '<input type="hidden" name="sub_que_count'+total_counter+'" id="sub_que_count'+total_counter+'" value="1" class="sub_que_count'+total_counter+'"/>';
									 main_que += '</td>';
									 main_que += '<td>';
							...
							// for(var i =0; i< entity_val;i++ ){
								if(msg.qp_entity[i].entity_id == 22){
									 ...
									 main_que += '<button class="btn btn-primary add_subque" type="button" id="add_subque_'+total_counter+'_1" name="add_subque_'+total_counter+'_1" abbr="Q No '+total_counter+'-a" main_que_abbr="Q No '+que_counter+'-a"><i class="icon-plus-sign icon-white"></i></button>';
									 ...
									 main_que += '</tr>';
									 main_que += '<tr id="sub_que_ref_div_'+total_counter+'" class="row_'+total_counter+' sub_que_ref_div_'+total_counter+'"></tr>';
						//console.log(sub_que);
...
if(total_que_val > 1){
			...
			$('.ques_nme_'+main_que_array[i]).each(function(){
				...
				$('#add_subque_'+main_que_array[i]+'_1').attr('main_que_abbr','Q No '+mq+'-'+alphabet);
...
function delete_main_question(array_to_del,key) {
var array_length = array_to_del.length;
var i = 0;
 while(i < array_to_del.length) {
	if (array_to_del[i].index==key) {
	array_to_del.splice(i,1);
	i = 0;
 }else{
	i++;
 }
