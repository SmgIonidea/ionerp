...
$output = '<table class="table table-bordered char_font_size_12">
		<thead>	
			<b> Publication Name :</b>  ' . $company[0]['publ_name'] . '
	   		<tr>
...
foreach ($files as $file) {
    ...
    $file_value = $file['publ_id'];
    $name = "tech_magazine";
    $folder_name = $file_value . '_' . $name;
    $date = explode("-", $file['upload_actual_date']);
    $date = $date[2] . '-' . $date[1] . '-' . $date[0];
    $output .= '<tr>
		    <td style="text-align:right;"><input name="company_id" type="hidden" value="' . $file['publ_id'] . '">' . $i . '</td>
		    <td><a href="' . base_url() . 'uploads/publ_tech_magazine/' . $folder_name . '/' . $file['upload_file_name'] . '" target="_blank" >' . $new_name . '</a></td>
