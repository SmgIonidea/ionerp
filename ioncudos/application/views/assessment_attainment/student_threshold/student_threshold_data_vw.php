...
<!-- displays list of student USN along with their respective COs -->
	<div class="container-fluid">
		<input type="hidden" id="entity_id" name="entity_id" value="<?php echo $entity_id; ?>">
			...
			<input type="hidden" id="occasion_id" name="occasion_id" value="<?php echo $occasion_id; ?>">
			<input type="hidden" id="qpd_id" name="qpd_id" value="<?php echo $qpd_id; ?>">
	<?php if($count>0){ ?>
		<div class="span6" id="improvement_deatils">
					...
					<tr id="imp_plan_row_<?php echo $imp_plan['sip_id']; ?>">
						...
						<td>
							<?php if($imp_plan['qpd_type'] == 5) {
								...
								} elseif($imp_plan['qpd_type'] == 3 && $imp_plan['section_id'] != 0) {
									echo $this->lang->line('entity_cie');
								}elseif($imp_plan['qpd_type'] == 3 && $imp_plan['section_id'] == 0) {
									echo $this->lang->line('entity_mte');
								} else {
									echo $this->lang->line('entity_mte') .'&'. $this->lang->line('entity_tee');
								} ?>
						</td>
						<td>
		...
		<br/><br/><br/>
			...
			<?php $this->load->library('table');
				...
				echo strtoupper($this->table->generate($student_details));
			?>
		</div>		
	<?php }else{ ?>
			</br>
		<div> <p> <font color="red"> No student(s) found matching  this criteria. </font></p></div>
	<?php }?>
	</div>
	...
	<div id="myModal_view_ip" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;" data-controls-modal="myModal_view_ip" data-backdrop="static" data-keyboard="false">
		...
		<div class="modal-body">
			<table class="table table-bordered table-hover">
				...
				<tr>
					<td style="width: 150px;" > Students: </td>
