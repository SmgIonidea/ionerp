<?php
/**
 * Description          :	View for NBA SAR Report - Section 3.1 (TIER 1) - Course grid view.
 * Created              :	27-04-2017
...
foreach ($clo_detail as $clo) {
        ...
        $final_string .= $span_12 . '<div class="span4">
                                        <div class="term_container">
                                            <table border="1" class="term_parent" cellpadding="4">
                                                <tr>
                                                    <td colspan=4 gridspan=3 class="orange" title="keep cursor on course code to see course title.">
    ...
    foreach ($crs_id_list as $crs_id_list_key => $crs_id_list_value) {
        if ($crs_id_list_value) {
            ...
            if ($is_check && $is_export) {
                $final_string.= '<tr>
                                <td title="' . $crs_list[$crs_id_list_key] . '" ' . $table_width . '>' . $crs_code_list[$crs_id_list_key] . '</td>
                                <td ' . $table_width . '>&#10004;</td>
                            </tr>';
            } else {
                $final_string.= '<tr>
                                <td ' . $table_width . ' title="' . $crs_list[$crs_id_list_key] . '">' . $crs_code_list[$crs_id_list_key] . '</td>
                                <td ' . $table_width . '>' . form_checkbox('cos_checkbox_list__' . $view_id . '_' . $nba_report_id, $crs_id_list_value, $is_check, 'class="filter cos_checkbox select_all__' . $i . '"') . '</td>
                            </tr>';
            }
        }
    }
...
?>
<!-- End of file t1ug_c3_1_course_vw.php 
        Location: .nba_sar/ug/tier1/criterion_3/t1ug_c3_1_course_vw.php -->