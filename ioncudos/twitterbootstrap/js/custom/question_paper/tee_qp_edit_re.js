...
var size_bl = $.trim(msg.bl_result.length);
			...
			if (parseInt(entity_val) != 0) {
				...
				//$('#po_list_'+main_q_id+'_'+sub_q_id).multiselect('hide');
				//$('.po_list').remove();
				var size = $.trim(msg.po_result.length);
...
$('.qp_table').on('change', '.unit_onchange', function () {
	...
	$.ajax({
		...
		success : function (msg) {
			...
			//console.log(msg['qcode'][0]['qp_subq_code']);
...
$('#save_header').on('click', function () {
	...
	if (validation_flag == true) {
		if (grand_total >= max_marks) {
			...
			var data_options = '{"text":"Grand total Marks is less Maximum Marks.    ","layout":"center","type":"error","animateOpen": {"opacity": "show"}}';
			var options = $.parseJSON(data_options);
			noty(options);
			$('#loading').hide();
...
$('#example').on('click', '.edit_qp', function (e) {
	...
	$.ajax({
		...
		success : function (data) {
			console.log(data);
//			console.log(data.que_max_marks);
			$('#max_qpmarks').val(data.que_max_marks);
			$('#val_status').val(data.qp_rollout);
			...
			//console.log(data.entity_config[0]);
			...
			if(data.qt_result != null){var qn_size = data.qt_result.length;}else{ var qn_size = '';}
                       
			if (data.mapped_po_data === null) {}
			else {
			...
			if (data.entity_config[0]['qpf_config'] == 0) {
				...
				for (var p = 0; p < co_rel_po; p++) {
					$("#po_list_1_1").append($('<option>', {
							value : data.mapped_po_data[p]['po_id'],
							title : data.mapped_po_data[p]['po_statement'],
							text : data.mapped_po_data[p]['po_reference']
						}));
				}
			...
			for (var bl = 0; bl < level_size; bl++) {
				$("#bloom_list_1_1 option[value='" + data.level_mapping_data[bl]['actual_mapped_id'] + "']").attr("selected", "selected");
                        ...
                        if(qn_size != 0){
                        for (var qn = 0; qn < qn_size; qn++) {
				$("#qn_type_1_1 option[value='" + data.qt_result[qn]['mt_details_id'] + "']").attr("selected", "selected");
			}
                        $("#qn_type_1_1").multiselect('rebuild');
                    }else{
                        $("#qn_type_1_1").prop('selected',false);
                        $("#qn_type_1_1").multiselect('rebuild');
                    }

			if (data.qp_mq_code) {
				$("#Q_No_1_1 option:contains(" + data.qp_mq_code + ")").attr('selected', 'selected');
				// $('#Q_No_1_1 option').each(function(){
				// if($(this).text() == data.qp_mq_code){
				// $(this).attr("selected", "selected");
				// }
				// });

				//$("#Q_No_1_1 option[text='" + data.qp_mq_code + "']").attr("selected", "selected");
	...
	if (unit_sum == Grand_total) {
		...
		$.ajax({
			...
			success : function (data) {
				...
				$("#unit_list_1_1").empty();
				$("#unit_list_1_1").append($('<option>', {
						value : '',
						text : 'Select'
					}));
				for (var u = 0; u < unit_size; u++) {
					$("#unit_list_1_1").append($('<option>', {
							value : data.units[u]['qpd_unitd_id'],
							title : data.units[u]['qp_unit_code'],
							text : data.units[u]['qp_unit_code']
						}));
		...
		$('.qn_type_data').multiselect('rebuild');
...
$('#example').on('click', '.weightage_add', function () {
	...
	$('#myModal_mapping_weightage').modal({
		dynamic : true
	});
	var post_data = {
		'main_que_id' : main_que_id
	};
	$.ajax({
		type : 'POST',
...
$('#myModal_mapping_weightage').on('click', '#save_weight', function () {
	...
	var post_data = {
		'co_weight_array' : co_weight_array,
...
$('#update').on('click', function () {
	if ($('#val_status').val() == -2) {
		jQuery.validator.addMethod("greaterMax", function (value, element) {
			return this.optional(element) || (parseFloat(value) >= $('#max_qpmarks').val());
		}, "Marks cannot be less than max. marks : "+$('#max_qpmarks').val()+"");
	}
	var validation_flag = $('#add_question_details').valid();
	...
	if (validation_flag == true) {
		...
		post_data = {
			'qp_subq_code' : qp_subq_code,
			'qp_subq_marks' : qp_subq_marks,
			'qp_mq_id' : qp_mq_id,
			'qp_content' : que_content,
			'co' : co,
			'unit_q_no' : unit_q_no,
			'po' : po,
			'pi' : pi,
...
$('#generate_FM').on('click', function () {
	mytable = $('<table  class="table table-bordered table-hover" id="example" aria-describedby="example_info"></table>').attr({
			id : "basicTable"
		});
...
$(function () {
	...
	validator.settings.ignore = ':hidden:not(".co_list_data"):hidden:not(".po_list_data"):hidden:not(".topic_list_data"):hidden:not(".bloom_list_data"):hidden:not(".qn_type_data")';
