...
class Tier1_mte_clo_attainment extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->library('session');
        $this->load->helper('url');
        $this->load->model('assessment_attainment/tier_i/mte_attainment/tier1_mte_clo_attainment_model');
    ...
    public function index() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $data['deptlist'] = $this->tier1_mte_clo_attainment_model->dropdown_dept_title();
            $data['crclm_data'] = $this->tier1_mte_clo_attainment_model->crlcm_drop_down_fill();
            $data['title'] = "Course - " . $this->lang->line('entity_clo_singular') . " Attainment";
            $this->load->view('assessment_attainment/tier_i/tier1_mte_clo_attainment_vw', $data);
        }
    }
    ...
    public function select_term() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $term_data = $this->tier1_mte_clo_attainment_model->term_drop_down_fill($crclm_id);
    ...
    public function select_course() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $term_data = $this->tier1_mte_clo_attainment_model->course_drop_down_fill($term);
    ...
    public function select_occasion() {
        if (!$this->ion_auth->logged_in()) {
           ...
           // $section_id = $this->input->post('section_id');
            $occasion_data = $this->tier1_mte_clo_attainment_model->occasion_fill($crclm_id, $term_id, $crs_id);
	 ...
	 public function fetch_clo_ao_ttainment($crclm_id = NULL, $term_id = NULL, $course_id = NULL, $section_id = NULL, $occasion_id = NULL, $type_id = NULL, $export_flag = NULL, $graph = NULL, $co_details = NULL) {
        ...
        $section_list = $this->tier1_mte_clo_attainment_model->dropdown_list_finalize($crclm_id, $term_id, $course_id);
        $attainment_data = $this->tier1_mte_clo_attainment_model->fetch_clo_attainment_data($crclm_id, $term_id, $course_id, $type_id, $occasion_id , $occasion_not_selected);
	...
	//	$occasion_marks = $attainment_data['error_marks'];
		
		$error_status_data = $this->tier1_mte_clo_attainment_model->error_status_data($course_id, $section_id);
		$occasion_marks = '';
		for($k = 0 ; $k < count($error_status_data) ; $k++){
		$occasion_marks .= $error_status_data[$k]['status_data'];
		}		
	
		if(!empty($occasion_marks) && $occasion_marks != ' '){ $display_error_atatus  = '<table width="100%"><tr><td><font color="red"><center><b>You cannot Finalize the Course - '. $this->lang->line('entity_mte') .'- Course Outcomes (COs) Attainment . Kindly complete the below activities :<br/></b>'. $occasion_marks .'<br/><a class="cursor_pointer" href="'. base_url().'assessment_attainment/import_mte_data">Click here to Upload marks .</a></center></font></td></tr></table>';}else{  $display_error_atatus = '';}
        ...
        if (!empty($section_result)) {
            foreach ($section_result as $section_data) {
                ...
                if ($status[$j]['status'] > 0) {
                    $flag = 'Finalized';
                    $table_finalize .='<th colspan="2"><b>Status: <font color="#09C506">' . $this->lang->line('entity_mte') . ' Attainment is Finalized</font></b></th>';
                } else {
                    $flag = 'Not_finalized';
                    $table_finalize .='<th colspan="2"><b>Status: <font color="#FA7004">' . $this->lang->line('entity_mte') . ' Attainment is not Finalized</font></b></th>';
                }
                $table_finalize .='</tr>';
                $table_finalize .='<tr>';
                ...
                $table_finalize .='<th style="text-align: -webkit-center;"> ' . $this->lang->line('entity_mte') . ' Threshold %</th>';
                ...
                $table_finalize .='</tr>';
                $table_finalize .='</thead>';
                $table_finalize .='<tbody>';$size = 0;
                foreach ($section_over_all_attain[0] as $overall_attain) {

                    $size = count($section_over_all_attain[0]);                    
                        if ($overall_attain['avg_attainment'] != '') {
                            $avg = '%';
                        } else {
                            $avg = '-';
                        }
                        if (!empty($overall_attain)) {                         
                                $table_finalize .='<tr>';
                                $table_finalize .='<td>' . $overall_attain['clo_code'] . '</td>';
                                $table_finalize .='<td>' . $overall_attain['clo_statement'] . '</td>';
                                $table_finalize .='<td style="text-align: -webkit-right;">' . $overall_attain['cia_clo_minthreshhold'] . '%</td>';
                                $table_finalize .='<td style="text-align: -webkit-center;">' . $overall_attain['attainment'] . '%<br><a class="cursor_pointer tier1_attainment_drilldown" data-clo_id="' . $overall_attain['clo_id'] . '" data-section_id="' . $overall_attain['section_id'] . '">drill down</a></td>';
                                $table_finalize .='<td style="text-align: -webkit-right;">' . $overall_attain['avg_attainment'] . '' . $avg . '</td>';
                                $table_finalize .='</tr>';                           
								$attainment += $overall_attain['attainment'];
								$attainment_wgt += $overall_attain['threshold_clo_da_attainment_wg'];									
                        } else {
                            $table_finalize .='<tr>';
				...
				if(!empty($section_over_all_attain[0])){
					$attainment = $attainment / ($size+1);
					$attainment_wgt = $attainment_wgt / ($size+1);
        ...
        if (!empty($section_wise_attainment)) {
            ...
            $table .= '<th class=""> ' . $this->lang->line('entity_mte') . ' Threshold %</th>';
        ...
        if (!$export_flag) {
            ...
            if (!empty($section_result)) {
                foreach ($section_result as $section_data) {
                   ...
                   // $table_finalize .='<td width=100><b>Section / Division : ' . $section_data['section_name'] . '</b></td>';
                    if ($status[$j]['status'] > 0) {
                        $flag = 'Finalized';
                        $table_finalize .='<td width=600 colspan="4"><b>Status: <font color="#09C506">' . $this->lang->line('entity_mte') . ' Attainment is Finalized</font></b></td>';
                    } else {
                        $flag = 'Not_finalized';
                        $table_finalize .='<td width=600 colspan="4"><b>Status: <font color="#FA7004">' . $this->lang->line('entity_mte') . ' Attainment is not Finalized</font></b></td>';
                    }
                    $table_finalize .='</tr>';
                    $table_finalize .='<tr>';
                    ...
                    $table_finalize .='<td width=70> ' . $this->lang->line('entity_mte') . ' Threshold %</th>';
            ...
            if (!empty($section_wise_attainment)) {
                ...
                $table .= '<td width=70><b>' . $this->lang->line('entity_mte') . ' Threshold %</b></td>';
            ...
            return $this->load->view('export_document_view/assessment_attainment/tier_i/tier1_mte_clo_attainment_doc_vw', $this->data, true);
        }
    }
	    public function fetch_drilldown_attainment_data() {
        $clo_id = $this->input->post('clo_id');      
        $result_data = $this->tier1_mte_clo_attainment_model->fetch_drilldown_attainment_data($clo_id);
        ...
        foreach ($ao_attainment_tbl_details as $attainment) {
            ...
            if ($attainment['total_cia_weightage'] != '0.00') {
                $table .= '<td style="text-align: -webkit-right;">' . number_format((((float) $attainment['threshold_ao_direct_attainment'] * $attainment['total_mte_weightage']) / 100), 2, '.', '') . '%</td>';
                //$table .= '<td style="text-align: -webkit-right;">'.$attainment['threshold_ao_direct_attainment'].'</td>';
            } else {
                $table .= '<td style="text-align: -webkit-right;">' . $attainment['threshold_ao_direct_attainment'] . '%</td>';
            }
            ...
            $average_attain_after_wt = $average_attain_after_wt + number_format((((float) $attainment['threshold_ao_direct_attainment'] * $attainment['total_mte_weightage']) / 100), 2, '.', '');
        }
		...
		$json_data['mte_wieghtage'] = $ao_attainment_tbl_details[0]['total_mte_weightage'];
    ...
    function get_co_questions() {
     ...
     //   $section_id = $this->input->post('section_id');
		$occasion_not_selected = $this->input->post('occasion_not_selected');
        $questions = $this->tier1_mte_clo_attainment_model->get_co_mapped_questions($clo_id, $type, $qpd_id , $occasion_not_selected);
        if (!empty($questions)) {
        ...
        if ($questions) {
            ...
            foreach ($questions as $occa) {
                if ($occa['mt_details_name'] == 'CIA') {
                    $asses_type = $occa['ao_description'] ;//$this->lang->line('entity_mte');
                } else if ($occa['mt_details_name'] == 'TEE') {
                    $asses_type =  $occa['ao_description'] ;//$this->lang->line('entity_tee');
                } else {
    ...
    public function get_co_finalize() {
        ...
        $co_finalize = $this->tier1_mte_clo_attainment_model->co_finalize($crclm_id, $term_id, $course_id, $type_id, $occasion_id);
        if ($co_finalize == 'true') {
            echo 'true';
        } else {
            echo 'false';
	...
	public function export_to_doc() {
        ...
        $dept_name.= $this->tier1_mte_clo_attainment_model->dept_name_by_crclm_id($_POST['tier1_crclm_name']);
        ...
        if($_POST['form_name'] === 'indirect_direct_attainment_form') {
            ...
            $co_details = $this->tier1_mte_clo_attainment_model->fetch_co_details($param);
        ...
        header('Content-Disposition: attachment; filename=MTE_CLO_Attainment.doc');
