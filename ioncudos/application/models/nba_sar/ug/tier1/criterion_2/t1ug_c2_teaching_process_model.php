...
/**
 * Description          :	Model logic for Criterion 2 (TIER 1) - Teaching Learning Process.
 * Created              :	04-04-2017
...
class T1ug_c2_teaching_process_model extends CI_Model {

    /**
     * Function is to fetch list of curriculum courses.- section 2.2.2,3.2.1
    ...
    /**
     * Function is to fetch list of course question papers. - section 2.2.2,3.2.1
    ...
    public function qp_details($crclm_id, $term_id, $crs_id, $qp_type, $qpd_id) {
        ...
        $call_proce = $this->db->query('CALL nba_t1ug_c2_2_2_Question_Paper(' . $qpd_id . ')');
    ...
    public function get_quality_student_project($crclm_id = NULL, $course_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t1ug_c2_2_3_COPOMapping(' . $crclm_id . ', ' . $course_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    /**
     * Function is to fetch companies visited details.- section 2.2.4
     * @parameter   : program id
     * @return      : Array
     */
    public function fetch_program_companies_visited($pgm_id = NULL) {
        $call_proce = $this->db->query('CALL nba_t1ug_c2_2_4_CompaniesList(' . $pgm_id . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
    ...
    public function fetch_industry_internship($curriculum = NULL) {
        $call_proce = $this->db->query('CALL nba_t1ug_c2_2_5_IndustryInternship(' . $curriculum . ')');
        mysqli_next_result($this->db->conn_id);
        $proce_result = $call_proce->result_array();
