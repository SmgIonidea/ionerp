...
$('#crclm_id').on('change', function () {
    ...
    $.ajax({
	type: "POST",
	url: base_url + 'assessment_attainment/tier1_peo_attainment/direct_attainment',
	data: post_data,
	dataType: 'HTML',
	success: function (json_graph_data) {
		...
		table += '<thead>';
		table += '<tr><th><center>PEO Reference</center></th><th><center>Program Educational Outcome (PEO) Statement</center></th><th><center>Attainment %</center></th></tr>';
		table += '</thead>';
		table += '<tbody>';
		var i = 0;
		var datax = new Array();
...
$('.displayDrilldown').live('click', function () {
    ...
    $.ajax({
	type: "POST",
	url: base_url + 'assessment_attainment/tier1_peo_attainment/course_peo_attainment',
	data: post_data,
	dataType: 'JSON',
	success: function (json_graph_data) {
		...
		$('#course_peo_attainment > tbody:first').append('<tr><td><center><b>PO Statement</b></td><td><center><b> PO Attainment %</b></center></td></tr>');
		$.each(json_graph_data, function () {
		    var perc = value1[i];
		    if (perc == 0.00) {
			perc = "-";
		    ...
		    $('#course_peo_attainment > tbody:last').append('<tr><td><font color="blue">' + json_graph_data[i].po_reference + '</font>-' + json_graph_data[i].po_statement + '</td><td><center>' + perc + ' </center></td></tr>');
		    i++;
		});
...
function plot_graphs_indirect_attainment(survey_id) {
	...
	$.ajax({
	    type: "POST",
	    url: base_url + 'assessment_attainment/tier1_peo_attainment/get_survey_data_indirect_attainment',
	    data: post_data,
	    dataType: 'HTML',
		...
		if (data.length > 0) {
		    ...
		    table += '<thead><tr><th><center>PEO Reference</center></th><th><center>PEO Statement</center></th><th><center>Attainment %</center></th></tr></thead>';
		    table += '<tbody>';
		    var i = 0;
		    var datax = new Array();
		    var datay = new Array();
...
function plot_graphs_comparison_attainment(crclm_id, term_id, direct_attainment_val, indirect_attainment_val, type_data, survey_id_arr, survey_perc_arr, po_attainment_type, core_courses_cbk) {
    ...
    $.ajax({
	type: "POST",
	url: base_url + 'assessment_attainment/tier1_peo_attainment/get_direct_indirect_peo_attainment_data',
	data: post_data,
	dataType: 'JSON',
	success: function (json_graph_data) {
	    if (json_graph_data.length > 0) {
