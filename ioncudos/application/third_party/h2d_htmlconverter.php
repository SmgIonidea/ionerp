...
function htmltodocx_html_allowed_children($tag = NULL) {
  ...
  $allowed_children = array(
    ...
    'h4' => array('a', 'em', 'i', 'img', 'strong', 'b', 'br', 'span', 'code', 'u', 'sup', 'text'),
    ...
    'tbody' => array('tr'),
    'tr' => array('td', 'th'),
    'td' => array('p', 'a', 'h4', 'em', 'i', 'strong', 'b', 'ul', 'ol', 'img', 'br', 'span', 'code', 'u', 'sup', 'text', 'table'), // PHPWord does not allow you to insert a table into a table cell
    'th' => array('p', 'a','h4','em', 'i', 'strong', 'b', 'ul', 'ol', 'img', 'br', 'span', 'code', 'u', 'sup', 'text', 'table'), // PHPWord does not allow you to insert a table into a table cell
    'br' => array(),
    ...
    switch ($element->tag) {
      ...
      case 'h6':
        ...
        if (!$space_after_set) {
          ...
          // set:
		   if(!isset($element->notb) && $element->tag != 'p'){
  ...
  //notice the 'NULL' added
      ...
      case 'th':
        if (in_array($element->tag, $allowed_children) && $state['table_allowed']) {
		  ...
		  if(isset($element->gridspan)){
			$element->colspan = $element->gridspan - 1 ;
		  }else{
			$element->colspan = 1;
		  }
