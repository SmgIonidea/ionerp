 ...
 * 24-12-2015			Bhagyalaxmi S S						Modified crclm_getDeliveryMethods query
  ---------------------------------------------------------------------------- */
?>
...
class Curriculum_delivery_method_model extends CI_Model {
    ...
    public function crclm_getDeliveryMethods($crclm_id) {
        $query = 'SELECT crclm_dm_id, delivery_mtd_name, delivery_mtd_desc 
				  ...
				  WHERE crclm_id = "' . $crclm_id . '" ORDER BY delivery_mtd_name ASC';


        /*  $query = 'SELECT dm.crclm_dm_id, dm.delivery_mtd_name, dm.delivery_mtd_desc,group_concat(bl.bloom_actionverbs)
          FROM map_crclm_deliverymethod dm
          LEFT JOIN map_crclm_dm_bloomlevel mp ON mp.crclm_dm_id=dm.crclm_dm_id
          JOIN bloom_level bl ON mp.bloom_id=bl.bloom_id
          AND dm.crclm_id ="'.$crclm_id.'"
          group by(dm.crclm_dm_id)'; */
        $row = $this->db->query($query);
        $data = $row->result_array();
        return $data;
    }

    public function crclm_get_bloom_level($crclm_dm_id) {
        $query = 'SELECT bl.level , bl.description , bl.bloom_actionverbs FROM map_crclm_dm_bloomlevel m
				JOIN bloom_level bl ON m.bloom_id=bl.bloom_id 
                                JOIN map_crclm_deliverymethod as mp_crclm_del ON mp_crclm_del.crclm_dm_id = m.crclm_dm_id 
                                where mp_crclm_del.crclm_dm_id= "' . $crclm_dm_id . '"';
        $row = $this->db->query($query);
        $data = $row->result_array();
        return $data;
    }
    ...
    public function crclm_delivery_method_insert_record($delivery_method_name, $delivery_method_description, $bloom_level, $crclm_id) {
        ...
        //var_dump($blooms_level_size);exit;
    ...
    public function crclm_delivery_method_bloom_edit($delivery_method_id) {
        ...
        $blooms_level_query = 'SELECT bl.bloom_id, mp.map_crclm_dm_bloomlevel_id, bl.level, bl.learning, bl.description, bl.bloom_actionverbs
							   FROM bloom_level bl 
							   JOIN map_crclm_dm_bloomlevel mp
    ...
    public function crclm_dm_delete($crclm_dm_id) {
        ...
        $topic_dm_query = 'SELECT count(delivery_mtd_id) AS delivery_mtd_id
						 FROM topic_delivery_method 
						 WHERE delivery_mtd_id = "' . $crclm_dm_id . '"';
        $tlo_dm_data = $this->db->query($topic_dm_query);
        $topic_dm = $tlo_dm_data->row_array();

        if ($clo_dm['delivery_mtd_id'] == 0 && $tlo_dm['delivery_mtd_id'] == 0 && $topic_dm['delivery_mtd_id'] == 0) {
