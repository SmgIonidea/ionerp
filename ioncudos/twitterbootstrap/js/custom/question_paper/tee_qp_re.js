								...
								sub_que += '<input type="hidden" name="question_name_'+id+'_'+sub_que_count_val+'" id="question_name_'+id+'_'+sub_que_count_val+'" value="Q_No_'+id+'_'+sub_que_counter+'" class="input-mini question_name_'+id+'" readonly /></td>';
								 for(var i =0; i< entity_val;i++ ){
									if(msg.qp_entity[i].entity_id == 11){
										 sub_que += '<td>';
										 sub_que += '<label class="inline_class font_bold" for="inputPassword">CO<font color="red"><b>*</b></font> :</label>';
	...
	$('.qp_table').on('change','.unit_onchange', function(){
		...
		$.ajax({type: "POST",
						...
						console.log(msg);
							...
							var i;
							$('#Q_No_'+1+'_'+1).empty();	$('#questions').empty();$('#mar').empty();
							var my_ops = $('<option></option>').val('').attr('abbr','Select Q.dNO').text('Select');
							$('#Q_No_'+1+'_'+1).append(my_ops);
							var k=0;
							//for(var i=1;i<=3;i++){if(index==i){k=(Number(msg['data'][i].qp_total_unitquestion));}}
							if(index==1){k=1;} else if(index==2){ k+=(Number(msg['data'][0].qp_total_unitquestion)+Number(1));} else if(index==3){ k+=(Number(msg['data'][0].qp_total_unitquestion)+(Number(msg['data'][1].qp_total_unitquestion))+Number(1));}
							else if(index==4){ k+=(Number(msg['data'][0].qp_total_unitquestion)+(Number(msg['data'][1].qp_total_unitquestion))+(Number(msg['data'][2].qp_total_unitquestion))+Number(1));}
							else if(index==5){ k+=(Number(msg['data'][0].qp_total_unitquestion)+(Number(msg['data'][1].qp_total_unitquestion))+(Number(msg['data'][2].qp_total_unitquestion))+(Number(msg['data'][2].qp_total_unitquestion))+Number(1));}
						...
						$('#questions_one').show();
						for(var i=0;i<=c;i++){
						var qcode=msg['qcode'][i].qp_subq_code;	
						var q_marks = msg['qcode'][i].qp_subq_marks;	
...
$(document).ready(function(){
	...
	$('#Edit_FM').hide();
		if($('#FM').val()==0){$('#Edit_FM').hide();$('#toggle_FM').hide();}
		$("#example").html($(this).html());
		var v=$("#model_qp_existance").val();
		var qpd_id=$('#qpp_id').val();
		
		$('#update_header').hide();$('#edit').hide();$('#update').hide();
		if(v==1){ 
	 ...
	 //});
		...
		$('#update_header').on('click',function(){
				...
				post_data={'qpd_id':qpd_id,'qp_title':qp_title,'time':time,'Grand_total':Grand_total,'max_marks':max_marks,'qp_notes':qp_notes}
				$.ajax({type:"POST",
						url:base_url+'question_paper/manage_model_qp/update_header',
						data:post_data,
						dataType:'json',
						success:function(data){
						 $("#qp_title").prop("readonly", true); $("#Grand_total").prop("readonly", true); $("#qp_notes").prop("readonly", true);$("#total_duration").prop("readonly", true);$("#max_marks").prop("readonly", true); $("#update_header").hide(); $("#_header").hide();$('#edit').show();
						 $('#loading').hide();
						}
						});
			
		});
		$("#section_name").on('keyup',function(){ $("#l1").html($("#section_name").val());});
		$("#sec_id").on('change',function(){$("#l2").html($("#sec_id").val());});
		$('#Q_No_1_1,#sec_id2').change(function () {
					$('#save_main_que').prop('disabled', false);
					// var V1 = $('#Q_No_1_1').find(":selected").text();
					// var V2 = $('#sec_id2').find(":selected").text();
					// var n=($('#Q_No_1_1').val());
					// if(n=='1'){
							// $('#sec_id2').children().each(function (index, element) {
							// if ($(element).text() == V2) {$(this).prop('disabled', true);}
						// });}
					// else{$('select option').attr('disabled',false);}
					
					$("#ques_nme").val("Q_No_"+$("#Q_No_1_1").val()+'_'+$("#sec_id2").val());
		});
			...
			if(grand_total>=max_marks){
				...
				var all_unit_marks  = 0;
					$('.unit_max_marks').each(function(){
					 ...
					 if (isNaN(all_unit_marks)){all_unit_marks=0;}
				...
				//if(all_unit_marks > Grand_total){
					...
					var arr = [];var count =  $("#expense_table > tbody > tr").length;
					for (var i = 0; i <=count; i++) {
						arr.push({ val1: $("#units_0"+i).val(), val2: $("#ques_no_0"+i).val() ,val3:$("#marks_0"+i).val()});
					}
					
				post_data =  {'val':arr ,'b':b,'count':count,'qpf_id': qpf_id,'qpd_type':qpd_type,'crclm_id':crclm_id,'term_id':term_id,'crs_id':crs_id,'pgm_id':pgm_id,'qp_title':qp_title,'time':time ,'max_marks':max_marks,'Grand_total':Grand_total,'qp_notes':qp_notes}
		 	$.ajax({type: "POST",
				url: base_url+'question_paper/manage_model_qp/qp_def',
				data: post_data,
				dataType: 'json',
				success:function(data){if(data=="false"){$("#error_msg").html("ALL FIELDS ARE MANDATORY *");$("#qp_title").focus();}
					else if(data!="false"){$('#p_id').val(data[0]['qpd_id']);  
					window.location = base_url+'question_paper/manage_model_qp/generate_tee_qp/'+pgm_id+'/'+crclm_id+'/'+term_id+'/'+crs_id+'/5'+'/'+data[0]['qpd_id'];}
		...
		$('#save_main_que').on('click',function(){
			...
			if(validation_flag == true){
				...
				post_data =  {'qpd_id':qpd_id,'sec_name': sec_name,'qn':qn,'sec_name2':sec_name2,'sec_id':sec_id,'qes_num':qes_num,'co':co,'pi':pi,'po':po,'topic':topic,'bl':bl,'mark':mark ,'question':question}
				$.ajax({type: "POST",
				   ...
				   success: function(data){
					  var new_data = parseInt($.trim(data.marks));
											if(new_data > 0){
												...
												$('#sec_id2').find('option:selected').prop('selected',false);
												...
												$('#qp_max_marks').val($.trim(data.marks)+'/'+qp_marks[1]);
													...
													var unit_name = $('#unit_list_1_1 option:selected').text();
													data_full_modal(unit_name);
		...
		function fail_modal(msg){//$('#myModal_fail').modal('show');
				$('#loading').hide();
				var data_options = '{"text":"Sorry. This Question Number Already Exist.    ","layout":"center","type":"error","animateOpen": {"opacity": "show"}}';
									var options = $.parseJSON(data_options);
									noty(options);
									$('#loading').hide();
		}
		function exceed_fail_modal(msg){//$('#myModal_fail').modal('show');
				$('#save_main_que').prop('disabled', false);
				$('#loading').show();
				var data_options = '{"text":"Sorry. You cannot add '+msg+' because Sum of all unit marks exceeding question paper Grand total marks.    ","layout":"center","type":"error","animateOpen": {"opacity": "show"}}';
									var options = $.parseJSON(data_options);
									noty(options);
									$('#loading').hide();
				setTimeout( function() 
		...
		function data_full_modal(unit_name){//$('#myModal_data_full').modal('show');
				$('#loading').hide();
				var data_options = '{"text":"Marks Exceeding for this '+unit_name+'.","layout":"center","type":"error","animateOpen": {"opacity": "show"}}';
									var options = $.parseJSON(data_options);
									noty(options);
									$('#loading').hide();
		}
		...
		$('#example').on('click', '.edit_qp', function(e){
			...
			$.ajax({type:'POST',
					...
					$("#pi_code_1_1").multiselect('rebuild');
					$("#topic_list_1_1 option[value='" + data.topic_mapping_data[0]['actual_mapped_id'] + "']").attr("selected","selected");
					$("#topic_list_1_1").multiselect('rebuild');
					...
					$('#delet').show();
					tiny_init();
					/* var co_list=(data['re'][0]['actual_mapped_id']);
					var topic_list=(data['re'][1]['actual_mapped_id']);
					var level=(data['re'][2]['actual_mapped_id']);
					var po_list=(data['re'][3]['actual_mapped_id']);
					var po_list1=(data['res'][0]['po_reference']);
							tinymce.remove();
							$('#edit_qp').modal('show');
							$("#co_list_1_1 option[value='" + co_list + "']").attr("selected","selected");
							$("#topic_list_1_1 option[value='" + topic_list + "']").attr("selected","selected");
							$("#bloom_list_1_1 option[value='" + level + "']").attr("selected","selected");							
							$("#po_list_1_1").append($('<option>',{value:po_list1,text:po_list1}));
							$("#po_list_1_1 option[value='" + po_list1 + "']").attr("selected",true);
							$("#ques_nme").val(q_id);$('#mark').val(marks);
							$("#qp_mq_id").val(qp_mq_id);$("#question_1_1").val(qp_content);
							$("#ques_nme").prop("readonly", true);					
							$("#sec_id2").hide();$("#Q_No_1_1").hide();$('#unit_list_1_1').hide();$('#save_main_que').hide();
							$('#fetch').hide();$('#del').hide();$('#update').show();$('#delet').show();
							 tiny_init();   */
									}
			});
		...
		$.ajax({type: 'POST',
					...
					success: function(data) {
							...
							total_marks = parseInt($.trim(data));	
							$('#qp_max_marks').val(total_marks+'/'+qp_marks[1]);
					}
			});
			$('#edit_qp').modal('show');
	...
	$('#myModal_mapping_weightage').on('click','#save_weight', function(){
		...
		$('#update').on('click',function(){
			...
			post_data={'qp_subq_code':qp_subq_code,'qp_subq_marks':qp_subq_marks,'qp_mq_id':qp_mq_id,'qp_content':que_content, 'co':co,
						'po':po, 'pi':pi, 'topic':topic, 'bl':bl}
			$.ajax({type:'POST',
	...
	$('#add_ExpenseRow').click(function(){
		...
		$lastChar = $lastChar + 1;
		$newRow = "<tr> \
					<td><input type='text' id='units_0"+$lastChar+"' name='units_0"+$lastChar+"' maxlength='255' class='allownumericwithoutdecimal required '/></td> \
					<td><input type='text' id='ques_no_0"+$lastChar+"' name='ques_no_0"+$lastChar+"' maxlength='255' class='text_align_right allownumericwithoutdecimal required '/></td> \
					<td><input type='text' id='marks_0"+$lastChar+"' name='marks_0"+$lastChar+"' maxlength='11' class='text_align_right required unit_max_marks' /></td> \
										...
										if(data=="false"){
											fail_modal();
												...
												for(var j=1; j<=count;j++){
														...
														html_data += '<td><i id="'+data+'" class="icon-remove icon-black delete_Unit"> </i></td></tr>';
	...
	var all_unit_marks  = 0;
				...
				if(all_unit_marks > Grand_total){
					...
					var sub_marks = $("input[name='sub_marks[]']")
								  .map(function(){return $(this).val();}).get(); 
					post_data =  {'units':unit ,'no_q':no_q,'sub_marks':sub_marks,'qpd_id':qpd_id}
					//post_data={'val':arr}
							$.ajax({type: 'POST',
...
//Global variable.
	...
	var unit_id = $(this).attr('id');
		...
		$('#delete_Unit_modal').on('click',function(){
			...
			//var tr_val = $('#myModal5').data('tr_val');
			...
			$.ajax({type: 'POST',
					...
					success: function(data) {
						...
						//$('#Edit_FM_table tr:gt('+tr_val+')').remove();
