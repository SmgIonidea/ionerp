...
$('.qp_table').on('change', '.co_onchange', function () {
   ...
   /*              var size = $.trim(msg.po_result.length);
                 var size_bl = $.trim(msg.bl_result.length);
                var i;
                $('#bloom_list_1_1' + main_q_id + '_' + sub_q_id).empty();
                var my_ops = '';//$('<option></option>').val('').attr('abbr','Select PO').text('Select');
                $('#bloom_list_' + main_q_id + '_' + sub_q_id).append(my_ops);
                for (i = 0; i < size_bl; i++) {
                      my_ops = $('<option></option>').val(msg.bl_result[i]['bloom_id']).attr('abbr', msg.bl_result[i]['bloom_actionverbs']).attr('title', msg.bl_result[i]['level'] + ' || ' + msg.bl_result[i]['learning'] + ' || ' + msg.bl_result[i]['bloom_actionverbs'] ).text(msg.bl_result[i]['level']);
...
$('.qp_table').on('change', '.option_onchange', function () {
    ...
    var qp_crs_id = $(this).attr('qp_crs_id');
    var qp_qpd_id = $(this).attr('qp_qpd_id');
    $.ajax({
        type: "POST",
        url: base_url + 'question_paper/manage_model_qp/check_question',
        data: {'qpd_unitd_id': qpd_unitd_id, 'q_no': q_no, 'crs_id': qp_crs_id, 'qpd_id': qp_qpd_id},
...
$('.qp_table').on('change click', '.unit_onchange', function () {
...
$('.topic_list_data').on('change', function () {
    if ($(this).val() != '') {
        ...
        var id = $(this).attr('id');
        var opt_val = $("#" + id + " option:selected").attr('abbr');
        $(this).attr('data-original-title', 'se');
        $(this).tooltip('hide');
    } else {
        $(this).attr('data-original-title', 'Select CO');
    }
...
//http://115.249.77.76/ioncudos_bvcoe/configuration/users/list_users
...
$('#toggle_FM,#edit').on('click', function () {
    $("#qp_title").prop("readonly", false);
    $("#Grand_total").prop("readonly", false);
    $("#qp_notes").prop("readonly", false);
    $("#total_duration").prop("readonly", false);
    $("#max_marks").prop("readonly", false);
    if ($('#FM').val() == 0) {
        $('#update_header').hide();
    } else if ($('#FM').val() == 1) {
        $('#update_header').hide();
...
function update_header() {
    ...
    if (grand_total >= max_marks) {
        ...
        if (grand_total >= mark) {
            ...
            var qp_model = "-2";
            post_data = {'qp_model': qp_model, 'qpd_id': qpd_id, 'qp_title': qp_title, 'time': time, 'Grand_total': Grand_total, 'max_marks': max_marks, 'qp_notes': qp_notes}
            $.ajax({type: "POST",
        ...
        if (marks_sum <= parseInt(qp_marks[1])) {
            $.ajax({type: "POST",
                ...
                success: function (data) {
                    ...
                    questions_display = qes_num + ': ' + mark + ' || ';
                    $("#questions").append(questions_display);
                    if (new_data > 0) {
                        $("#unit_marks").val(" ");
                        if (data.entity == 0) {
...
function exceeind_grand_total(msg) {//$('#myModal_data_full').modal('show');
    $('#save_main_que').prop('disabled', false);
    $('#loading').hide();
    var data_options = '{"text":"The total sum of max marks of all the questions exceeding grand total marks, please verify!!!.","layout":"center","type":"error","animateOpen": {"opacity": "show"}}';
    var options = $.parseJSON(data_options);
    noty(options);
    $('#loading').hide();
}
...
$('#example').on('click', '.edit_qp', function (e) {
    ...
    var qpd_id = $('#qpp_id').val();
    //	var q_num_edit=$(this).attr('data-q_num');
    var qp_max_marks = $('#qp_max_marks').val();
    var qp_marks = qp_max_marks.split("/");
    var crclm_id = $("#crclm_id").val();
    var term_id = $('#term_id').val();
    ...
    post_data = {'qp_map_id': data_qp_map_id, // main question id
        'crclm_id': crclm_id,
        'term_id': term_id,
        'crs_id': crs_id,
        'qpd_id': qpd_id,
    ...
    $.ajax({type: 'POST',
        ...
        success: function (data) {
			...
			var qt_size = data.qt_result.length;
            ...
            $('#delet').show();
            tiny_init();
            question_remain(unit_id, qp_mq_code);//do not replace  						
        }
    });
...
$('#Add_Question').on('click', function () {
    ...
    if (unit_sum == Grand_total) {
        var qp_crs_id = $(this).attr('qp_crs_id');
        var qp_qp_id = $(this).attr('qp_qpd_id');
        ...
        $('#Q_No_1_1').find('option:selected').prop('selected', false);
        $('#sec_id2').find('option:first').prop('selected', true);
        $('#sec_id2').attr('qp_crs_id', qp_crs_id).attr('qp_qpd_id', qp_qp_id);
        $('#co_list_1_1').find('option:selected').prop('selected', false);
        $('#topic_list_1_1').find('option:selected').prop('selected', false);
        $('#bloom_list_1_1').find('option:selected').prop('selected', false);
...
/* 		$('.qp_type_data').multiselect({
		...
		$('.qp_type_data').multiselect('rebuild'); */
        tiny_init();
...
$('#update').on('click', function () {
    ...
    if (validation_flag == true) {
        ...
        post_data = {'qp_subq_code': qp_subq_code, 'qp_subq_marks': qp_subq_marks, 'qp_mq_id': qp_mq_id, 'qp_content': que_content, 'co': co, 'unit_q_no': unit_q_no,
            'po': po, 'pi': pi, 'topic': topic, 'bl': bl, 'mandatory': mandet, 'qp_mq_code': qp_mq_code , 'question_type': question_type}
        $.ajax({type: 'POST',
...
$get_lastID = function () {
    ...
    $newRow = "<tr> \
					...
					<td><input type='text' id='marks_0" + $lastChar + "' name='marks_0" + $lastChar + "' maxlength='11' class='text_align_right allownumericwithoutdecimal required unit_max_marks' /></td> \
...
$('#save_FM').on('click', function () {
    ...
    if (val_flag == true) {
        ...
        var unit_name = $('#units_01').val();
        $('.edit_unit_max_marks').each(function () {
            all_unit_marks = parseInt(all_unit_marks) + parseInt($(this).val());

        });

        if (parseInt(all_unit_marks) > parseInt(Grand_total)) {
                ...
                success: function (data) {
                    ...
                    if (data['result'] == "false") {
                        ...
                        for (var j = 1; j <= count; j++) {
                            ...
                            html_data += '<td><input type="text" name="no_q[]" id="no_q" class=" text_align_right allownumericwithoutdecimal input-mini required" value="' + $("#ques_no_0" + j).val() + '"</td>';
                            ...
                            html_data += '<td><i id="' + data['result'] + '" class="icon-remove icon-black cursor_pointer delete_Unit"> </i></td></tr>';
                        ...
                        $('#Edit_FM_table tbody').append(html_data);
                        // if(parseInt(tr_count)>0){
        ...
        if (grand_total >= max_marks) {
            ...
            if (grand_total >= mark) {
                ...
                if (validation_flag == true) {
                    ...
                    post_data = {'units': unit, 'unit_ids': unit_id, 'no_q': no_q, 'sub_marks': sub_marks, 'qpd_id': qpd_id, 'unit_marks': unit_marks}
                    $.ajax({type: 'POST',
...
$('#Edit_FM').on('click', '.delete_Unit', function (e) {
    ...
    var count = $("#Edit_FM_table > tbody > tr").length;

    if (count > 1) {
        $('#myModal5').data('id', unit_id).modal('show');
    } else {
        $("#test").html("You Can't delete");
    }
    ...
    $.ajax({type: 'POST',
        ...
        success: function (data) {
            ...
            if (data['deleted'] == true) {
                ...
                $('#Edit_FM_table_length').hide();
                $('#Edit_FM_table_paginate').hide();
...
$(function () {
   ...
   validator.settings.ignore = ':hidden:not(".co_list_data"):hidden:not(".po_list_data"):hidden:not(".topic_list_data"):hidden:not(".bloom_list_data"):hidden:not(".qp_type_data")';
    ...
    $('.co_list_data').multiselect({
        includeSelectAllOption: true,
        buttonWidth: '110px',
        nonSelectedText: 'Select ' + co_lang,
        templates: {
            button: '<button type="button" class="multiselect btn btn-link co_multi dropdown-toggle" data-toggle="dropdown"></button>'
        }
    });
    ...
    $('.po_list_data').multiselect({
        includeSelectAllOption: true,
        buttonWidth: '110px',
        nonSelectedText: 'Select ' + po_lang,
        templates: {
            button: '<button id="" type="button" class="multiselect po_multi dropdown-toggle btn btn-link" data-toggle="dropdown"></button>'
        }
...
$('#rev_assin_question_list').on('click', '.question_type', function () {
    ...
    //$('#rev_assin_question').dataTable().fnDestroy();
