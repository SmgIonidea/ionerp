...
function fetch_program() {
    ...
    if (department_id) {
        $.ajax({
            type: "POST",
            url: base_url + 'nba_sar/curriculum_student_info/fetch_program',
            data: post_data,
...
function fetch_curriculum() {
    ...
    if (program_id) {
        $.ajax({
            type: "POST",
            url: base_url + 'nba_sar/curriculum_student_info/fetch_curriculum',
            data: post_data,
            success: function (msg) {
                $("#curriculum").html(msg);
                $('#loading').hide();
...
function fetch_details() {
    $.cookie('stud_perm_curriculum', $('#curriculum option:selected').val(), {
        expires: 90, 
        path: '/'
    });
    var crclm_id = $('#curriculum').val();
    var program_id = $('#program').val();
    var pgm_type_id = $('#program option:selected').attr("data-pgm_type");
    var post_data = {
        'crclm_id': crclm_id,
        'program_id': program_id,
    ...
    if (crclm_id) {
        $.ajax({
            type: "POST",
            url: base_url + 'nba_sar/curriculum_student_info/fetch_details',
            data: post_data,
            dataType: 'json',
            success: function (msg) {
...
//function to reset the all form feilds
