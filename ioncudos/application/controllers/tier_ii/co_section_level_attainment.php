...
class Co_section_level_attainment extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('assessment_attainment/tier_ii/co_section_level_attainment/co_section_level_attainment_model');
...
//
    public function index() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $data['deptlist'] = $this->co_section_level_attainment_model->dropdown_dept_title();
            $data['crclm_data'] = $this->co_section_level_attainment_model->crlcm_drop_down_fill();
            $data['title'] = "Course Level Attainment";
            $this->load->view('assessment_attainment/tier_ii/co_section_level_attainment/co_section_level_attainment_vw', $data);
        }
    }
    ...
    public function select_term() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $term_data = $this->co_section_level_attainment_model->term_drop_down_fill($crclm_id);
    ...
    public function select_course() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $term_data = $this->co_section_level_attainment_model->course_drop_down_fill($term);
    ...
    public function select_survey() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $survey_data = $this->co_section_level_attainment_model->survey_drop_down_fill($course);
    ...
    public function select_section(){
          if (!$this->ion_auth->logged_in()) {
            ...
            $section_data = $this->co_section_level_attainment_model->section_drop_down($term,$crclm_id,$crs_id);
            if ($section_data) {
                $i = 0;
    ...
    public function select_occasion() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $occasion_data = $this->co_section_level_attainment_model->occasion_fill($crclm_id, $term_id, $crs_id , $section_id);
            $i = 0;
            $list = array();			
	...
	public function fetch_section_data(){
		...
		$section_id = $this->input->post('section_id');
        $cla_data = $this->co_section_level_attainment_model->get_crs_level_attainment($course_id);
        
		$section_list = $this->co_section_level_attainment_model->section_dropdown_list_finalize($crclm_id, $term_id, $course_id ,$section_id);
        ...
        foreach($section_result as $section_data){
            ...
            $table .='<tr>';
			//$table .="<th> Sl No.</th>";
            $table .='<th width = 150 ><b>Section /Division - '.$section_data['section_name'].'</b></th>';
                        ...
                        for($k=0;$k<$size;$k++){                            
                            if(!empty($overall_attain[$k])){
                                if($overall_attain[$k]['section_id'] == $section_data['section_id']){
                                    $table .='<tr>'; 
                                    $table .='<td width = 150  title="'. $overall_attain[$k]['clo_statement'] .'"> '. $overall_attain[$k]['clo_code'] .'</td>'; 
                                    $table .='<td width = 166  style="text-align: -webkit-right;">'.$overall_attain[$k]['attainment'].'%<br></td>'; 
									$table .='<td width = 166  style="text-align: -webkit-right;">'.$overall_attain[$k]['attainment_level'].'</td>'; 
									$table .='<td width = 166  style="text-align: -webkit-right;">'.$overall_attain[$k]['avg_attainment'].'%<br></td>';
                                    $table .='</tr>';
    ...
    function get_tire_ii_clo_attainment( $crs_id =NULL , $section_id = NULL , $qpd_id = NULL , $type_id = NULL, $export_flag = NULL, $graph = NULL, $co_details = NULL ) {   
	   if($export_flag == NULL){
		$course = $this->input->post('course');
        ...
        $col_data = $this->co_section_level_attainment_model->get_clo_attainment($course, $qpd_id, $type ,$section_id ,$occasion_not_selected);    

        if(count($col_data) == 0){ 
			$fetch_status  = $this->co_section_level_attainment_model->fetch_status($course, $qpd_id, $type ,$section_id , $occasion_not_selected);
			$data['col_data'] = $fetch_status;
		
    	}else{
		 	$occasions_status  = $this->co_section_level_attainment_model->find_occsions_finalized_or_not($course, $qpd_id, $type ,$section_id ,$occasion_not_selected); 			
    ...
    function get_co_questions() {
		...
		$section_id = $this->input->post('section_id');
		$occasion_not_selected = $this->input->post('occasion_not_selected');
        $questions = $this->co_section_level_attainment_model->get_co_mapped_questions($clo_id, $type, $qpd_id ,$section_id , $occasion_not_selected);
        if (!empty($questions)) {
        ...
        if ($questions) {
			...
			if($occa['mt_details_name'] == 'CIA'){ $asses_type = $this->lang->line('entity_cie');}else if($occa['mt_details_name'] == 'TEE'){
    ...
    function get_drilldown_for_co() {
        ...
        $crs_id = $this->input->post('crs_id'); 
		$section_id = $this->input->post('section_id');
        $co_drilldown_data = $this->co_section_level_attainment_model->get_drilldown_for_co($clo_id, $ao_id, $crs_id , $section_id);
	...
	function get_drilldown_for_co_average() {
        ...
        $crs_id = $this->input->post('crs_id');  
		$section_id = $this->input->post('section_id');
        $co_drilldown_data = $this->co_section_level_attainment_model->get_drilldown_for_co_average($clo_id, $ao_id, $crs_id , $section_id);
        if ($co_drilldown_data) {
            echo json_encode($co_drilldown_data);
        } else {
            echo 0;
    ...
    function finalize_clo_attainment() {
        ...
        $type_id = 2;
        $is_added = $this->co_section_level_attainment_model->finalize_clo_attainment($crclm_id, $term_id, $course_id , $section_id ,$type_id , $occasion );
        if ($is_added)
            echo "success";
        else
            echo "failed";
	...
	public function export_to_doc(){
	  ...
	  $this->load->helper('to_word_helper');
			...
			$selected_param = $this->co_section_level_attainment_model->fetch_selected_param_details($param);
			extract($selected_param);
    
			$dept_name = "Department of ";
			$dept_name.= $this->co_section_level_attainment_model->dept_name_by_crclm_id($_POST['crclm_name']);
			...
			$filename = $this->lang->line('entity_cie').'_'.$this->lang->line('entity_clo').'_Attainment(Section/Division_wise)';
			//'CO_Attainment';
