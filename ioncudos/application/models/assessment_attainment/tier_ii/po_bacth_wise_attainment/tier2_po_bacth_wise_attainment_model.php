 ...
 */
  ...
  class Tier2_po_bacth_wise_attainment_model extends CI_Model 
  {
...
/*
        ...
        public function po_attainment_data($crclm_id,$po_id){
            $po_attainment_query = 'SELECT term_name, po_reference, po_statement, po_min, avg_po_attain, avg_po_att_lvl, avg_da, thr_po_att_lvl,
                                    hml_wieghted_avg_da, hml_avg_att_lvl, hml_wgtd_multiply_map_da, hml_wtd_avg_mul_att_lvl, crs_id, crclm_term_id, crclm_id
                                    FROM (
                                    SELECT term.term_name, p.po_reference,p.po_statement,IF(p.po_minthreshhold is null,0,p.po_minthreshhold) as po_min, 
                                    ROUND(AVG(IF(at.average_po_direct_attainment is null,0,at.average_po_direct_attainment)),2) as avg_po_attain, 
                                    ROUND(IF(at.average_po_attainment_level is null,0,at.average_po_attainment_level)) as avg_po_att_lvl ,
                                    ROUND(AVG(IF(at.threshold_po_direct_attainment is null,0,at.threshold_po_direct_attainment)),2) as avg_da, 
                                    ROUND(IF(at.threshold_po_attainment_level is null,0,at.threshold_po_attainment_level))as thr_po_att_lvl, 
                                    ROUND(AVG(IF(at.hml_weighted_average_da is null,0,at.hml_weighted_average_da)),2) as hml_wieghted_avg_da, 
                                    ROUND(IF(at.hml_wtd_avg_attainment_level is null,0,at.hml_wtd_avg_attainment_level)) as hml_avg_att_lvl,
                                    ROUND(AVG(IF(at.hml_weighted_multiply_maplevel_da is null,0,at.hml_weighted_multiply_maplevel_da)),2) as hml_wgtd_multiply_map_da , 
                                    ROUND(IF(at.hml_wtd_avg_mul_attainment_level is null,0,at.hml_wtd_avg_mul_attainment_level)) as hml_wtd_avg_mul_att_lvl,
                                    at.crs_id, at.crclm_term_id, at.crclm_id  
                                    FROM tier_ii_po_direct_attainment as at
                                    ...
                                    UNION
                                    SELECT terms.term_name, 0 po_reference, 0 po_statement, 0 po_min,0 avg_po_attain, 0 avg_po_att_lvl, 0 avg_da, 0 thr_po_att_lvl, 0 hml_wieghted_avg_da, 0 hml_avg_att_lvl,
                                    0 hml_wgtd_multiply_map_da,0 hml_wtd_avg_mul_att_lvl, 0 crs_id, terms.crclm_term_id, terms.crclm_id
                                    FROM crclm_terms as terms
                                    LEFT JOIN tier_ii_po_direct_attainment as att ON att.crclm_id = terms.crclm_id AND att.crclm_term_id = terms.crclm_term_id
