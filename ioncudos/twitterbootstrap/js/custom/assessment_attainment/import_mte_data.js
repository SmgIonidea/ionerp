	...
	$.cookie('remember_dept',$('#department option:selected').val(),{expires:90, path:'/'});
		...
		$.ajax({type: "POST",
			url: base_url+'assessment_attainment/import_mte_data/select_pgm_list',
			data: post_data,
	...
	function select_crclm_list()
	{
		...
		$.ajax({type: "POST",
			url: base_url+'assessment_attainment/import_mte_data/select_crclm_list',
			data: post_data,
			success: function(msg) {
	...
	function select_termlist() {
		...
		$.ajax({type: "POST",
			url: base_url+'assessment_attainment/import_mte_data/select_termlist',
			data: post_data,
			success: function(msg) {
	...
	*/	
	...
	$.cookie('remember_term',$('#term option:selected').val(),{expires:90, path:'/'});
		...
		$.ajax({type: "POST",
			url: base_url+'assessment_attainment/import_mte_data/show_course',
			data: post_data,
			dataType: 'json',
	...
	function populate_table(msg) {
		...
		$('#example').dataTable(
				{"aoColumns": [
						...
						{"sTitle": "View "+entity_mte+" Details", "mData": "cia_details"},
						{"sTitle": "Manage "+entity_mte+" MArks", "mData": "manage_cia"},
...
//                    bHideGroupingColumn: true});
	...
	$('#example').on('click','.get_cia_details', function(e) {
		...
		var mte_flag = $(this).attr('data-mte_flag');
		var post_data = {
			'crs_id': data_rowId,
			'pgm_id': pgm_id,
			'crclm_id':crclm_id,
...
/* 		$.ajax({type: "POST",
			url: base_url + 'assessment_attainment/import_mte_data/get_cia_details',
			data: post_data,
			datatype: "JSON",
			success: function(msg) {
			...
			$.ajax({type: "POST",
						url: base_url + 'assessment_attainment/import_mte_data/get_mte_details',
						data: post_data,
						datatype: "JSON",
						success: function(msg) {
						//	document.getElementById('get_cia_details_list').innerHTML = msg;
							$('#get_mte_details_list').html(msg);
						}
					});
...
$('#generate').on('click','.import_data_details',function(e){
    ...
    $.ajax({type: "POST",
			url: base_url + 'assessment_attainment/import_mte_data/get_organisation_type',
			data: post_data,
			dataType: "JSON",
			success: function(msg) {
...
$('#generate').on('click','.qp_download_template',function(){
   ...
   $.ajax({ type: "POST",
            url: base_url + 'assessment_attainment/import_mte_data/check_student_uploaded_or_not',
            data: post_data,
            dataType: "JSON",
            success: function(msg) {
...
$('#generate_mte').on('click','.qp_download_template',function(){
   var crclm_id = $(this).attr('data-crclm_id');
   var download_excel_url = $(this).attr('abbr_href');
   var post_data = {
       'crclm_id':crclm_id,
...
$('#myModal_cia_details').on('click','.view_rubrics',function(){
    ...
    $.ajax({type: "POST",
	url: base_url + 'assessment_attainment/import_mte_data/get_rubrics_table_modal_view',
	data: post_data,
	dataType: 'html',
