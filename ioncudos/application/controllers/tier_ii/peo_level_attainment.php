...
class Peo_level_attainment extends CI_Controller {
	...
	$this->load->model('assessment_attainment/tier_ii/peo_level_attainment/peo_level_attainment_model');
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $data['po_attainment_type'] = $this->peo_level_attainment_model->getPOAttainmentType();
	    $data['crclmlist'] = $this->peo_level_attainment_model->crlcm_drop_down_fill();
	    $data['title'] = "PEO Level Attainment";
	    $this->load->view('assessment_attainment/tier_ii/peo_level_attainment/peo_level_attainment_vw', $data);
	}
    }
	...
	$peo_data = $this->peo_level_attainment_model->direct_attainment_list($crclm_id);
	if ($peo_data) {
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $crclm_id = $this->input->post('crclm_id');
	    $survey_data = $this->peo_level_attainment_model->survey_drop_down($crclm_id);
	...
	$survey_data = $this->peo_level_attainment_model->get_survey_data($crclm_id, $survey_id);
	if ($survey_data) {
	...
	$graph_data = $this->peo_level_attainment_model->get_course_peo_attainment($crclm_id, $peo_id);
	...
	$survey_data = $this->peo_level_attainment_model->get_survey_names();
	++$counter;
	$add_row = "";
	$add_row .="<tr>";
	...
	$crclm_id = $this->input->post('crclm_id');
	$result = $this->peo_level_attainment_model->get_finalized_peo_data($crclm_id);
	...
	if (!$this->ion_auth->logged_in()) {
	    ...
	    $graph_data = $this->peo_level_attainment_model->get_direct_indirect_peo_attainmentData($crclm_id, $qpd_id, $usn, $qpd_type, $direct_attainment, $indirect_attainment, $survey_id, $survey_perc_arr, $po_attainment_type, $core_courses_cbk);
