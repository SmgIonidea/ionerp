...
class Users_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if (isset($additional_data['bos_dept_id'])) {
            $dept_name = $this->db->select('dept_name')
                    ->where('dept_id', $additional_data['bos_dept_id'])
                    ->get('department')
                    ->result_array();
            /* 				 
            ...
            $additional_data['username'] = $email;
            $additional_data['password'] = $password1;
            $this->ion_auth->send_email($id, $cc = NULL, $links, $entity_id, $state = 2, $crclm_id = NULL, $additional_data);
        } else {
            //filter out any data passed that doesnt have a matching column in the users table
            //and merge the set user data and the additional data
            $dept_name = $this->db->select('dept_name')
                    ->where('dept_id', $additional_data['base_dept_id'])
                    ->get('department')
                    ->result_array();
            $links = base_url();
            $entity_id = '7';
        ...
        if (!empty($groups)) {
            ...
            foreach ($groups as $group) {
                ...
                // to insert user_id, dept_id, role_id onto `map_user_dept_role` table - multi-user dept model.
                $user_dept_role_data = array(
                    'user_id' => $id,
                    //'dept_id' => $additional_data['user_dept_id'],
                    'dept_id' => $additional_data['base_dept_id'],
                    'role_id' => $group,
                ...
                // User will be included under BoS table if BoS role is assigned 
                if ($group == 15) {
                    $user_bos_role = array(
                        'bos_user_id' => $id,
                        'bos_dept_id' => $additional_data['base_dept_id'],
                    );
                    $this->db->insert('bos', $user_bos_role);
                }
            }
        }
    ...
    public function row() {
        ...
        if (empty($row)) {
            ...
            $row->dept_name = $this->get_user_dept_name($row->user_dept_id); //to display user dept name.
            $row->org_name = $this->get_org_name(); //to display user dept name.
            $this->response->free_result();
            return $row;
        }
    }

    // Function to get user department name.
    public function get_user_dept_name($dept_id = NULL) {
        if (empty($dept_id)) {
            return 0;
        } else {
            $query = $this->db->query('SELECT dept_name , dept_id
										FROM department
										WHERE dept_id = "' . $dept_id . '"  LIMIT 1');
            $row_data = $query->row();
            return $row_data->dept_name;
        }  // returns the single value in the form of object.
    }

    // Function to get user department name.
    public function get_org_name() {
        $query = $this->db->query('SELECT org_name,org_type, oe_pi_flag, theory_iso_code, lab_iso_code , mte_flag
									FROM organisation
     ...
     * 02-09-2013		   Arihant Prasad			File header, function headers, indentation 
      and comments.
     *
      ---------------------------------------------------------------------------------------------- */

    /**
     * Function checks for user permission and fetches groups list as array
     * @parameters: group list and permission allocated
     * @return boolean 
     */
    public function has_permission($group_list, $permission_name) {
        $query = $this->db->query("SELECT permission_id FROM `usergroup_permission` WHERE usergroup_id IN ($group_list) AND permission_id = (SELECT permission_id FROM permission WHERE permission_function = '$permission_name')");
    ...
    public function user_number($username, $department_id) {
        $query = (($department_id > 0) ? ($this->db->query("SELECT id FROM users WHERE username LIKE '%$username%' AND user_dept_id = '$department_id'")) : ($this->db->query("SELECT id FROM users WHERE username LIKE '%$username%'")));
    ...
    /* This function is also known as default_department
     * Function is used to department related to general department for guest/new users
     * @return: department id
     */

    function general_department() {
    ...
    public function get_department_name_by_id($department_id) {
        ...
        return $this->db->get_where('department', array('dept_id' => $department_id))->result_array();
    }

    /* Function is used to add more than one user at the same time by uploading .csv file 
     * @parameters: file name to be uploaded 
     */

    public function bulk_upload($file_name) {
        $file_path = $_SERVER['DOCUMENT_ROOT'] . 'ionbvbcd/uploads/' . $file_name;

        $query = "LOAD DATA LOCAL INFILE '$file_path' INTO TABLE temp_user_list FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\\\\' LINES TERMINATED BY '\\r\\n' IGNORE 1 LINES";
        $this->db->query($query);
        $this->db->query("CALL curriculum.importusers();");
    }

    public function get_all_dept() {
        $dept_list_data['logged_in_dept'] = $this->ion_auth->user()->row()->user_dept_id;
        $dept_list = $this->db
                ->select('dept_id,dept_name , dept_acronym')
                ->order_by('dept_name', 'asc')
                ->get('department');
        $dept_list_result = $dept_list->result_array();
 /*        $user_list = 'SELECT u.id, u.first_name, u.title,u.last_name, u.email, u.user_dept_id, u.active, d.dept_name, d.dept_acronym , ud.designation_name
						FROM users u, department d,user_designation ud
						WHERE u.user_dept_id = d.dept_id
						AND u.designation_id = ud.designation_id
						AND d.dept_id =' . $dept_list_data['logged_in_dept']; */
						
		$user_list = 'SELECT group_concat(g.name ORDER BY FIELD(g.id, 1, 16, 10, 6, 15, 2, 5)  SEPARATOR ", ") as group_name ,u.id, u.title, u.first_name, u.last_name, u.email, u.user_dept_id ,u.base_dept_id, u.active, d.dept_name,d.dept_acronym , ud.designation_name
						FROM  users u, department d,user_designation ud , groups g , users_groups ug
        ...
        $user_list_result = $this->db->query($user_list);
        $dept_list_data['user'] = $user_list_result->result_array();
        $dept_list_data['dept'] = $dept_list_result;
     ...
     */
    public function user_list($dept_id) {

/*         $user_list = 'SELECT u.id, u.title, u.first_name, u.last_name, u.email, u.base_dept_id, u.active, d.dept_name,d.dept_acronym , ud.designation_name
						FROM users u, department d,user_designation ud
						WHERE u.base_dept_id = d.dept_id
						AND u.designation_id = ud.designation_id
						AND u.is_student != 1						
						AND d.dept_id =' . $dept_id; */
		$user_list = 'SELECT group_concat(g.name ORDER BY FIELD(g.id, 1, 16, 10, 6, 15, 2, 5)  SEPARATOR ", ") as group_name  ,u.id, u.title, u.first_name, u.last_name, u.email, u.base_dept_id, u.active, d.dept_name,d.dept_acronym , ud.designation_name
						FROM  users u, department d,user_designation ud , groups g , users_groups ug
						WHERE u.base_dept_id = d.dept_id
						AND u.designation_id = ud.designation_id
						AND u.id = ug.user_id
						...
						group by u.id';
        $user_list_result = $this->db->query($user_list);
        $user_list_data['user'] = $user_list_result->result_array();
...
// End of function ao_method_list.
     ...
     * Location: .configuration/users/users_model.php */
}
