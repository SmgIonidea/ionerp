/*
 * Description	:	Tier II MTE CO Level Attainment Script
...
function select_crclm() {
 $('#occasion').trigger('change');
    ...
    var crclm_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/select_crclm';
...
$('#finalize_attainment').empty();
 $('#occasion').trigger('change');
    ...
    empty_all_divs();
    var term_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/select_term';
		...
		$.ajax({
			...
			success: function (msg) {
				...
				if ($.cookie('remember_term') != null) {
					...
					select_course(); $('#occasion').trigger('change');
				}
...
function select_course() {
 $('#occasion').trigger('change');
    ...
    empty_all_divs();
    var course_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/select_course';
...
$('.navbar1').hide();
 ...
 var crs_id = $('#course').val();  
    ...
    if (type_data == 2 && crs_id) {
        ...
        $.ajax({
            type: "POST",
            url: base_url + 'tier_ii/mte_co_section_level_attainment/select_occasion',
            data: post_data1,
            success: function (occasionList) {
				...
				$.ajax({
					type: "POST",
					url: base_url + 'tier_ii/mte_co_section_level_attainment/fetch_section_data',
					data: post_data1,
					dataType: "json",
					success: function (success_msg) {
							    if(success_msg.table_finalize_flag == 'Finalized'){										
									...
									$('#section_finalize_status_tbl').show();	$('#finalised_data').show();
...
function show_finalized_co_grid() {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/get_finalized_co_attainment_data',
        data: post_data,
        dataType: 'HTML',
...
$('#occasion').on('change', function () {
   ...
   tier_ii_plot_graphs(course, qpd_id, type , occasion_not_selected);
});

function tier_ii_plot_graphs(course, qpd_id, type , occasion_not_selected) {
   ...
   // $('#loading').show();
	...
	if($('#crclm_name').val() =='' || $('#term').val() == '' || $('#course').val() == '' ||  $('#occasion').val() == null ){
		$('#export_doc').prop('disabled', true);
	}else{
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/get_tire_ii_clo_attainment',
        data: post_data,
        dataType: 'HTML',
		...
		//	table_data += '<tr><td>'+attainment+'</td></tr>';
			...
			if (data['col_data'].length != 0){
                ...
                if (occasion_not_selected == 0 && $('#section_id').val() != '' ) {	
					...
					var error_msg  = '';	var j = 0;
						 ...
						 $('#co_level_nav').empty('');
						 $('#co_level_nav').append('<div class="span12 bs-docs-example"><b><span class="" style = "color:red"> You cannot Finalize the Course - ' + entity_mte + '-' + course_outcome +' Attainment. Kindly complete the below activities : <br/>  '+ error_msg +'</span></b><center><p><a href="'+base_url+'assessment_attainment/import_mte_data/"class="cursor_pointer" target="_blank" >Click here to upload course '+ entity_mte +' data </a></p></center></div><br/><br/><br/><br/>')
					}
                } else {
						var error_msg  = '';	var j = 0;
						 $.each(data['occasions_status'], function () {
						 ...
						 $('#co_level_nav').empty('');
						if(error_msg != ""){ $('#co_level_nav').append('<div class="span12 bs-docs-example"><b><span class="" style = "color:red"> You cannot Finalize the Course - ' + entity_mte + '-' + course_outcome +' Attainment  . Kindly complete the below activities : <br/>  '+ error_msg +'</span></b><center><p><a href="'+base_url+'assessment_attainment/import_mte_data/"class="cursor_pointer" target="_blank" >Click here to upload course '+ entity_mte +' data </a></p></center></div><br/><br/><br/><br/>');}
                    $('#finalize_attainment').html("");
                }
            }
			}else{
				$('#co_level_attain_data').hide();
				if(qpd_id == "all_occasion"){
				$('#chart_plot_1').append('<div class="span12 bs-docs-example"><b><span class="" style = "color:red"> You cannot Finalize the Course - ' + entity_mte + '-' + course_outcome +' Attainment. Kindly complete the below activities : <br/>  '+ data['col_data'][0]['status_data'] +'</span></b><center><p><a href="'+base_url+'assessment_attainment/import_mte_data/"class="cursor_pointer" target="_blank">Click here to upload course '+ entity_mte +' data</a></p> </center></div>');
				}else{$('#chart_plot_1').append('<br/><div class="" ><center><p><b><span class = "" style="color:red">' + data['col_data'][0]['status_data'] + '</span></b></p><p><a href="'+base_url+'assessment_attainment/import_mte_data/"class="cursor_pointer" target="_blank" >Click here to upload course '+ entity_mte +' data </a></p></center></div>');}
				$('#export_doc').prop('disabled', true);
				$("#finalize_attainment").empty();
			}
            $('#loading').hide();
...
function get_course_attainment_levels(course_id) {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/display_course_level_attainemnt',
        data: post_data,
        async: false,
        dataType: 'HTML',
        success: function (json_cla_data) {
            ...
            if (cla_data.length != 0) {
                ...
                $.each(cla_data, function () {
                    ...
                    table += '<td width = 300 >' + cla_data[j].mte_direct_percentage + '% students scoring ' + cla_data[j].conditional_opr + ' ' + cla_data[j].mte_target_percentage + '% marks out of relevant<br/> maximum marks.</td>';
                    table += '</tr>';
                    j++;
                });
                table += '</tbody>';
...
$(document).on('click', '.view_questions', function () {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/get_co_questions',
        data: post_data,
        dataType: 'HTML',
        success: function (json_clo_data) {		
...
$(document).on('click', '.view_drilldown', function () {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/get_drilldown_for_co',
        data: post_data,
        dataType: 'HTML',
        success: function (json_clo_data) {
            ...
            if (arr_data.query_data.length > 0) {
                ...
                var table = '';
                table += '<div class="row-fluid"><div class="span6"><b>' + entity_mte + ' Weight: </b>' + arr_data.wgt_arr.cia_wgt + '% </div></div><hr/>';
...
$(document).on('click', '.view_drilldown_average', function () {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/get_drilldown_for_co_average',
        data: post_data,
        dataType: 'HTML',
        success: function (json_clo_data) {
            var arr_data = jQuery.parseJSON(json_clo_data);
            if (arr_data.query_data.length > 0) {
                ...
                table += '<div class="row-fluid"><div class="span6"><center><b>' + entity_mte + ' sd Weight: </b>' + arr_data.wgt_arr.cia_wgt + '% </center></div></div>';
                table += '<table class="table table-bordered table-stripped">';
...
$('#finalize_clo_attainment').on('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/finalize_clo_attainment',
        data: post_data,
        dataType: 'HTML',
        success: function (msg) {
...
function select_pgm_indirect() {
    ...
    empty_all_divs();
    var pgm_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/select_pgm';
...
function select_survey() {
    ...
    empty_all_divs();
    var course_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/select_survey';
...
/* 	function comparison_direct_indirect(){
 ...
 var course_list_path = base_url + 'tier_ii/mte_co_section_level_attainment/getDirectIndirectCOAttaintmentData';
        ...
        $.ajax({
            ...
            success: function (occasionList) {
                if (occasionList != 0) {
                    ...
                    $('#survey_occasion_actual_data_div').html('<font color="red">Continuous Internal Assessment ('+ entity_mte +') Occasions are not defined</font>');
                }
            }
        });
...
$('#finalize_direct_indirect_confrim').on('click', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/finalize_getDirectIndirectCOAttaintmentData',
        data: post_data,
        dataType: 'JSON',
        success: function (msg) {
            if (msg != 0) {
...
function plot_graphs_comparison_attainment(survey_id, course_id, direct_attainment_val, indirect_attainment_val, type_data, occasion) {
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'tier_ii/mte_co_section_level_attainment/getDirectIndirectCOAttaintmentData',
        data: post_data,
        dataType: 'JSON',
...
values = (values.slice(1,-1));
	...
	if($("#occasion").val() == 'all_occasion'){ var occasion_name = values.join(',');}else{var occasion_name = $('#occasion :selected').text();}
		...
		$('#head_data').html('<table class="table table-bordered" style="width:100%;"><tr><td width=325><b needAlt=1 class="h_class font_h ul_class" style="font-weight:normal;">Curriculum : </b><b needAlt=1 class="font_h ul_class">'+ crclm_name +'</b></td><td width=325><b needAlt=1 class="h_class font_h ul_class" style="font-weight:normal;">Term : </b><b needAlt=1 class="font_h ul_class">'+ term_name +'</b></td></tr><tr><td width=325><b needAlt=1 class="h_class font_h ul_class" style="font-weight:normal;">Course : </b><b needAlt=1 class="font_h ul_class">'+ course_name +'</b></td></tr><tr><td width=325><b needAlt=1 class="h_class font_h ul_class" style="font-weight:normal;">Occasion : </b><b needAlt=1 class="font_h ul_class">'+ occasion_name +'</b></td></tr></table><br/><b><center> '+ course_outcome +'Attainment </center></b><br/>');
